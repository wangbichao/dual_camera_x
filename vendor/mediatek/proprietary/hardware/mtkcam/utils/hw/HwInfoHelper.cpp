...
{
    return  mpImp->getImgoFmt(bitDepth, fmt);
}
...
{
    return  mpImp->getRrzoFmt(bitDepth, fmt);
}
...
{
    return  mpImp->alignPass1HwLimitation(pixelMode, imgFormat, isImgo, size, stride);
}
...
#define RRZO_SCALE_RATIO_10X    (4)
    if( result.w < sensorSize.w * RRZO_SCALE_RATIO_10X / 10 )
    {
        ...
        result = MSize(sensorSize.w * RRZO_SCALE_RATIO_10X / 10, result.h);
    }
    if( result.h < sensorSize.h * RRZO_SCALE_RATIO_10X / 10 )
    {
        MY_LOGD("lower: refine %dx%d", result.w, result.h);
        result = MSize(result.w, sensorSize.h * RRZO_SCALE_RATIO_10X / 10);
    }
    //
    if( result.w > sensorSize.w) {
        MY_LOGD("upper: refine %dx%d", result.w, result.h);
        result.w = sensorSize.w;
    }
    if( result.h > sensorSize.h ) {
        MY_LOGD("upper: refine %dx%d", result.w, result.h);
        result.h = sensorSize.h;
    }
    //
    return MTRUE;
