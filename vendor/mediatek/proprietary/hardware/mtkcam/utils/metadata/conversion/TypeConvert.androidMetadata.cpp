...
convert(const IMetadata &rMetadata, camera_metadata* &pDstMetadata) const
{
    ...
    MY_LOGD("Allocating %zu entries, %zu extra bytes from HAL modules", entryCount, dataCount);
...
tryToConvert(const IMetadata &rMetadata, camera_metadata* &pDstMetadata, size_t &entryCount, size_t &dataCount) const
{
    ...
    if(current_entryCount > entryCount || current_dataCount > dataCount) {
        CAM_LOGD("tryToConvert buffer not enough, original entryCount = %zu, dataCount = %zu, new entryCount = %zu, dataCount = %zu", entryCount, dataCount, current_entryCount, current_dataCount);
