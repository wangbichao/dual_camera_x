...
convert(const camera_metadata *pMetadata, IMetadata &rDstBuffer) const
{
    ...
    for (unsigned int i = 0; i < get_camera_metadata_entry_count(pMetadata); i++) {
        ...
        CAM_LOGV(" android (tag: 0x%x, type: %d), data count: %zu, mtk (tag: 0x%x, name: %s, type: %d)", android_entry.tag, android_type, android_entry.count, mtk_tag, getTagInfo()->getName(mtk_tag), mtk_type);
