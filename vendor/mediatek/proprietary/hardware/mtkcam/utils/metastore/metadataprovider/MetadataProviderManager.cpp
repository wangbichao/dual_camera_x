...
add(int32_t deviceId, IMetadataProvider* pProvider)
{
    ...
    MY_LOGD("[%zd] deviceId:%d -> %p", index, deviceId, pProvider);
    return index;
}
 ...
 ******************************************************************************/
ssize_t
remove(int32_t deviceId)
{
    RWLock::AutoWLock _l(gRWLock);
    ssize_t index = gMap.removeItem(deviceId);
    MY_LOGD("[%zd] deviceId:%d removed", index, deviceId);
    return index;
}
