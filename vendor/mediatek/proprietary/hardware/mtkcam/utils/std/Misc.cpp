...
/******************************************************************************
 * get the bit depth for IMGO and RRZO raw
 ******************************************************************************/
int
getRawBitDepth()
{
    MINT32 ret = 10;//default 10 bit raw
    MINT32 bitDepth = property_get_int32("debug.camera.raw.bitdepth", -1);
    switch( bitDepth )
    {
        case 10:
            ret = 10;
    ...
    MY_LOGD("property(%d), ret(%d)", bitDepth, ret);
    return ret;
}
...
{
    int32_t level = ::property_get_int32("persist.mtk.camera.log_level", -1);
    ALOGD("###### get camera log property =%d", level);
    if  (-1 == level) {
...
void setLogLevelToEngLoad(bool is_camera_on_off_timing, bool set_log_level_to_eng)
{
#if (1 == MTKCAM_USER_DEBUG_LOAD)
    char value[16];
    if(is_camera_on_off_timing) {
        if(set_log_level_to_eng) {
            sprintf(value,"%d",MTKCAM_ANDROID_LOG_MUCH_COUNT);
            property_set("logmuch.detect.value",value);
            MY_LOGI("###### set log level to %s",value);
        }
        else {
            property_set("logmuch.detect.value","0");
