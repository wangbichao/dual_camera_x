...
drawScanLine(int imgWidth, int imgHeight, void* virtAddr, int bufSize, int imgStride)
{
    ...
    MY_LOGD("para:(w,h,s,VA,size)=(%d,%d,%d,%p,%d) prop:(v,h,w,s)=(%d,%d,%d,%d) line:(h,d)=(%d,%d)",
            imgWidth,
            imgHeight,
            imgStride,
            virtAddr,
    ...
    if(width > 0)
    {
        if (fullscreen)
        {
            ...
            {
                memset((void*)((char*)virtAddr+widthShift+imgStride*i),
                               value,
                               width);
            }
        }
        else
        {
            ...
            {
                memset((void*)((char*)virtAddr+widthShift+imgStride*displacement+imgStride*i),
                               value,
                               width);
            }
        }
    ...
    {
        memset((void*)((char*)virtAddr+imgStride*displacement),
                       value,
