...
#include <ion/ion.h>
//
static MINT32 gIonDevFD = -1;
static MINT32 gIonCount = 0;
//
/******************************************************************************
 *
 ******************************************************************************/
...
{
    MY_LOGD_IF(getLogCond(), "[%s] this:%p %dx%d format:%#x, gIonCount(%d)",
        mCallerName.string(), this, mImgSize.w, mImgSize.h, mImgFormat, gIonCount);
    //
...
)
{
    ...
    //
    MY_LOGD_IF(getLogCond(), "[%s] this:%p %dx%d format:%#x, gIonCount(%d)",
        mCallerName.string(), this, imgSize.w, imgSize.h, imgFormat, gIonCount);
    //
    return  initLocked();
}
...
initLocked()
{
    MBOOL ret = MFALSE;
    //
    if  ( 0 == android_atomic_inc(&gIonCount))
    {
        gIonDevFD = ::mt_ion_open("HalMemoryAdapter");
        if  ( 0 > gIonDevFD )
        {
...
uninitLocked()
{
    ...
    if  ( 0 >= gIonCount )
    {
        MY_LOGW("%s@ No User (%d)", getMagicName(), gIonCount);
        return  MTRUE;
    }
    //
    if  ( 1 == android_atomic_dec(&gIonCount) )
    {
        if  ( 0 <= gIonDevFD )
        {
            ::ion_close(gIonDevFD);
...
helpMapPhyAddr(char const* szCallerName, HelperParamMapPA& rParam)
{
    ...
    if ( 0 > mBufMap.indexOfKey(memID) )
    {
        ...
        else
        {
            ...
            if(ion_import(gIonDevFD, memID, &pIonHandle))
            {
            ...
            if(ion_free(gIonDevFD, pIonHandle))
            {
