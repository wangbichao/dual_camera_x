...
impInit(BufInfoVect_t const& rvBufInfo)
{
    MBOOL ret = MFALSE;
    //
    mIonDevFD = ::mt_ion_open(mCallerName.string());
    if  ( 0 > mIonDevFD )
    {
...
impUninit(BufInfoVect_t const& /*rvBufInfo*/)
{
    for (MUINT32 i = 0; i < mvBufInfo.size(); i++)
    {
        ...
        if( pBufInfo->va ) {
            ...
            pBufInfo->va = 0;
            pBufInfo->iProt_flag_va = 0;
        }
        if( pBufInfo->pa ) {
            doUnmapPhyAddr(getMagicName(), *pHeapInfo, *pBufInfo);
            pBufInfo->pa = 0;
        }
        //
        doDeallocIon(*pHeapInfo, *pBufInfo);
...
doAllocIon(MyHeapInfo& rHeapInfo, MyBufInfo& rBufInfo)
{
    ...
    err = ::ion_alloc_camera(mIonDevFD, rBufInfo.sizeInBytes, rBufInfo.iBoundaryInBytesToAlloc, ion_prot_flags, &rHeapInfo.pIonHandle);
    if  ( 0 != err )
    {
...
doDeallocIon(MyHeapInfo& rHeapInfo, MyBufInfo& /*rBufInfo*/)
{
    ...
    //
    //  ion_free: buf handle
    if  ( 0 != rHeapInfo.pIonHandle )
    {
        ::ion_free(mIonDevFD, rHeapInfo.pIonHandle);
        rHeapInfo.pIonHandle = 0;
    }
}
...
)
{
    ...
    for (MUINT32 i = 0; i < rvBufInfo.size(); i++)
    {
        ...
        sp<MyBufInfo> pStored = mvBufInfo[i];
        //
        //  SW Access.
        if  ( 0 != (usage & eBUFFER_USAGE_SW_MASK) )
        {
            ...
            if( ( pStored->iProt_flag_va ^ prot_flag) & prot_flag ) {
                if( pStored->va )
                    ::ion_munmap(mIonDevFD, (void *)pStored->va, pStored->sizeInBytes);
                pStored->va = (MUINTPTR)::ion_mmap(mIonDevFD, NULL, pStored->sizeInBytes, prot_flag, MAP_SHARED, pHeapInfo->heapID, 0);
                pStored->iProt_flag_va = prot_flag;
            }
            //
            pBufInfo->va = pStored->va;
            if  ( 0 == pBufInfo->va || -1 == pBufInfo->va )
            {
                MY_LOGE(
                    "ion_mmap returns %#" PRIxPTR " - DevFD:%d BufFD:%d BufSize:%zu",
        ...
        if  ( 0 != (usage & eBUFFER_USAGE_HW_MASK) )
        {
            if( pStored->pa == 0 )
            {
                if  ( ! doMapPhyAddr(szCallerName, *pHeapInfo, *pStored) )
                {
                    MY_LOGE("%s@ doMapPhyAddr at %d-th plane", szCallerName, i);
                    goto lbExit;
                }
            }
            pBufInfo->pa = pStored->pa;
        }
    }
    //
    ret = MTRUE;
