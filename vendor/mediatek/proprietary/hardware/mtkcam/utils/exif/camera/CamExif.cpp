    ...
    , mpDebugIdMap(NULL)
{
    ...
    if ( mLogLevel == 0 ) {
        mLogLevel = ::property_get_int32("debug.camera.log.exif", 0);
    }
...
~CamExif()
{
    MY_LOGD_IF(mLogLevel >= 1, "- this:%p", this);
}
...
)
{
    ...
    MY_LOGI("u4GpsIsOn(%d), u4GPSAltitude(%d), uGPSLatitude(%s), uGPSLongitude(%s), uGPSTimeStamp(%s), uGPSProcessingMethod(%s), u4Orientation(%d), u4Facing(%d)",
            mCamExifParam.u4GpsIsOn, mCamExifParam.u4GPSAltitude, mCamExifParam.uGPSLatitude, mCamExifParam.uGPSLongitude,
            mCamExifParam.uGPSTimeStamp, mCamExifParam.uGPSProcessingMethod, mCamExifParam.u4Orientation, mCamExifParam.u4Facing);
    MY_LOGI("u4ZoomRatio(%d), u4ImgIndex(%d), u4GroupId(%u), u4FocusH/L(%d/%d), u4RefocusPos(%d), uJpsFileName(%s), u4ShotMode(%d), u4CamMode(%d), bStereoCam(%d)",
            mCamExifParam.u4ZoomRatio, mCamExifParam.u4ImgIndex, mCamExifParam.u4GroupId, mCamExifParam.u4FocusH, mCamExifParam.u4FocusL,
            mCamExifParam.u4RefocusPos, mCamExifParam.uJpsFileName, mCamDbgParam.u4ShotMode, mCamDbgParam.u4CamMode, mCamDbgParam.bStereoCam);
    ...
    if ( !(mpBaseExif->init(mCamExifParam.u4GpsIsOn)) )
    {
...
)
{
    ...
    exifImgInfo.bufAddr     = reinterpret_cast<MUINTPTR>(puApp1Buf);
...
#define mySwap16(x) (((x & 0xff00) >> 8) | ((x & 0x00ff) << 8))
    /*
     ...
     */
    MUINT16 length      = mySwap16((sizeof(exifAPP3Info.stereoDesc) + 2));
    ...
    //
    MUINT16 cmtSize    = mySwap16(sizeof(exifAPP3Info.cmt));
...
)
{
    ...
    MY_LOGD("APP(%d) size(0x%x) addr(%p)", u4Appn, size, puBuf);
    //
    if ( puBuf != NULL )
    {
...
{
    MY_LOGD_IF(mLogLevel >= 1, "E - this:%p", this);
    ...
    **********************************************************************************/
    ...
    if((tm = localtime(&tv.tv_sec)) != NULL)
    {
        ...
        snprintf((char *)pexifApp1Info->strSubSecTime, 3, "%02ld\n", tv.tv_usec);
        MY_LOGI("strDateTime(%s), strSubSecTime(%s)", pexifApp1Info->strDateTime, pexifApp1Info->strSubSecTime);
    }
    ...
    **********************************************************************************/
    if (mp3AEXIFInfo != NULL) {
        ...
        if(mp3AEXIFInfo->u4CapExposureTime == 0){
            ...
            pexifApp1Info->exposureTime[1] = 1;
        }
        else{
            //RAW sensor
    ...
    **********************************************************************************/
    {
        ...
        if ( ::strcmp(make, "0") != 0 ) {
            ...
            ::strncpy((char*)pexifApp1Info->strMake, (const char*)make, 31);
        }
        // [Model]
        if ( ::strcmp(model, "0") != 0 ) {
            ::memset(pexifApp1Info->strModel, 0, 32);
            ::strncpy((char*)pexifApp1Info->strModel, (const char*)model, 31);
        }
    }
    MY_LOGD_IF(mLogLevel >= 1, "- X");
...
set3AEXIFInfo(EXIF_INFO_T* p3AEXIFInfo)
{
    ...
    if ( NULL != mp3AEXIFInfo )
    {
        ...
        MY_LOGI("FNumber(%d/10) FocalLength(%d/100) SceneMode(%d) AWBMode(%d) flash(%d) AEMeterMode(%d) AEExpBias(%d) AEExpTime(%d) ISO(%d) RealISO(%d)",
            ...
            mp3AEXIFInfo->u4AEMeterMode, mp3AEXIFInfo->i4AEExpBias, mp3AEXIFInfo->u4CapExposureTime, mp3AEXIFInfo->u4AEISOSpeed, mp3AEXIFInfo->u4RealISOValue);
    }
    else
    {
        MY_LOGE(" mp3AEXIFInfo == NULL");
