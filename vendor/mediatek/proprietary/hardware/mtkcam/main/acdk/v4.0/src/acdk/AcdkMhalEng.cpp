...
MINT32 AcdkMhalEng::acdkMhalPreviewStart(MVOID *a_pBuffIn)
{
    ...
    /* PASS1 */
    m_pINormPipe = INormalPipe::createInstance(mAcdkMhalPrvParam.sensorIndex, THIS_NAME);
    ...
    INormalPipe::query(PORT_IMGO.index, ENPipeQueryCmd_X_PIX|ENPipeQueryCmd_STRIDE_BYTE,
    ...
    do {
        portInfo OutPort(
                    ...
                    MSize((mAcdkMhalPrvParam.sensorWidth*3)/4, (mAcdkMhalPrvParam.sensorHeight*3)/4), //dst size
                    MRect((mAcdkMhalPrvParam.sensorWidth*3)/4, (mAcdkMhalPrvParam.sensorHeight*3)/4), //crop
                    (mAcdkMhalPrvParam.sensorWidth*3)/4, 0, 0,
                    0, //pureraw
                    MTRUE); //packed
        vPortInfo.push_back(OutPort);
    ...
    for (i = 0; i < OVERLAY_BUFFER_CNT; ++i) {
        ...
        if (1) { //RRZO
            ...
            IImageBufferAllocator::ImgParam imgParam(mAcdkMhalPrvParam.sensorFormat, rrzoDstSize,
                                                bufStridesInBytes, bufBoundaryInBytes, 1);
            IImageBuffer*   pIBuf;
            ...
            if (i < P1_STATIC_ENQ_CNT) {
                ...
                ACDK_LOGD("p1_enq: dma:x%x V:%p P:x%x", PORT_RRZO.index, (void*)pIBuf->getBufVA(0), pIBuf->getBufPA(0));
            }
        }
    ...
    #ifdef ACDK_CAMERA_AAA
    for (i = mControls3AList.size() ; i < AAA_PROC_DEPTH; i++) {
...
MINT32 AcdkMhalEng::acdkMhalPreviewProc()
{
    ...
    MUINTPTR    dispo_virAddr = NULL, dispo_phyAddr = NULL, imgo_virA, imgo_phyA;
    ...
    while ((acdkMhalGetState() & ACDK_MHAL_PREVIEW_MASK) || (ACDK_MHAL_PRE_CAPTURE == acdkMhalGetState())) {
        ...
        mpAaa->get(dequeBufInfo.mvOut.at(0).mMetaData.mMagicNum_hal, result3A); //no need?

        if (0 == frameCount) {
        	/* 1st deque, initiates 3A */
            char *strBuf = (char*)malloc(16*mControls3AList.size());

            if (strBuf) {
                MUINT32 str_ofst = 0;
                List<MetaSet_T>::iterator itr = mControls3AList.begin();

                strBuf[0] = 0;
                for (str_ofst = 0; itr != mControls3AList.end(); itr++) {
        	    ...
        	    ACDK_LOGD("1st set 3AList(%d): %s", mControls3AList.size(), strBuf);
                free(strBuf);
            }
            else {
        	    ACDK_LOGD("1st set 3AList(%d)", mControls3AList.size());
            }
        ...
        if (!bypass_p2) {
            ...
            if (pSrc != NULL) {
                ...
                ACDK_LOGD("pass2 enq depi %p", (void*)pSrc);
            }
            #endif
			#endif
            ...
            case ACDK_CAPRAW_READY:
                {
                    ...
                    ACDK_LOGD("pass2 enque Prv P(x%x) %dx%d", dst.mBuffer->getBufPA(0),
            ...
            case ACDK_CAPRAW_START:
                {
                    ...
                    enqueParams.mvOut.push_back(capDst);

                    ACDK_LOGD("pass2 enque Qvw P(x%x) %dx%d", dst.mBuffer->getBufPA(0), crop3.mResizeDst.w, crop3.mResizeDst.h);
                    ACDK_LOGD("pass2 enque Cap P(x%x) %dx%d", capDst.mBuffer->getBufPA(0), crop2.mResizeDst.w, crop2.mResizeDst.h);
                }
                break;
            }
            ...
            if (!bypass_p2) {
                dispo_virAddr = dequeParams.mvOut[0].mBuffer->getBufVA(0);
                dispo_phyAddr = dequeParams.mvOut[0].mBuffer->getBufPA(0);
            }
        }
        else { // bypass_p2
                                ...
                                MINTPTR _ext3)
{
    ...
    ACDK_LOGD("_msgType(%d),_ext1(%ld)", _msgType, _ext1);
        ...
        case IHal3ACb::eID_NOTIFY_3APROC_FINISH:
    		 ...
    		 */
            {
                ...
                if (pMagicSet->vNumberSet.size() != 1) {
                    ACDK_LOGE("ERROR: strange request num: %d", pMagicSet->vNumberSet.size());
                 ...
                 */
                if (mControls3AList.size() < 3) {
                    ACDK_LOGE("Invalid list size : %d", mControls3AList.size());
                    break;
                }

                itr++; itr++; /* retrive list[2] */
                ...
                if (!bDummy) {
                    ...
                    if (1) { //RRZO
                        ...
                        MSize           _crop(DUMMY_RRZ_W, DUMMY_RRZ_H);
...
void AcdkMhalEng::onProcess3A()
{
    ...
    {
        Mutex::Autolock _lock(mControls3AListLock);
...

    {
        ...
        if (strBuf) {
    	    ...
    	    ACDK_LOGD("3AList(%d): %s", mControls3AList.size(), strBuf);
            free(strBuf);
        }
        else {
            ACDK_LOGD("3AList(%d)", mControls3AList.size());
        }
    }
     ...
     */
    while (mControls3AList.size() < (size_t)AAA_PROC_DEPTH) {
        addFrameMeta(requestMagicNum(), 1);
    }
...
MBOOL AcdkMhalEng::addFrameMeta(MUINT32 Magic, MBOOL bDummy)
{
    ...
    if (SENSOR_SCENARIO_ID_NORMAL_CAPTURE == mAcdkMhalPrvParam.sensorConfigParam[0].scenarioId) {
        ...
        entry4.push_back(1, Type2Type< MINT32 >()); // 1: for pure raw 0: for process raw
    ...
    mControls3AList.push_back(metaInfo);
