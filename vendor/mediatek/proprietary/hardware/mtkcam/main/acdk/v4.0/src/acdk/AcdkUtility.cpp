...
AcdkUtility *AcdkUtility::createInstance()
{
    ...
    GET_PROP("debug.acdk.util.log", "0", mDebugEnable);
                                       ...
                                       MUINT32 dstImgH)
{
    ...
    ACDK_LOGD_IF(mDebugEnable, "srcImem : size(%u),vir(0x%x),phy(0x%x),mimID(%d)",srcImem.size,srcImem.virtAddr,srcImem.phyAddr,srcImem.memID);
    ACDK_LOGD_IF(mDebugEnable, "dstImem : size(%u),vir(0x%x),phy(0x%x),mimID(%d)",dstImem.size,dstImem.virtAddr,dstImem.phyAddr,dstImem.memID);
                                       ...
                                       MUINT32 a_Stride)
{
    ...
    ACDK_LOGD_IF(mDebugEnable, "srcImem : VA(0x%x),PA(0x%x),ID(%d),SZ(%u)",srcImem.virtAddr,
                                                           srcImem.phyAddr,
                                                           srcImem.memID,
                                                           srcImem.size);

    ACDK_LOGD_IF(mDebugEnable, "dstImem : VA(0x%x),PA(0x%x),ID(%d),SZ(%u)",dstImem.virtAddr,
                                                           dstImem.phyAddr,
                                                           dstImem.memID,
                                                           dstImem.size);
...
MINT32 AcdkUtility::allocVirtBuf(AMEM_BUF_INFO* pInfo)
{
    ...
    if (!pInfo->virtAddr)
    {
        ACDK_LOGE("Cannot map ion buffer. memID(0x%x)/size(0x%x)/va(0x%x)",
            pInfo->memID,pInfo->size,pInfo->virtAddr);
        return -1;
    }

	ACDK_LOGD("alloc ion flag(%d) memid(%d) va(%p) pa(%p) size(%d)",
        ion_prot_flags, pInfo->memID, pInfo->virtAddr, pInfo->phyAddr, pInfo->size);
...
MINT32 AcdkUtility::mapPhyAddr(AMEM_BUF_INFO* pInfo)
{
    ...
    mm_data.config_buffer_param.eModuleID = M4U_PORT_CAM_IMGO;
    ...
    ACDK_LOGD("mID(0x%x),size(0x%x),VA(x%x),PA(0x%x),S/C(%d/%d)",
...
MINT32  AcdkUtility::cacheSyncbyRange(AMEM_CACHECTRL_ENUM ctrl, AMEM_BUF_INFO* pInfo)
{
    ...
    ACDK_LOGD("+, c/m/va/sz/pa(%d/0x%x/0x%x/0x%x/0x%x)",
    ...
    switch(ctrl) {
        ...
        case AMEM_CACHECTRL_ENUM_INVALID:
            ...
            break;
        case AMEM_CACHECTRL_ENUM_INVALID_ALL:
