...
//#include <drv / src/ isp/ mt6757/ iopipe/ CamIO/ FakeSensor.h>
#endif
...
MINT32 AcdkMain::startPreview(Func_CB prvCb)
{
    ...
    #if 1
    RRZOWidth = (DUMMY_RRZ_W>>6)<<6;
    RRZOStride[0] = RRZOWidth;
    RRZOImgSize = (RRZOWidth * DUMMY_RRZ_H*15)>>3;
    ...
    for(MINT32 i = 0; i < SURFACE_NUM; ++i)
    {
        ACDK_LOGD("surfaceVA[%d] = 0x%lx, surfacePA[%d] = %p",i,(MUINT8 *)mSurfaceIMemInfo[i].virtAddr,i,(MUINT8 *)mSurfaceIMemInfo[i].phyAddr);
    }
    ...
    #ifdef MTK_SUB2_IMGSENSOR
    else if(mSensorDev == SENSOR_DEV_SUB || mSensorDev == SENSOR_DEV_MAIN_2 || mSensorDev == SENSOR_DEV_SUB_2)// main2 & sub & sub2 sensor
    #else
    ...
    if (1) {
        ...
        if (mCapType & 0x3C) {
            if (mSensorType == SENSOR_TYPE_YUV) {
                ...
                acdkCapInfo.RAWImgBufInfo.imgSize   = acdkCapInfo.RAWImgBufInfo.imgWidth*acdkCapInfo.RAWImgBufInfo.imgHeight*10/8; //FIXME !!!for raw8 and others
                ...
                MINT32  const isSaveImg)
{
...

    {
        ...
        property_get("camera.acdkdump.raw", rawVal, "0");
...
)
{
    ...
    else if (a_u4Ioctl == ACDK_CMD_SET_SRC_DEV)
    {
        ...
        if(*pSrcDev == 0x08){ // 1: main, 2: sub, 4: main2, 8: sub2
            *pSrcDev = 4;
        }
        else if(*pSrcDev == 0x10) {
            *pSrcDev = 8;
        }
