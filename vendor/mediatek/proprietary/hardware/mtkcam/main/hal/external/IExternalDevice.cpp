...
public:
    ...
    status_t  openDevice ( int yuvFromat, int jpegFormat )
    {
        ALOGI("openDevice");
        android::Parcel data, reply;
        data.writeInterfaceToken(IExternalDevice::getInterfaceDescriptor());
        // arg0
        data.writeInt32(yuvFromat);
        // arg1
        data.writeInt32(jpegFormat);
        //
    ...
    uint32_t code, const android::Parcel& data, android::Parcel* reply, uint32_t flags)
{
    switch(code) {
        ...
        case OPEN_DEVICE: {
            ...
            CHECK_INTERFACE(IExternalDevice, data, reply);
            // arg0
            int yuvFromat = data.readInt32();
            // arg1
            int jpegFromat = data.readInt32();
            reply->writeNoException();
            reply->writeInt32(openDevice(yuvFromat, jpegFromat));
            return NO_ERROR;
        } break;
        case SET_CALLBACK: {
            ALOGI("SET_CALLBACK");
