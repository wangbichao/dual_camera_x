...
onInit()
{
    ...
    mpParamsMgrV3 = IParamsManagerV3::createInstance(mDevName, mi4OpenId, mpParamsMgr);
...
startRecording()
{
    ...
    if ( !strcmp(mpCamAdapter->getName(), MtkCameraParameters::APP_MODE_NAME_MTK_ZSD) )
    {
        ...
        if( OK != status )
        {
            MY_LOGE("startPreview Fail");
            goto lbExit;
        }
        else if( mpCamAdapter == NULL )
        {
            MY_LOGE("NULL Camera Adapter after startPreview");
            goto lbExit;
        }
    }
...
initCameraAdapter()
{
    ...
    mpCamAdapter = ICamAdapter::createInstance(mDevName, mi4OpenId, mpParamsMgrV3);
...
//          then this method may be called once with a non-null parameter to set the preview surface.
    ...
    mpDisplayClient = IDisplayClient::createInstance(getOpenId());
    if  ( mpDisplayClient == 0 )
    {
...
addCamClient(sp<ICamClient> const& client, const char* name)
{
    ...
    for (it = vmpCamClient.begin(); it != vmpCamClient.end(); ++it) {
        if ((*it) == pClient) {
            MY_LOGE("client(%p) was already subscribed", pClient.get());
