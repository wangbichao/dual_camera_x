...
~Cam3DeviceImp()
{
    CamManagerV3::getInstance()->decDevice(mi4OpenId, mDevName.string());
}
...
Cam3DeviceImp(
    ...
    //
{
    CamManagerV3::getInstance()->incDevice(i4OpenId, rDevName.string());
}
 ...
 */
    ...
    //
    {
        ...
        //
        if  ( mAppContext.pPipelineMgr != 0 ) {
            mAppContext.pPipelineMgr = NULL;
        }
        //
...
onFlushLocked()
{
    ...
    disableRequesting();
    //
    err = flushAndWait(getSafeAppContext());
    //
    enableRequesting();
    //
    return err;
...
flushAndWait(AppContext const& appContext)
{
    ...
    MERROR err = OK;
    //
    if  ( appContext.pPipelineMgr != 0 ) {
        err = appContext.pPipelineMgr->beginFlush();
    ...
    //
    if  ( appContext.pPipelineMgr != 0 ) {
        appContext.pPipelineMgr->endFlush();
    }
    //
    MY_LOGD("-");
    return OK;
...
)
{
    ...
    //
    if  ( mAppContext.pPipelineMgr == 0 ) {
        mAppContext.pPipelineMgr = IPipelineModelMgr::create(this);
        if ( mAppContext.pPipelineMgr == 0 ) {
    ...
    //
    {
        IPipelineModelMgr::ConfigureParam params;
        ...
        params.operation_mode       = stream_list->operation_mode;
        //
        err = mAppContext.pPipelineMgr->configurePipeline(params);
        if  ( OK != err ) {
...
)
{
    ...
    IPipelineModelMgr* pPipeline         = appContext.pPipelineMgr.get();
