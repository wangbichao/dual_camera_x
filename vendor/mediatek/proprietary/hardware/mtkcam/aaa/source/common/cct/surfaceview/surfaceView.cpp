    ...
    ,mFBBufferIndex(0)
{
    ...
    memset (&mFinfo, 0, sizeof(fb_fix_screeninfo));
    memset (&mLayerInfo, 0, sizeof(fb_overlay_layer)*VIDOE_LAYER_NO);
    m_pFrameBuf = NULL;
    m_pM4UDrv   = NULL;
...
MINT32 surfaceView::getBootMode(MUINT32 &mode)
{
    ...
    char bootMode[4];
    ssize_t _readsz = ::read(fd, (MVOID*)bootMode, 4);
    if(_readsz == 0)
    {
        ...
        MUINT32 const vFlip)
{
    ...
    {
        g_fp = fopen ("//sdard//prv.raw", "wb");
        if (NULL == g_fp)
        {
            ACDK_LOGE("[setOverlayBuf] Can not open file /sdard/prv.raw");
        }
    ...
    else
    {
        ...
        for(MUINT32 i = 0; i < mLayerInfo[0].src_height; ++i)
        {
            memcpy((m_pFrameBuf + (mFBBufferIndex * mFBBufferSize))+ mLayerInfo[0].tgt_offset_x + (mVinfo.xres_virtual * mLayerInfo[0].tgt_offset_y * bytePrePixel) + (i * mVinfo.xres_virtual * bytePrePixel),
                    (void*)((long)mLayerInfo[0].src_base_addr + (i * mLayerInfo[0].src_width * 2)),
                    mLayerInfo[0].src_width * 2);
        }
