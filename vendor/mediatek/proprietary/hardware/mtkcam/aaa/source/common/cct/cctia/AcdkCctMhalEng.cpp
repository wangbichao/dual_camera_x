...
status_t AcdkCctMhalOpenDevice(camera_module_t *module, const char *name)
{
    String8 const mName(name);
    ACDK_LOGD("Opening camera %s + \n", mName.string());
    // module->methods->openLegacy <-- open cam1 device
    // module->methods->open <-- open cam3 device
...
AcdkCctMhalEng::AcdkCctMhalEng()
{
    ...
    GET_PROP("debug.acdkcct.eng.log", "0", mDebugEnable);
...
MINT32 AcdkCctMhalEng::acdkCctMhalPreviewStart(MVOID *a_pBuffIn)
{
    ...
    if(mAcdkCctMhalPrvParam.sensorIndex == 0)
        AcdkCctMhalOpenDevice(mModule, "0");
    else if(mAcdkCctMhalPrvParam.sensorIndex == 1)
        AcdkCctMhalOpenDevice(mModule, "1");
    else
        AcdkCctMhalOpenDevice(mModule, "2");
