...
MVOID VHdrDrvImp::ConfigRmg_ivHDR(const MUINT32 &aLeFirst)
{
    ...
    MAKE_IspMgrIf().configRMG_RMG2(tuningSensorDevType, rmgPara);
...
MVOID VHdrDrvImp::EnableRmg(const MBOOL &aEn)
{
    ...
    NS3Av3::IspMgrIf& rIspMgrIf = MAKE_IspMgrIf();
    rIspMgrIf.setRMGEnable(tuningSensorDevType, aEn);

    // enable debug log, we don't want to add API, so we add here.
    if(aEn){
        rIspMgrIf.setRMGDebug(tuningSensorDevType, g_debugDump);
    }else{
        rIspMgrIf.setRMGDebug(tuningSensorDevType , 0);
    }
}
...
MVOID VHdrDrvImp::EnableRmm(const MBOOL &aEn)
{
    ...
    NS3Av3::IspMgrIf& rIspMgrIf = MAKE_IspMgrIf();
    rIspMgrIf.setRMMEnable(tuningSensorDevType, aEn);

    // enable debug log, we don't want to add API, so we add here.
    if(aEn){
        rIspMgrIf.setRMMDebug(tuningSensorDevType, g_debugDump);
    }else{
        rIspMgrIf.setRMMDebug(tuningSensorDevType , 0);
    }
}
