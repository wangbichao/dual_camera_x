                    ...
                    MVOID* pParamsMgr)
{
    ...
    IHal3A* p3AHal = MAKE_Hal3A(mpParamsMgr->getOpenId(),"UtilityHal");
    if (!p3AHal)
    {
    ...
    CAM_LOGD("- (app1Size, appnSize, exifSize) = (%d, %d, %d)",
        (int)u4App1HeaderSize, u4AppnHeaderSize, u4FinalExifSize);
    return ret;
}
      ...
      , MUINT32 & u4EncSize)
{
    MBOOL ret = MTRUE;
    // (0). debug
    CAM_LOGD("[createJpegImg] - rSrcImgBufInfo.u4BufVA=0x%p", (void*)rSrcBufInfo.u4BufVA);
    ...
    {
        CAM_LOGD("ION mode srcFD(%x) dstFD(%x)", rSrcBufInfo.i4MemID,rDstBufInfo.i4MemID);
                  ...
                  , MUINT32     quality)
{
    ...
    *pImageInfo = srcBuff.virtAddr;
    CAM_LOGD("[createPanoJpegImg] out addr 0x%p", (void*)srcBuff.virtAddr);
    return ret;
}
