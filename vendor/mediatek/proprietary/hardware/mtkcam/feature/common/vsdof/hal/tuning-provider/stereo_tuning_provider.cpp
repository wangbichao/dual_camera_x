...
#define PROPERTY_GF_TUNING      PROPERTY_TUNING_PREFIX"gf"      //debug.STEREO.tuning.gf, 1 to read/write tuning file
...
#define DPE_TUNING_FILE_PATH    TUNING_FILE_PATH"/dpe_params"    //0: Not to read tuning file
#define WMF_TUNING_FILE_PATH    TUNING_FILE_PATH"/wmf_params"
#define GF_TUNING_FILE_PATH     TUNING_FILE_PATH"/gf_params"
#define BOKEH_TUNING_FILE_PATH  TUNING_FILE_PATH"/bokeh_params"
...
inline void mkdir()
{
    ...
    if(stat(TUNING_FILE_PATH, &st) == -1) {
        mkdir(TUNING_FILE_PATH, 0755);
    }
}
...
StereoTuningProvider::initDPETuningInfo(DVEConfig *tuningBuffer)
{
    ...
    StereoArea area = StereoSizeProvider::getInstance()->getBufferSize(E_MV_Y, eSTEREO_SCENARIO_CAPTURE);
    area *= 8;
...
StereoTuningProvider::getWMFTuningInfo(WMFEFILTERSIZE &size, void *tbliBuffer)
{
    ...
    if(LOG_ENABLED) {
        ...
        for(int i = 0; i < TBLI_SIZE; i++) {
            if((i+1) % 8 == 0) {
                MY_LOGD("%4d, %4d, %4d, %4d, %4d, %4d, %4d, %4d,",
...
bool
StereoTuningProvider::getGFTuningInfo(std::vector<MINT32> &tuningTable)
{
//    const MINT32 DEFAULT_TABLE[] = { 119, 102,  86,  69,  53,  36,   20,    3,
//                                     -14, -30, -47, -63, -80, -96, -113, -129 };
    const MINT32 DEFAULT_TABLE[] = {  124, 113, 102, 91, 80, 69, 58, 47,
                                      36, 25, 14, 3, -8, -19, -30, -41, -52 };

    static bool useDefault = true;
    #define USE_DEFAULT tuningTable.insert(tuningTable.begin(), std::begin(DEFAULT_TABLE), std::end(DEFAULT_TABLE))
    ...
    if(propVal != 1) {
        ...
        struct stat st;
        FILE *fp = NULL;
        if(0 == stat(GF_TUNING_FILE_PATH, &st)) {
            useDefault = false;
            tuningTable.clear();

            //Tuning file exist, read from file
            MINT32 val;
            fp = fopen(GF_TUNING_FILE_PATH, "r");
            if(fp) {
                while(fscanf(fp, "%d ", &val) != EOF) {
                    tuningTable.push_back(val);
                }
                fclose(fp);
            }
        } else {
            mkdir();
            //Tuning file not exist, write to file
            if(useDefault) {
                USE_DEFAULT;
            }

            fp = fopen(GF_TUNING_FILE_PATH, "w+");
            if(fp) {
                for(std::vector<MINT32>::iterator it = tuningTable.begin() ; it != tuningTable.end(); ++it) {
                    fprintf(fp, "%d ", *it);
                }
                fflush(fp);
                fclose(fp);
    ...
    if(StereoSettingProvider::isLogEnabled(PERPERTY_BOKEH_NODE_LOG))
    {
        MY_LOGD("===== GF Clear Range Table =====");
        for(MINT32 i = 0; i < tuningTable.size(); i++) {
            MY_LOGD("%d", tuningTable[i]);
        }
    }
...
bool
StereoTuningProvider::getFETuningInfo(dip_x_reg_t *isp_reg, const int BLOCK_SIZE)
{
    if(NULL == isp_reg) {
        return false;
    }
