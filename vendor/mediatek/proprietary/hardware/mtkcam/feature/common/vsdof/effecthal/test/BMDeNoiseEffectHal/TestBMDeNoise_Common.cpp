...
#define LOG_TAG "TestBMDeNoise_Common"
...
ImgParam getImgParam_BMDENOISE_final_result()
{
    MSize MY_S_SIZE(3072, 1728);
    int MY_S_STRIDES[3] = {MY_S_SIZE.w, MY_S_SIZE.w/2 , MY_S_SIZE.w/2};
    return ImgParam(eImgFmt_YV12, MY_S_SIZE, MY_S_STRIDES, FULL_RAW_BOUNDARY, 3);
}
...
MVOID ut_finishCB(bool status)
{
    if(status)
    {
        ...
        WaitingListener::CB(NULL, String8(BMDENOISE_COMPLETE_KEY), mEffReq);
    }
}
