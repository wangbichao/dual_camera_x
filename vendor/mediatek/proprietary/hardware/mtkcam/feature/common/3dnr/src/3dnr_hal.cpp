...
Hal3dnr::init(MINT32 force3DNR)
{
    ...
    memset(mpNr3dParam, 0, sizeof(mpNr3dParam));
    ...
    mLogLevel = ::property_get_int32("camera.3dnr.log.level", 0);
...
Hal3dnr::setGMV(MUINT32 frameNo, MINT32 gmvX, MINT32 gmvY, MINT32 cmvX_Int, MINT32 cmvY_Int)
{
    ...
    if (mForce3DNR)
    {
        MINT32 value = ::property_get_int32("camera.3dnr.forcegmv.enable", 0);
        if (value)
        {
            i4TempNmvXFromQueue = ::property_get_int32("camera.3dnr.forcegmv.x", 0);

            i4TempNmvYFromQueue = ::property_get_int32("camera.3dnr.forcegmv.y", 0);
...
//    ::property_get_int32("camera.3dnr.usecmv.enable", 1);
...
//    if (aInputValue[0] == '1')   // For EIS 1.2 (use CMV). gmv_crp (t) = gmv(t) - ( cmv(t) - cmv(t-1) )
    ...
    MY_LOGD_IF(mLogLevel >= 0, "STEP 4: gmv cal,ST=%d, gmv(x,y)=(%5d,%5d),CmvX/Y(%5d,%5d),NmvX/Y(%5d,%5d), (cmv diff %5d,%5d), frame:%d",
...
Hal3dnr::setVipiParams(MBOOL isVIPIIn, MUINT32 vipiW, MUINT32 vipiH, MINT imgFormat, size_t stride)
{
    ...
    if (isVIPIIn)
    {
        ...
        MY_LOGD_IF(mLogLevel >= 2, "vipi offset=%d,(xy=%d,%d), stride=%d, u4PixelToBytes=%d",mpNr3dParam->vipi_offst, u4VipiOffset_X, u4VipiOffset_Y, stride, u4PixelToBytes);
        ...
        MINT32 value = ::property_get_int32("camera.3dnr.forceskip", 0);
        if (value)
        {
