...
SDKcreateJpegImg(IImageBuffer* JPGSrcImgBuffer, IImageBuffer* JPGDstImgBuffer, ExifParams* exifPara)
{
    ...
    MUINT32 inFmt = (MUINT32)JPGSrcImgBuffer->getImgFormat();
    if(bufDump != 0)
    {
        char filename[256];
        sprintf(filename, "/sdcard/JpgEffecthalSrc_%dx%d_%d.yuv", JPGSrcImgBuffer->getImgSize().w, JPGSrcImgBuffer->getImgSize().h, inFmt);
        JPGSrcImgBuffer->saveToFile(filename);
        CAM_LOGD("Save src : %s", filename);

    }
    my_encode_params params;
    CAM_LOGD("[%s] picture size w = %d h = %d, inFmt=%d", __FUNCTION__ ,JPGSrcImgBuffer->getImgSize().w, JPGSrcImgBuffer->getImgSize().h, inFmt);
    ...
    IImageBuffer* mpPostviewImgBuf = BufAllocUtil::getInstance().allocMem(LOG_TAG, inFmt, 160, 128);

    ImgProcess(JPGSrcImgBuffer, JPGSrcImgBuffer->getImgSize().w, JPGSrcImgBuffer->getImgSize().h, (EImageFormat)inFmt, mpPostviewImgBuf, mpPostviewImgBuf->getImgSize().w, mpPostviewImgBuf->getImgSize().h, (EImageFormat)inFmt);
...
#ifdef Debug_Mode
    JPGSrcImgBuffer->saveToFile("/sdcard/MainSrc.yuv");
    ...
    mpPostviewImgBuf->saveToFile("/sdcard/ThumbResultSrc.yuv");
    ALOGD("[SDKcreateFBJpegImg] Thumbnail Src: w = %d h = %d BitstreamSize = %d",  mpPostviewImgBuf->getImgSize().w, mpPostviewImgBuf->getImgSize().h,thumbBuf->getBitstreamSize());
    ...
    CAM_LOGD("[%s] create exif - : exifSize = %zu thumbnailMaxSize = %zu",__FUNCTION__,exifSize,thumbnailMaxSize);
    ...
    CAM_LOGD("[%s] integrate JPG +, needOutputSize(%d), dstImgbufSize(%d)",__FUNCTION__, outputsize, JPGDstImgBuffer->getImgSize().w);
    if(outputsize > JPGDstImgBuffer->getImgSize().w)
    {
        ALOGE("[%s] Destination Buffer size is too small! needOut,dst=(%d,%d) \n",__FUNCTION__, outputsize, JPGDstImgBuffer->getImgSize().w);
    ...
    CAM_LOGD("[%s] integrate JPG - : jpegsize_final = %zu %d",__FUNCTION__,jpegsize_final,JPGDstImgBuffer->getImgSize().w);
