...
#define MAKE_EISHAL_OBJ(id) \
    ...
    template <> MUINT64 EisHalObj<id>::gLastGyroTimestamp = 0;
...
#define EISHAL_WaitGyroQueue(id, last, target) \
    ...
    {\
        ...
        if( last.ts <= target)\
        {\
            EIS_LOG("video (%llu) > gyro.ts (%llu) => wait",target,last.ts);\
            EISHAL_WaitGyroCond_Wait(id);\
        }else\
        {\
            if (UNLIKELY(mDebugDump >= 1))\
            {\
                EIS_LOG("video (%llu) < gyro.ts (%llu) => go GIS",target, last.ts);\
            }\
        }\
    }else\
    {\
