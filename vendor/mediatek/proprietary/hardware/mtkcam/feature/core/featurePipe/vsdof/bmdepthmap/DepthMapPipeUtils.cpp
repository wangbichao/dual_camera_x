...
MVOID
TemplateStatistics::debug()
{
    MY_LOGD("=============mvIn section============== size=%d", mFrameIDToLocMap_mvIn.size());
    for(ssize_t index=0;index<mFrameIDToLocMap_mvIn.size();++index)
    {
        MUINT32 frameID = mFrameIDToLocMap_mvIn.keyAt(index);
        TemplateLocStat stat = mFrameIDToLocMap_mvIn.valueAt(index);
        MY_LOGD("=============mvIn section============== FrameID=%d", frameID);
        for(ssize_t index2=0;index2<stat.mKeyToLocMap.size();++index2)
        {
            MUINT32 portID = stat.mKeyToLocMap.keyAt(index2);
            MUINT32 pos = stat.mKeyToLocMap.valueAt(index2);
            MY_LOGD("====== portID=%d  pos=%d ", portID, pos);
        }
    }

    MY_LOGD("=============mvOut section============== size=%d", mFrameIDToLocMap_mvOut.size());
    for(ssize_t index=0;index<mFrameIDToLocMap_mvOut.size();++index)
    {
        MUINT32 frameID = mFrameIDToLocMap_mvOut.keyAt(index);
        TemplateLocStat stat = mFrameIDToLocMap_mvOut.valueAt(index);
        MY_LOGD("=============mvOut section============== FrameID=%d", frameID);
        for(ssize_t index2=0;index2<stat.mKeyToLocMap.size();++index2)
        {
            MUINT32 portID = stat.mKeyToLocMap.keyAt(index2);
            MUINT32 pos = stat.mKeyToLocMap.valueAt(index2);
    ...
    MY_LOGD("=============mvCrop section============== size=%d", mFrameIDToLocMap_mvCrop.size());
    for(ssize_t index=0;index<mFrameIDToLocMap_mvCrop.size();++index)
    {
        MUINT32 frameID = mFrameIDToLocMap_mvCrop.keyAt(index);
        TemplateLocStat stat = mFrameIDToLocMap_mvCrop.valueAt(index);
        MY_LOGD("=============mvCrop section============== FrameID=%d", frameID);
        for(ssize_t index2=0;index2<stat.mKeyToLocMap.size();++index2)
        {
            MUINT32 portID = stat.mKeyToLocMap.keyAt(index2);
            MUINT32 pos = stat.mKeyToLocMap.valueAt(index2);
...
QParamTemplateGenerator::
QParamTemplateGenerator(MUINT32 frameID, MUINT32 iSensorIdx, ENormalStreamTag streamTag, TemplateStatistics* pLearningStat)
...
MBOOL
QParamTemplateGenerator::
generate(QParamTemplate& rQParamTmpl)
{
    rQParamTmpl.mTemplate.mvStreamTag.push_back(mStreamTag);
    rQParamTmpl.mTemplate.mvSensorIdx.push_back(miSensorIdx);
    rQParamTmpl.mTemplate.mvIn.appendVector(mvIn);
    rQParamTmpl.mTemplate.mvOut.appendVector(mvOut);
    rQParamTmpl.mTemplate.mvCropRsInfo.appendVector(mvCropRsInfo);
    rQParamTmpl.mTemplate.mvModuleData.appendVector(mvModuleData);


    if(checkValid())
    {
        rQParamTmpl.mStats = mStats;
        return MTRUE;
    }
    else
        return MFALSE;
