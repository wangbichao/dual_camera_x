...
sp<GraphicBuffer>* IIBuffer::getGraphicBufferAddr()
{
  TRACE_FUNC_ENTER();
  sp<GraphicBuffer> *addr = NULL;
  android::sp<IImageBuffer> buffer;
  buffer = this->getImageBuffer();
  if( buffer != NULL && buffer->getImageBufferHeap() )
  {
    addr = (sp<GraphicBuffer>*)buffer->getImageBufferHeap()->getGraphicBuffer();
  }
  TRACE_FUNC_EXIT();
  return addr;
}
...
sp<IImageBuffer> IIBuffer_IImageBuffer::operator->() const
{
  return mBuffer;
}
