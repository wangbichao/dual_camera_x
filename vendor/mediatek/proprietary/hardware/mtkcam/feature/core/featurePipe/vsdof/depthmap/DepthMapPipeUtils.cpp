...
QParamTemplateGenerator::generate(QParams& rOutputParams, TemplateStatistics& rStats)
{
    ...
    rOutputParams.mvSensorIdx.push_back(miSensorIdx);
...
IMetadata* getMetadataFromFrameInfoPtr(sp<EffectFrameInfo> pFrameInfo)
{
    ...
    result = reinterpret_cast<IMetadata*>(effParam->getPtr(VSDOF_PARAMS_METADATA_KEY));
    return result;
}
...
DepthMapBufferID mapQueuedBID(DepthMapBufferID bid)
{
    switch(bid)
    {
        ...
        case BID_META_IN_HAL:
            return BID_META_IN_HAL_QUEUED;
        ...
        default:
            MY_LOGE("Cannot find its queued BID, bid=%d", bid);
            break;
    }
    return BID_DEPTHMAPPIPE_INVALID;
}
//
MBOOL isQueueDepthFlow(DepthNodeOpState eState, DepthFlowType eFlowType)
{
    if(eFlowType == eDEPTH_FLOW_TYPE_QUEUED_DEPTH && eState == eSTATE_NORMAL)
        return MTRUE;
    else
        return MFALSE;
}
