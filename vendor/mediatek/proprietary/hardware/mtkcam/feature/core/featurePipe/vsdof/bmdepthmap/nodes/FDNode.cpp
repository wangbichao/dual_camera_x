...
: DepthMapPipeNode(name, nodeID, pFlowOpt, setting)
{
    ...
    mFD_IMG_SIZE = pFlowOpt->getFDBufferSize();
}
...
onThreadLoop()
{
    ...
    // FD input src buffer
    IImageBuffer* pFDBuf;
    bRet &= pBufferHandler->getEnqueBuffer(getNodeId(), BID_P2AFM_OUT_FDIMG, pFDBuf);
    ...
    {
        VSDOF_PRFLOG("halFDDo Done, reqId=%d", pRequest->getRequestNo());
    }
    ...
    VSDOF_PRFLOG("FD threapLoop end, reqId=%d", pRequest->getRequestNo());
