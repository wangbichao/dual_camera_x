...
MVOID prepareFDInputImgInfoMap(sp<EffectRequest> pEffectReq, ImgInfoMapPtr& pToFDImgInfoFromP2A)
{
    ...
    pToFDImgInfoFromP2A->addImageBuffer(BID_P2AFM_OUT_FDIMG, smImgBuf_FD);
    ...
    sp<EffectFrameInfo> frameInfo = new EffectFrameInfo(pEffectReq->getRequestNo(), BID_FD_OUT_EXTRADATA);
    frameInfo->setFrameBuffer(buf);
    MY_LOGD("ExtraData imageBuffer=%x", buf);
    pEffectReq->vOutputFrameInfo.add(BID_FD_OUT_EXTRADATA, frameInfo);

    MY_LOGD("BID_FD_OUT_EXTRADATA===>%d", pEffectReq->vOutputFrameInfo.indexOfKey(BID_FD_OUT_EXTRADATA));
    MY_LOGD("-");
}
