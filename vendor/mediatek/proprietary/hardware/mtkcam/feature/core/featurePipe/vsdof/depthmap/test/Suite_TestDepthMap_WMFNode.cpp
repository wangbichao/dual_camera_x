...
init(const MSize& szDMHSize)
{
    ...
    MSize P2AFM_FE1CO_INPUT_SIZE = pass2SizeInfo.areaIMG2O;
...
MVOID prepareWMFInputImgInfoMap_Cap(sp<EffectRequest> pEffReq, ImgInfoMapPtr& pToWMFImgInfoFromOCC, ImgInfoMapPtr& pToWMFImgInfoFromP2A)
{
    ...
    const char * sMYSLFilename = "/sdcard/vsdof/WMF/BID_P2AFM_FE1CO_INPUT_480x272.yuv";

    SmartImageBuffer smImgBuf_MY_SL = OCCBufPoolSet.mMYSLImgBufPool->request();
    LOAD_SMART_IMAGE_BUFFER(smImgBuf_MY_SL, "MYSLBufPool", sMYSLFilename, ImageBufferPool::USAGE_HW);
    // because the BID_P2AFM_FE1C_INPUT is also the BID_TO_GF_MY_SL
    pToWMFImgInfoFromP2A->addImageBuffer(BID_P2AFM_FE1C_INPUT, smImgBuf_MY_SL);
    ...
    pEffReq->vOutputFrameInfo.add(BID_OCC_OUT_MY_S, pFrameInfo);
    ...
    pToWMFImgInfoFromOCC->addImageBuffer(BID_OCC_OUT_DMH, smImgBuf_DMH);

    // prepare output frame: DMW
    pFrameInfo = new EffectFrameInfo(pEffReq->getRequestNo(), BID_WMF_OUT_DMW);
    ...
    pEffReq->vOutputFrameInfo.add(BID_WMF_OUT_DMW, pFrameInfo);


    // prepare output frame: DepthMap
    pFrameInfo = new EffectFrameInfo(pEffReq->getRequestNo(), BID_GF_OUT_DEPTHMAP);
    IImageBufferAllocator::ImgParam imgParam_DepthMap = getImgParam_DepthMap();
    buf = createEmptyImageBuffer(imgParam_DepthMap, "DEPTHMAP", ImageBufferPool::USAGE_HW);
    pFrameInfo->setFrameBuffer(buf);
    pEffReq->vOutputFrameInfo.add(BID_GF_OUT_DEPTHMAP, pFrameInfo);
...
MVOID prepareWMFInputImgInfoMap_Rec(sp<EffectRequest> pEffReq, ImgInfoMapPtr& pToWMFImgInfoFromOCC)
{
    ...
    #endif
    // inset MY_S into EffectRequest's OutputFrameInfo
    sp<EffectFrameInfo> pFrameInfo = new EffectFrameInfo(pEffReq->getRequestNo(), BID_OCC_OUT_MY_S);
    SmartImageBuffer smImgBuf_MY_S = OCCBufPoolSet.mMYSImgBufPool->request();
    LOAD_SMART_IMAGE_BUFFER(smImgBuf_MY_S, "MYSBufPool", sMYSFilename, ImageBufferPool::USAGE_HW);
    pFrameInfo->setFrameBuffer(smImgBuf_MY_S->mImageBuffer);
    pEffReq->vOutputFrameInfo.add(BID_OCC_OUT_MY_S, pFrameInfo);
...
MVOID setupDumpProperties(WMFNode *pWMFNode)
{
    ...
    snprintf(prop_name, 256, "%s.%s", name, DepthMapDataHandler::ID2Name(WMF_TO_GF_DMW_MY_S));
    property_set(prop_name, "1");
    snprintf(prop_name, 256, "%s.%s", name, DepthMapDataHandler::ID2Name(GF_OUT_DEPTHMAP));
    property_set(prop_name, "1");
}
...
MVOID preview()
{
    ...
    pWMFNode->connectData(WMF_TO_GF_DMW_MY_S, WMF_TO_GF_DMW_MY_S, pCallbackUTNode);
    pWMFNode->connectData(GF_OUT_DEPTHMAP, GF_OUT_DEPTHMAP, pCallbackUTNode);
...
//MVOID capture()
{
    ...
    vector<DepthMapDataID> vWaitDataID;
    vWaitDataID.push_back(WMF_TO_GF_DMW_MY_S);
    vWaitDataID.push_back(GF_OUT_DEPTHMAP);
    pCallbackUTNode->setWaitingDataIDAndReqID(vWaitDataID, vReqIDVec);
