...
MBOOL GpuWarp_v1::onConfig(MUINT32 feature, const GB_PTR_ARRAY &in, const GB_PTR_ARRAY &out, const MSize &maxImage, const MSize &maxWarp, RegisterMode registerMode)
{
    TRACE_FUNC_ENTER();
    MBOOL ret = MFALSE;
    ...
    {
        ret = initWarp(GB_PTR_ARRAY(), GB_PTR_ARRAY(), maxImage);
    }
    else
    {
...
MBOOL GpuWarp_v1::onRegisterGB(GB_PTR gb, GBDir dir)
{
    TRACE_FUNC_ENTER();
    MBOOL ret = MFALSE;
    struct WarpImageExtInfo warpInfo;
    ...
    if( mRegisterMode == REGISTER_MODE_RUNTIME && gb )
    {
        ret = MTRUE;
        configBasicWarpInfo(&warpInfo, &gpuTuning, mFeature, mMaxWarp);
        ...
        if( dir == GB_DIR_IN )
        {
            warpInfo.SrcGraphicBuffer = (void*)&gb;
            warpInfo.DstGraphicBuffer = NULL;
            warpInfo.InitBufferType = 0;
        }
        else
        {
            warpInfo.SrcGraphicBuffer = NULL;
            warpInfo.DstGraphicBuffer = (void*)&gb;
            warpInfo.InitBufferType = 1;
        }
        TRACE_GPU_START();
        mpGpuWarp->WarpFeatureCtrl(WARP_FEATURE_INIT_EGLIMAGE, &warpInfo, NULL);
        TRACE_GPU_STOP("init egl image");
    }
    TRACE_FUNC_EXIT();
    return ret;
}

MUINT32 GpuWarp_v1::getNumRegisterSlot() const
{
    return NUM_REGISTER_SLOT;
}
...
MBOOL GpuWarp_v1::initWarp(const GB_PTR_ARRAY &inputBuffers, const GB_PTR_ARRAY &outputBuffers, const MSize &maxImage)
{
    ...
    MUINT32 inSize, outSize;

    inSize = std::min((MINT32)inputBuffers.size(), NUM_REGISTER_SLOT);
    outSize = std::min((MINT32)outputBuffers.size(), NUM_REGISTER_SLOT);

    inputArray = new sp<GraphicBuffer>* [inSize];
    outputArray = new sp<GraphicBuffer>* [outSize];

    if( prepareBufferArray(inputArray, outputArray, inputBuffers, outputBuffers, inSize, outSize) )
    {
        ...
        warpInfo.InputGBNum = inSize;
        warpInfo.OutputGBNum = outSize;
...
MBOOL GpuWarp_v1::prepareBufferArray(GB_PTR *inputArray, GB_PTR *outputArray, const GB_PTR_ARRAY &inputBuffers, const GB_PTR_ARRAY &outputBuffers, MUINT32 inSize, MUINT32 outSize)
{
    TRACE_FUNC_ENTER();
    MBOOL ret = MTRUE;
    MUINT32 i;
...
MVOID GpuWarp_v1::configBasicWarpInfo(struct WarpImageExtInfo *warpInfo, GpuTuning *gpuTuning, MUINT32 feature, const MSize &maxWarp)
{
    ...
    warpInfo->OutImgFmt = SUPPORT_GPU_YV12 ? WARP_IMAGE_YV12 : WARP_IMAGE_RGBA8888;
    ...
    warpInfo->DisableEGLImageInit = (mRegisterMode == REGISTER_MODE_RUNTIME) ? 1 : 0;
