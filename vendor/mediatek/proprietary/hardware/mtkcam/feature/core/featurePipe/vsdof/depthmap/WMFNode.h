...
private:
    ...
    public:
        ...
        MBOOL init(DepthFlowType flowType);
        MBOOL uninit();
    public:
        android::sp<ImageBufferPool> mpTbliImgBufPool;
        android::sp<ImageBufferPool> mpDMWImgBufPool;
    };

private:
    MVOID cleanUp();
    /**
     * @brief Prepare the WMF enque config
     * @param [in] pImgBuf_SrcImg source image
     * @param [in] pImgBuf_SrcDepth source depth
     * @param [out] rWMFConfig WMF enque config
     * @param [out] pEnqueBufPool enque buffer pool instance
     */
