...
MBOOL GpuWarpBase::config(const GB_PTR_ARRAY &in, const GB_PTR_ARRAY &out, MUINT32 feature)
{
    ...
    else if( needConfig(in, out, feature) )
    {
        ...
        if( this->onConfig(mFeature, in, out, mMaxImage, mMaxWarp, mRegisterMode) )
        {
            ...
            {
                this->registerGB(in, mInGBSet, GB_DIR_IN);
                this->registerGB(out, mOutGBSet, GB_DIR_OUT);
            }
            mStage = STAGE_CONFIG;
        }
        else
...
MBOOL GpuWarpBase::isSubset(const GB_SET &whole, const GB_PTR_ARRAY &part)
{
    TRACE_FUNC_ENTER();
    MBOOL subset = MTRUE;
    for( GB_PTR_ARRAY::const_iterator it = part.begin(), end = part.end(); it != end; ++it )
    {
        if( *it )
        {
            if( whole.count((*it)->get()) == 0 )
            {
                subset = MFALSE;
                break;
            }
...
MBOOL GpuWarpBase::isSubset(const GB_SET &whole, const GB_PTR part)
{
    TRACE_FUNC_ENTER();
    MBOOL ret = MFALSE;
    if( part && whole.count(part->get()) )
    {
        ret = MTRUE;
    }
    TRACE_FUNC_EXIT();
...
MBOOL GpuWarpBase::isGBRegister(GB_PTR in, GB_PTR out)
{
    TRACE_FUNC_ENTER();
    Mutex::Autolock lock(mMutex);
    MBOOL ret = MFALSE;
    ...
    if( mStage == STAGE_CONFIG )
    {
        ret = registerGB(in, mInGBSet, GB_DIR_IN) &&
              registerGB(out, mOutGBSet, GB_DIR_OUT);
    }
    TRACE_FUNC_EXIT();
    return ret;
}
...
MBOOL GpuWarpBase::registerGB(GB_PTR gb, GB_SET &set, GBDir dir)
{
    TRACE_FUNC_ENTER();
    MBOOL ret = MTRUE;

    if( gb && gb->get() && !isSubset(set, gb) )
    {
...
MVOID GpuWarpBase::registerGB(const GB_PTR_ARRAY &gbs, GB_SET &set, GBDir dir)
{
    TRACE_FUNC_ENTER();
    for( GB_PTR_ARRAY::const_iterator it = gbs.begin(), end = gbs.end(); it != end; ++it )
    {
