...
MBOOL MDPNode::prepareOut(const RequestPtr &request, const ImgBuffer &gpuOutput)
{
    ...
    MBOOL useGpuRecord = request->useDirectGpuOut();

    for( unsigned i = 0, n = request->mQParams.mvOut.size(); i < n; ++i )
    {
        if( !(skipDisplay && isDisplayOutput(request->mQParams.mvOut[i])) &&
            !(useGpuRecord && isRecordOutput(request->mQParams.mvOut[i])) )
        {
    ...
    if (request->getEISRecordCrop(CropInfo, WARP_DOMAIN))
    {
        mdpCrop = CropInfo.mCropRect;
        MY_LOGD("MDPNode mdpCrop use video x/y(%u,%u) ,w/h(%u,%u)", mdpCrop.p_integral.x , mdpCrop.p_integral.y, mdpCrop.s.w, mdpCrop.s.h);
    }else if (request->getEISDisplayCrop(CropInfo, WARP_DOMAIN, gpuOutput->getImageBuffer()->getImgSize()))
    {
        mdpCrop.p_integral.x &= (~1);
        mdpCrop.p_integral.y &= (~1);

        MY_LOGD("MDPNode mdpCrop use display x/y(%u,%u) ,w/h(%u,%u)", mdpCrop.p_integral.x , mdpCrop.p_integral.y, mdpCrop.s.w, mdpCrop.s.h);
    }else
    {
        mdpCrop.s.w = gpuOutput->getImageBuffer()->getImgSize().w;
        mdpCrop.s.h = gpuOutput->getImageBuffer()->getImgSize().h;
        MY_LOGD("MDPNode mdpCrop use buffer size x/y(%u,%u) ,w/h(%u,%u)", mdpCrop.p_integral.x , mdpCrop.p_integral.y, mdpCrop.s.w, mdpCrop.s.h);

    }

    //ret = convertRGBA8888(gpuOutput->getImageBufferPtr(), request->mvOut, mdpCrop);
    ret = convertRGBA8888(gpuOutput->getImageBufferPtr(), outputs, mdpCrop);

    if( request->needDump() )
    {
        for( unsigned i = 0; i < outputs.size(); ++i )
        {
            dumpData(request, outputs[i].mBuffer, "mdp_%d", i);
        }
    }
