...
public:
    // GpuWarpBase member
    virtual MBOOL onConfig(MUINT32 feature, const GB_PTR_ARRAY &in, const GB_PTR_ARRAY &out, const MSize &maxImage, const MSize &maxWarp, RegisterMode registerMode);
    ...
    virtual MBOOL onRegisterGB(GB_PTR gb, GBDir dir);
    virtual MUINT32 getNumRegisterSlot() const;

private:
    MBOOL initWarp(const GB_PTR_ARRAY &inputBuffers, const GB_PTR_ARRAY &outputBuffers, const MSize &maxImage);
    MVOID uninitWarp();

    MBOOL prepareBufferArray(GB_PTR *inputArray, GB_PTR *outputArray, const GB_PTR_ARRAY &inputBuffers, const GB_PTR_ARRAY &outputBuffers, MUINT32 inSize, MUINT32 outSize);

    MVOID configBasicWarpInfo(struct WarpImageExtInfo *warpInfo, GpuTuning *gpuTuning, MUINT32 feature, const MSize &maxWarp);
