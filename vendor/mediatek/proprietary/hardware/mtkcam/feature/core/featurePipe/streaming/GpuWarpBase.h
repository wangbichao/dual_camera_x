...
public:
    ...
    enum GBDir { GB_DIR_IN, GB_DIR_OUT };
    enum Feature {
                    USE_CPU = (1<<0),
                    FEATURE_VFB = (1<<1),
                    FEATURE_EIS2 = (1<<2),
    ...
    typedef std::set<android::GraphicBuffer*> GB_SET;
    ...
    MBOOL isGBRegister(GB_PTR in, GB_PTR out);
...
protected:
    virtual MBOOL onConfig(MUINT32 feature, const GB_PTR_ARRAY &in, const GB_PTR_ARRAY &out, const MSize &maxImage, const MSize &maxWarp, RegisterMode registerMode) = 0;
    ...
    virtual MBOOL onRegisterGB(GB_PTR gb, GBDir dir) = 0;

private:
    MBOOL needConfig(const GB_PTR_ARRAY &in, const GB_PTR_ARRAY &out, MUINT32 feature);
    MVOID updateConfig(MUINT32 feature);
    MBOOL isSubset(const GB_SET &whole, const GB_PTR_ARRAY &part);
    MBOOL isSubset(const GB_SET &whole, const GB_PTR part);
    MBOOL checkProcessParam(GB_PTR in, GB_PTR out, IImageBuffer *warpMap, const MSize &inputSize, const MSize &outputSize);
    MBOOL registerGB(GB_PTR gb, GB_SET &map, GBDir dir);
    MVOID registerGB(const GB_PTR_ARRAY &gbs, GB_SET &map, GBDir dir);
