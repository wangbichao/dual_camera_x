...
onThreadLoop()
{
    ...
    VSDOF_PRFLOG("DVE Enque start, reqID=%d t=%d", iReqIdx, pEnqueBufPool->startTimer());
    MBOOL bRet = mpDPEStream->DVEenque(enqueParams);
    VSDOF_PRFLOG("DVE Enque, reqID=%d, config time=%f", iReqIdx, pEnqueBufPool->countTimer());
...
handleDPEEnqueDone(DVEParams& rParams, EnquedBufPool* pEnqueBufPool)
{
    ...
    VSDOF_PRFLOG("+, reqID=%d, DPE exec-time=%f msec",
            pN3DImgInfo->getRequestPtr()->getRequestNo(), pEnqueBufPool->countTimer());
    ...
    GET_SMBUFFER_FROM_ENQPOOL_AND_INVALIDATE(pEnqueBufPool, BID_DPE_OUT_DMP_L, smImgBuf_DMP_L);
    GET_SMBUFFER_FROM_ENQPOOL_AND_INVALIDATE(pEnqueBufPool, BID_DPE_OUT_DMP_R, smImgBuf_DMP_R);GET_SMBUFFER_FROM_ENQPOOL_AND_INVALIDATE(pEnqueBufPool, BID_DPE_OUT_CFM_L, smImgBuf_CFM_L);
    ...
    // DMP /CFM
    TO_OCC_ImgBufInfo->addImageBuffer(BID_DPE_OUT_DMP_L, smImgBuf_DMP_L);
    TO_OCC_ImgBufInfo->addImageBuffer(BID_DPE_OUT_DMP_R, smImgBuf_DMP_R);
    TO_OCC_ImgBufInfo->addImageBuffer(BID_DPE_OUT_CFM_L, smImgBuf_CFM_L);
    TO_OCC_ImgBufInfo->addImageBuffer(BID_DPE_OUT_CFM_R, smImgBuf_CFM_R);
    // forward N3D output: MV_Y/SV_Y to OCC
    TO_OCC_ImgBufInfo->addImageBuffer(BID_N3D_OUT_MV_Y, pN3DImgInfo->getImageBuffer(BID_N3D_OUT_MV_Y));
    TO_OCC_ImgBufInfo->addImageBuffer(BID_N3D_OUT_SV_Y, pN3DImgInfo->getImageBuffer(BID_N3D_OUT_SV_Y));
    // post to OCC node
...
prepareDPEEnqueConfig(ImgInfoMapPtr pN3DImgInfo, DVEConfig& rDPEConfig, EnquedBufPool *pEnqueBufPool)
{
    ...
    SmartImageBuffer smImgBuf_MV_Y = pN3DImgInfo->getImageBuffer(BID_N3D_OUT_MV_Y);
    SmartImageBuffer smImgBuf_SV_Y = pN3DImgInfo->getImageBuffer(BID_N3D_OUT_SV_Y);
    SmartImageBuffer smImgBuf_MASK_M = pN3DImgInfo->getImageBuffer(BID_N3D_OUT_MASK_M);
    SmartImageBuffer smImgBuf_MASK_S  = pN3DImgInfo->getImageBuffer(BID_N3D_OUT_MASK_S);
    ...
    {
        // Main1 locations: L
        setupDPEBufInfo(DMA_DVE_IMGI, smImgBuf_MV_Y, rDPEConfig.Dve_Imgi_l);
        setupDPEBufInfo(DMA_DVE_IMGI, smImgBuf_SV_Y, rDPEConfig.Dve_Imgi_r);
        setupDPEBufInfo(DMA_DVE_MASKI, smImgBuf_MASK_M, rDPEConfig.Dve_Maski_l);
        setupDPEBufInfo(DMA_DVE_MASKI, smImgBuf_MASK_S, rDPEConfig.Dve_Maski_r);
    }
    else
    {   // Main1 location: R
