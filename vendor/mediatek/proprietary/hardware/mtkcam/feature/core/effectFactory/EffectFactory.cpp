...
#include "fb/cfb_effecthal_pp/include/FaceBeautyEffectHal.h"
#include "jpg/include/JPGEffectHal.h"
...
getSupportedVersion(String8 effectName, Vector<EffectHalVersion> &versions)
{
    FUNCTION_LOG_START;
    versions.clear();
    ALOGD("[%s], input effectName(%s), name leng(%d)", __FUNCTION__, effectName.string(), effectName.size());
    //@todo implement this
    #if 1   //EXAMPLE
    ...
    else if (effectName == "FaceBeauty")
    {
        version.effectName = "FaceBeauty";
#if '1' == EFFECTHAL_CFB_SUPPORT
        version.major = 1;
        version.minor = 0;
#else
        version.major = 0;
    ...
    {
        version.effectName = "Unknown";
        version.major = 0;
        version.minor = 0;
    }
    ALOGD("[%s], final name=%s, major(%d) minor(%d)", __FUNCTION__,  version.effectName.string(), version.major, version.minor);
