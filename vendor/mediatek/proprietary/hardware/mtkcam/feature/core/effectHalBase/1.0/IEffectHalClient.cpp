...
//public: // would not change state
    case GET_NAME_VERSION:
    {
        ...
        {
            reply->writeInt32(1);
            reply->writeString16(String16(version.effectName));
            reply->writeInt32(version.major);
            reply->writeInt32(version.minor);
        }
        else
        {
            reply->writeInt32(0);
...
# endif
    ...
    case GET_CAPTURE_REQUIREMENT:
    {
        ...
        ALOGD("requirements size=%zu", requirements.size());
...
//public: // for client
    case GET_INPUT_SURFACES:
    {
        ...
        for (size_t i=0; i<producers.size(); ++i)
        {
            ...
            if (producer != 0)
            {
                ALOGD("GET_INPUT_SURFACES, index=%zu, address=%p", i, producer.get());
                reply->writeInt32(1);  // marker that surface is not null

                char str[32];
                sprintf(str, "IEffectHalClient-producer-%zu", i);
