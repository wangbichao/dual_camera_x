...
#define MFLL_FEEDRAW_CONFIG                             "/sdcard/mfll/feed.config"
...
{
    MfllCore* pCore = new MfllCore;
    mfllLogD("MFNR Core Version: %d.%d.%d",
            MFLL_CORE_VERSION_MAJOR, MFLL_CORE_VERSION_MINOR, MFLL_CORE_VERSION_BUGFIX);
    mfllLogD("Create MFNR core version: %d.%d.%d", 1, 6, MFLL_CORE_VERSION_BUGFIX);
    return (IMfllCore*)pCore;
}
...
MfllCore::MfllCore(void)
{
    ...
    m_spPerfService = new MfllFeaturePerf;
...
enum MfllErr MfllCore::init(const MfllConfig_t &cfg)
{
    ...
    /* read property for checking is feeding RAW or not */
    m_isFeedRaw = m_spProperty->getProperty(Property_FeedRaw);
    if (isFeedRaw()) {
        MfllFeedRawConfig_t c;
        if (readFeedRawConfiguration(&c) != MfllErr_Ok)
            mfllLogE("ask to feed raw, but read feed raw config failed");
        else {
            m_width = c.width;
            m_height = c.height;
        }
...
unsigned int MfllCore::getReversion(void)
{
    ...
    v0 = MFLL_CORE_VERSION_BUGFIX;
    v1 = MFLL_CORE_VERSION_MINOR;
    v2 = MFLL_CORE_VERSION_MAJOR;
...
{
    char memchunk[32];
    sprintf(memchunk, "%d.%d.%d", MFLL_CORE_VERSION_MAJOR, MFLL_CORE_VERSION_MINOR, MFLL_CORE_VERSION_BUGFIX);
...
bool MfllCore::isFeedRaw(void)
{
    return (m_isFeedRaw > 0);
}

enum MfllErr MfllCore::readFeedRawConfiguration(MfllFeedRawConfig_t *config)
{
    MfllFeedRawConfig_t c;
    std::ifstream r(MFLL_FEEDRAW_CONFIG, std::ifstream::in);
    if (!r) {
        mfllLogE("%s: read feed raw config failed", __FUNCTION__);
        return MfllErr_NullPointer; // file not exists or open failed
    }
...
#if 0
    mfllLogD("FeedConfig: width     = %d", c.width);
    mfllLogD("FeedConfig: height    = %d", c.height);
    for (size_t i = 0; i < MFLL_MAX_FRAMES; i++)
    mfllLogD("FeedConfig: raw%d      = %s", i, c.fileName[i].c_str());
    mfllLogD("FeedConfig: reserved1 = %s", c.reserved1.c_str());
    mfllLogD("FeedConfig: reserved2 = %s", c.reserved2.c_str());
    mfllLogD("FeedConfig: reserved3 = %s", c.reserved3.c_str());
    mfllLogD("FeedConfig: reserved4 = %s", c.reserved4.c_str());
#endif
    return MfllErr_Ok;
}
...
sp<IMfllImageBuffer> MfllCore::retrieveBuffer(const enum MfllBuffer &s, int index /* = 0 */)
{
    ...
    switch (s) {
    case MfllBuffer_Raw:
        if (index < 0 || index >= static_cast<int>(getCaptureFrameNum())) {
    ...
    case MfllBuffer_AlgorithmWorking:
        if (index < 0 || index >= static_cast<int>(getCaptureFrameNum()-1)) {
...
enum MfllErr MfllCore::doEncodeYuvGolden(JOB_VOID)
{
    ...
    if (m_bypass.bypassEncodeYuvGolden || status.ignore) {
        ...
        err = m_spMfb->encodeRawToYuv(imgSrc.get(), m_imgYuvGolden.get(), YuvStage_GoldenYuy2);
        if (err != MfllErr_Ok) {
            mfllLogE("%s: Encode RAW to YUV fail", __FUNCTION__);
            goto lbExit;
        }
...
enum MfllErr MfllCore::doAllocQyuvBuffer(void *void_index)
{
    ...
    if (m_bypass.bypassAllocQyuvBuffer[index] || status.ignore) {
        ...
        /* full size YUV, and it's been inited */
            ...
            status.ignore = 1;
            pFullSizeImg->setAligned(16, 16);
            pFullSizeImg->convertImageFormat(ImageFormat_Yv16);
        }
        else {
...
void MfllCore::tellsFutureMemc()
{
    ...
    m_futureMemc.fu = std::async(std::launch::async, [this]()->void
    {
        ...
        auto funcProcessMemc = [this](int index)->void
        {
            ...
            MfllErr err = MfllErr_Ok;
            auto threadMe = std::async(std::launch::async, [this, index]()->MfllErr {
                // why we need to create a thread here because maybe we need to
                // adjust thread priority or CPU affinity for ME in the future.
                return doMotionEstimation((void*)(long)index);
            });

            threadMe.wait(); // wait for done

            if (threadMe.get() != MfllErr_Ok) {
                mfllLogW("ME(%d) failed, ignore MC and blending(%d)", index, index);
                m_bypass.bypassMotionCompensation[index] = 1;
                m_bypass.bypassBlending[index] = 1;
            }

            if (doMotionCompensation((void*)(long)index) != MfllErr_Ok) {
