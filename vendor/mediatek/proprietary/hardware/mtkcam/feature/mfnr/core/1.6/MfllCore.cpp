    ...
    if (m_bypass.bypassBss || status.ignore || (bssFrameCount <= 0)) {
        ...
        if (frameNumToSkip > 0) {
            ...
            if (idxForOperation < 0) {
                mfllLogE("%s: drop frame number(%u) is more than capture number(%u), ignore all frames",
                        __FUNCTION__,
                        frameNumToSkip,
                        getCaptureFrameNum());
                idxForOperation = 0;
            ...
            for (size_t i = static_cast<size_t>(idxForOperation); i < MFLL_MAX_FRAMES; i++) {
                ...
                if (i >= static_cast<size_t>(idxForImages)) {
                    mfllLogD("%s: clear images (index = %u)", __FUNCTION__, i);
                ...
                if (i >= static_cast<size_t>(idxForOperation)) {
                    mfllLogD("%s: bypass MEMC/BLD (index = %u)", __FUNCTION__, i);
                    m_bypass.bypassMotionEstimation[i] = 1;
                    m_bypass.bypassMotionCompensation[i] = 1;
                    m_bypass.bypassBlending[i] = 1;
                }
