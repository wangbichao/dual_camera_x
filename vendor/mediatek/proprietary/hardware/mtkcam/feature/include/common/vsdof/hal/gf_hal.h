...
struct GF_HAL_IN_DATA
{
    ...
    MUINT8          *depthMap;
    std::vector<IImageBuffer *> images;

    MFLOAT          convOffset; //from N3D HAL
    ...
    GF_HAL_IN_DATA()
        : magicNumber(-1)
        , scenario(eSTEREO_SCENARIO_UNKNOWN)
        , dofLevel(-1)
        , depthMap(NULL)
        , convOffset(0.0f)
        , touchPosX(0)
...
{
    MUINT8 *dmbg;       //Always 240x136
    MUINT8 *depthMap;   //Output size is configured in camera_custom_stereo.cpp of each target
