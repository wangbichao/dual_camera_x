...
IHDRProc2* IHDRProc2::loadModule()
{
    return &HDRProc2::getInstance();
}

// ---------------------------------------------------------------------------

static const int32_t moduleVersion = HDR_MODULE_API_VERSION_2_2;

ANDROID_SINGLETON_STATIC_INSTANCE(HDRProc2);

static Mutex sHDRListLock;
static DefaultKeyedVector<hdr_object_t, sp<NSCam::HDR>> sHDRList;
...
MBOOL HDRProc2::prepare(const HDRHandle& handle)
{
    ...
    const sp<NSCam::HDR>& obj(sHDRList.valueFor(handle.id));
        ...
        HDROutputType type, sp<IImageBuffer>& outBuffer)
{
    ...
    return sHDRList.valueFor(handle.id)->addOutputFrame(type, outBuffer);
}
