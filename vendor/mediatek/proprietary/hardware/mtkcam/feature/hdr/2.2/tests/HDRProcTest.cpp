...
#include <mtkcam/feature/hdr/HDRProc2.h>
...
MBOOL HDRProcTest::HDRProcCompleteCallback(
        MVOID* user, const sp<IImageBuffer>& hdrResult, MBOOL ret)
{
...
TEST_F(HDRProcTest, HDRProc)
{
    ...
    ASSERT_EQ(MTRUE, hdrProc.init(openID, mHDRHandle));
    ...
    // will be processed.
    hdrProc.setParam(mHDRHandle, HDRProcParam_Set_AOEMode, 0, 0);
    hdrProc.setParam(mHDRHandle, HDRProcParam_Set_MaxSensorAnalogGain, 16384, 0);
    hdrProc.setParam(mHDRHandle, HDRProcParam_Set_MaxAEExpTimeInUS, 500000, 0);
    hdrProc.setParam(mHDRHandle, HDRProcParam_Set_MinAEExpTimeInUS, 500, 0);
    hdrProc.setParam(mHDRHandle, HDRProcParam_Set_ShutterLineTime, 13139, 0);
    hdrProc.setParam(mHDRHandle, HDRProcParam_Set_MaxAESensorGain, 131072, 0);
    hdrProc.setParam(mHDRHandle, HDRProcParam_Set_MinAESensorGain, 1195, 0);
    hdrProc.setParam(mHDRHandle, HDRProcParam_Set_ExpTimeInUS0EV, 29996, 0);
    hdrProc.setParam(mHDRHandle, HDRProcParam_Set_SensorGain0EV, 3712, 0);
    hdrProc.setParam(mHDRHandle, HDRProcParam_Set_FlareOffset0EV, 0, 0);
    hdrProc.setParam(mHDRHandle, HDRProcParam_Set_GainBase0EV, 0, 0);
    hdrProc.setParam(mHDRHandle, HDRProcParam_Set_LE_LowAvg, 0, 0);
    hdrProc.setParam(mHDRHandle, HDRProcParam_Set_SEDeltaEVx100, 0, 0);
    hdrProc.setParam(mHDRHandle, HDRProcParam_Set_DetectFace, 0, 0);
    ...
    hdrProc.setParam(
            ...
            reinterpret_cast<MUINTPTR>(u4Histogram), 0);
    ...
    hdrProc.setParam(
            ...
            reinterpret_cast<MUINTPTR>(u4FlareHistogram), 0);
    ...
    hdrProc.setParam(
            mHDRHandle, HDRProcParam_Set_PLineAETable,
    ...
    hdrProc.getHDRCapInfo(
            mHDRHandle, HDRFrameNum, vu4Eposuretime,vu4SensorGain,vu4FlareOffset);
    ...
    hdrProc.setJpegParam(mHDRHandle, jpegSize, thumbnailSize, rot);

    MSize pictureSize(pic_w, pic_h);
    MSize postviewSize(800, 600);
    hdrProc.setShotParam(mHDRHandle, pictureSize, postviewSize, cropRegion);

    hdrProc.setParam(
            mHDRHandle, HDRProcParam_Set_sensor_size, sensor_w, sensor_h);

    hdrProc.setParam(
            mHDRHandle, HDRProcParam_Set_sensor_type, 1/*NSCam::SENSOR_TYPE_RAW*/, 0);

    // should input 3A information and get cap info before prepare.
    hdrProc.prepare(mHDRHandle);
    ...
    hdrProc.getParam(mHDRHandle, HDRProcParam_Get_src_main_format, uSrcMainFormat, empty);
    hdrProc.getParam(mHDRHandle, HDRProcParam_Get_src_main_size, uSrcMainWidth, uSrcMainHeight);
    hdrProc.getParam(mHDRHandle, HDRProcParam_Get_src_small_format, uSrcSmallFormat, empty);
    hdrProc.getParam(mHDRHandle, HDRProcParam_Get_src_small_size, uSrcSmallWidth, uSrcSmallHeight);
    ...
    ASSERT_EQ(MTRUE, hdrProc.start(mHDRHandle));

    ALOGD("mpHdrProc addOutputFrame (VA)addr:%p", DstImgBuffer->getBufVA(0));
    ASSERT_EQ(MTRUE, hdrProc.addOutputFrame(
                mHDRHandle, HDR_OUTPUT_JPEG_YUV ,DstImgBuffer));
    ALOGD("mpHdrProc addOutputFrame (VA)addr:%p", DstImg_TB_Buffer->getBufVA(0));
    ASSERT_EQ(MTRUE, hdrProc.addOutputFrame(
                mHDRHandle, HDR_OUTPUT_JPEG_THUMBNAIL_YUV ,DstImg_TB_Buffer));
    ...
    for (MINT32 i = 0; i < HDRFrameNum*2; i++)
    {
        ASSERT_EQ(MTRUE, hdrProc.addInputFrame(mHDRHandle, i,SrcImgBuffer[i]));
    }
#endif
