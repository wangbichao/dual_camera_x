    ...
    while   (1)
    {
        ...
        if  ( ! isEnabledState() || ! pBufQueue->isProcessorRunning())
        {
            ...
            while(1)
            {
                ...
                //
                if(i < mvRecBufInfo.size())
                {
                    MY_LOGI("Still buffer not return");
                    waitFrameCnt++;
                    if(waitFrameCnt > mRecBufNum)
                    {
                        MY_LOGI("force stop wait buffer return");
                        break;
                    }
                    waitAndHandleReturnBuffers(pBufQueue);
                }
    ...
    {
        {
            ...
            for(i=0; i<size; i++)
            {
                ...
                {
                    MY_LOGW("CNT(%d):Wait STA(%d)/FD(%d)/ADDR(%p/%p) to be released",
                            releaseFrameCnt,
                            mvRecBufInfo[i].Sta,
                            (uint32_t)(mvRecBufInfo[i].BufIonFd),
                            (MUINTPTR)(mvRecBufInfo[i].CbVirAddr),
