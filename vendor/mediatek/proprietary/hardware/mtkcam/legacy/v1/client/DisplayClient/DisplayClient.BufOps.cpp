...
#define GET_IIMGBUF_IMG_STRIDE_IN_BYTE(pStreamImgBuf, plane)   \
                ((NSCam::Utils::Format::queryPlaneCount(NSCam::Utils::Format::queryImageFormat(pStreamImgBuf->getImgFormat().string())) >= (plane+1)) ? \
                ((pStreamImgBuf->getImgWidthStride(plane)*NSCam::Utils::Format::queryPlaneBitsPerPixel(NSCam::Utils::Format::queryImageFormat(pStreamImgBuf->getImgFormat().string()),plane)))>>3 : 0)
...
prepareOneTodoBuffer(sp<IImgBufQueue>const& rpBufQueue)
{
    ...
    if (mFirstFrameBlack)
    {
        ...
        MY_LOGD("first Frame black format=%s ori=%d w=%d h=%d bufsize=%d",
            ...
            pStreamImgBuf->getOrientation(),
            pStreamImgBuf->getImgWidth(),
            pStreamImgBuf->getImgHeight(),
            pStreamImgBuf->getBufSize()
            );
        //
        void* virtAddr = pStreamImgBuf->getVirAddr();
        int value = 0;
        if(pStreamImgBuf->getOrientation() & HAL_TRANSFORM_ROT_90)  //It also included rot 270 case, because rot 270 = rot90|flipH|flipV )
        {
            ...
            {
               memset((void*)((char*)virtAddr+i*2),value,1);
            }
        }
        else
        {
            ...
            memset((void*)((char*)virtAddr+YSize), iFirstFrameColor, UVSize);
        }
        //
