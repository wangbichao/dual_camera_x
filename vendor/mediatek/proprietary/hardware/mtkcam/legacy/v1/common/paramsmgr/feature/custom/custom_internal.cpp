...
int getDumpFlag()
{
    ...
    property_get("debug.camera.dft.dump", value, "0");
    ...
    */
    ...
    while(should_calculate && baseW <= max_w && baseH <= max_h)
    {
        ...
        if(max_w >= 1024 && max_h >= 768)
            size_arr[currCnt++] = "1024x768";
        if(max_w >= 2048 && max_h >= 1536)
            size_arr[currCnt++] = "2048x1536";
    #endif
#endif
...
int add5_3_PictureSizes(char **size_arr, int addedCntAfter, int max_w, int max_h)
{
    ...
    */
    ...
    if(max_w <= 640 && max_h <= 480)
    {
        size_arr[currCnt++] = "320x192";
        size_arr[currCnt++] = "640x384";
        return currCnt;
    }
...
int add16_9_PictureSizes(char **size_arr, int addedCntAfter, int max_w, int max_h)
{
    ...
    */
    ...
    if(max_w <= 640 && max_h <= 480)
    {
        size_arr[currCnt++] = "256x144";
        size_arr[currCnt++] = "512x288";
        return currCnt;
    }
...
label_exit:
    ...
    MY_LOGE("parseStrToSize failed, s:[%s], set default value:640x480", ret, s);
    *w = 640;
    *h = 480;
