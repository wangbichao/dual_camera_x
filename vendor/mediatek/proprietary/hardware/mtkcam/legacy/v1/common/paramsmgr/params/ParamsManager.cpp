...
getIfFirstPreviewFrameAsBlack()
{
    ...
    MY_LOGD_IF(mEnableDebugLog, "firsFrameBlack(%d)",firsFrameBlack);
    if (firsFrameBlack == 1)
    {
        ret = true;
    }
...
setDisplayRotationSupported(bool isSupport)
{
    ...
    MY_LOGD_IF(mEnableDebugLog, "setDisplayRotationSupported = %s", isSupport ? CameraParameters::TRUE : CameraParameters::FALSE);
}
