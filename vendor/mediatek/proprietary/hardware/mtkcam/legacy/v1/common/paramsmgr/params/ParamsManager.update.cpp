...
updateParams(int32_t const i4FKeyIndex)
{
    ...
    if  ( rSceneKeyedMap.getType() == "default-values" )
    {
        ...
        MY_LOGD_IF(mEnableDebugLog, "[%02d] %s=%s;%s=%s", i4FKeyIndex, s8KeyName.string(), s8ItemDefault.string(), s8ItemListKeyName.string(), s8ItemList.string());
        return  true;
    }
    //
...
updateZoomParams(NSCameraFeature::FeatureInfo const& rFeatureInfo)
{
    ...
    //
    MY_LOGD_IF(mEnableDebugLog, " %s=%s", CameraParameters::KEY_ZOOM_RATIOS, s8ZoomRatios.string());
    //
...
updatePreviewFpsParams(NSCameraFeature::FeatureInfo const& rFeatureInfo)
{
    ...
    //
    MY_LOGD_IF(mEnableDebugLog, "%s=%s;%s=%s",
    ...
    MY_LOGD_IF(mEnableDebugLog, "FrameRateList=%s;UpdateFpsRangeList=%s",s8FrameRateList.string(),s8UpdateFpsRangeList.string());
    //
    ...
    MY_LOGD_IF(mEnableDebugLog, "FPS:%d,%d-%d",
            mDefaultPreviewFps,
            mDefaultPreviewFpsRange[0],
            mDefaultPreviewFpsRange[1]);
    //
...
updateExposureCompensationParams(NSCameraFeature::FeatureInfo const& rFeatureInfo)
{
       ...
       NULL != mParameters.get(CameraParameters::KEY_EXPOSURE_COMPENSATION))
    {
        ...
        MY_LOGD_IF(mEnableDebugLog, "%s=%s;%s=%s;%s=%s;%s=%s",
...
updateVideoSnapshotParams(NSCameraFeature::FeatureInfo const& rFeatureInfo)
{
    ...
    //
    MY_LOGD_IF(mEnableDebugLog, "%s=%s", CameraParameters::KEY_VIDEO_SNAPSHOT_SUPPORTED, rs8Supported.string());
    //
...
updateDNGParams(NSCameraFeature::FeatureInfo const& rFeatureInfo)
{
    String8 const& rs8Supported = rFeatureInfo.getDefaultItem();
    //
    MY_LOGD_IF(mEnableDebugLog, "%s=%s", MtkCameraParameters::KEY_DNG_SUPPORTED, rs8Supported.string());
    //
...
updateDefaultParams1()
{
    ...
    {
        MY_LOGD_IF(mEnableDebugLog, "Reset Panel Size to 0x0");
        ...
        for ( unsigned int i = 0 ; i < vdoSizes.size() ; i++ )
        {
            ...
            for ( Vector<Size>::iterator prvIt = prvSizes.end()-1; prvIt != prvSizes.begin()-1; prvIt-- )
            {
                if ( maxWidth == prvIt->width && maxHeight == prvIt->height )
                {
                    ...
                    MY_LOGD_IF(mEnableDebugLog, "KEY_PREFERRED_PREVIEW_SIZE_FOR_VIDEO=%s", psize);
                    return true;
                }
            }
        }
...
evaluateHalAppModeUtility(String8& mode) const
{
    ...
    MY_LOGD_IF(mEnableDebugLog, "KEY_CAMERA_MODE:%d, HAL Force Mode = [%s]", i4CamMode, halForceMode);
    //  Force Hal App mode (system property)
    if(strcmp(halForceMode, "None")) {
        MY_LOGD_IF(mEnableDebugLog, "Applying Halforcemode via property");
    ...
    if  ( NSCam::Utils::Property::tryGet(String8(PROPERTY_KEY_HAL_FORCEAPPMODE), mode ) )
    {
        if( mode != "" )
        {
            MY_LOGD_IF(mEnableDebugLog, "force mode: %s", mode.string());
            //clear property
    ...
    // check recording-hint
    {
        ...
        if ( ! p )
        {
            MY_LOGD_IF(mEnableDebugLog, "No KEY_RECORDING_HINT");
            recordingHint = false;
        }
        else
        {
        ...
        if( recordingHint )
        {
            MY_LOGD_IF(mEnableDebugLog, "recording hint = true, use default-mode");
    ...
    if  ( i4CamMode == MtkCameraParameters::CAMERA_MODE_NORMAL )
    {
        ...
        else
        {
            ...
            MY_LOGD_IF(mEnableDebugLog, "zsd is \"on\": default zsd-mode");
    ...
    if  ( i4CamMode == MtkCameraParameters::CAMERA_MODE_MTK_PRV )
    {
        ...
        // zsd: only for test
        {
            ...
            MY_LOGD_IF(mEnableDebugLog, "zsd mode %d", i4ZsdMode);
...
lbExit:
    ...
    //
    MY_LOGD_IF(mEnableDebugLog, "property:%s=%s[%d]", PROPERTY_KEY_HAL_APPMODE, s8HalAppMode.string(), mi4HalAppMode);
    return true;
}
