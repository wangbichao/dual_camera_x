...
{
    FUNCTION_LOG_START;
    MY_LOGD("%s: size: I/O Surface(%zu,%zu), I/O FrameInfo(%zu,%zu), I/O Buffer(%zu,%zu), Listener(%zu), mOutputEffectParams(%zu), mBufferMap(%zu), mOutputBuffers(%zu)",
...
status_t
EffectHalClient::
setParameters(const sp<EffectParameter> parameter)
{
    FUNCTION_LOG_START;
    status_t ret = OK;
...
addBufferQueue(Vector< sp<IGraphicBufferProducer> > &input, int index, int *height, int *width, int format)
{
    ...
    MY_LOGD_IF(mLogLevel, "[%s]: producer(Binder):%p, minBufs=%zu, mMaxBufferQueueSize=%zu, itemConsumer=%p",
...
getInputSurfaces(Vector< sp<IGraphicBufferProducer> > &input)
{
    ...
    MY_LOGD_IF(mLogLevel, "[%s]: requirements.size()=%zu", __FUNCTION__, requirements.size());
    ...
    MY_LOGD_IF(mLogLevel, "[%s]: init vInputFrameInfo, size=%zu", __FUNCTION__, requirements.size());
    ...
    MY_LOGD_IF(mLogLevel, "[%s]: mListener size=%zu", __FUNCTION__, mListener.size());
    FUNCTION_LOG_END;
    return OK;
}
...
setOutputSurfaces(Vector< sp<IGraphicBufferProducer> > &ouput, Vector<sp<EffectParameter> > &effectParams)
{
    ...
    for (size_t idx = 0; idx < ouput.size(); ++idx)
    {
        ...
        MY_LOGD_IF(mLogLevel, "[%s]: idx=%zu, producer(Binder):%p", __FUNCTION__, idx, IInterface::asBinder(ouput[idx]).get());
        ...
        if (format == HAL_PIXEL_FORMAT_BLOB)
        {
            ...
            if (maxJpegBufferSize == 0)
            {
                ...
                {
                    ALOGE("[%s]: Error staticMetadat, maxJpegWidth=%zd, maxJpegHeight=%zd", __FUNCTION__, maxJpegWidth, maxJpegHeight);
                    return -22;
                }
                ...
                MY_LOGD_IF(mLogLevel, "[%s]: jpegBufferSize=%zd, kMinJpegBufferSize=%zd, scaleFactor=%f",
            ...
            MY_LOGD_IF(mLogLevel, "[%s]: native_window_set_buffers_dimensions: (w = %d, h = %d), realH:%zd, realW:%d",
    ...
    MY_LOGD_IF(mLogLevel, "[%s]: init vOutputFrameInfo, size=%zu", __FUNCTION__, outputFrameCount);
...
{
    MY_LOGD_IF(mLogLevel, "[%s]: index=%d, mInputFrameInfo size=%zu repeat(%d)",
        __FUNCTION__, index, mInputFrameInfo.size(), repeat);
...
setInputsyncMode(int index, bool sync)
{
    //david add
    FUNCTION_LOG_START;
    if ((size_t)index > mInputSyncMode.size()-1)
    {
        //return outof index;
        return OK;
    }
...
setOutputsyncMode(int index, bool sync)
{
    //david add
    FUNCTION_LOG_START;
    if ((size_t)index < mOutputSyncMode.size()-1)
    {
        //return outof index;
        return OK;
    }
...
getOutputBuffer(int surfaceIdx, int bufferCount, sp<ANativeWindow> anw, sp<EffectParameter> param)
{
    ...
    for(int i=0; i<bufferCount; i++)
    {
        ...
        if (res != OK)
        {
            ALOGE("[%s]:  Unable to dequeue buffer: %s (%d) surfaceId:%d bufferIndex:%zu",
...
onBufferFrameAvailable(int idx)
{
    ...
    MY_LOGD_IF(mLogLevel, "[%s]: index=%d, mInputBufferInfo size=%zu, sync mode=%d", __FUNCTION__, idx, mInputBufferInfo.size(), int(sync));
    ...
    if (!sync)
    {
        MY_LOGD_IF(mLogLevel, "[%s]: do updateEffectRequest, vIn size=%zu, vOut size=%zu, ts=%" PRId64 "",
...
EffectHalClient::configureANW(const sp<ANativeWindow>& anw,
    ...
    MY_LOGD_IF(mLogLevel, "set_buffers_dimensions, result: %d", err);

    err = native_window_set_buffers_format(anw.get(), params.format);
    MY_LOGD_IF(mLogLevel, "set_buffers_format, result: %d", err);
...
EffectHalClient::produceOneFrame(const sp<ANativeWindow>& anw,
    ...
    status_t err;
    ANativeWindowBuffer* anb;
    MY_LOGD_IF(mLogLevel, "Dequeue buffer from %p, w(%d)", anw.get(), params.width);
...
onAborted(const IEffectHalClient* effectClient, const EffectResult& result)
{
    ...
    for(size_t i=0; i<mInputBufferInfo.size(); i++)
    {
        ...
        {
            MY_LOGD_IF(mLogLevel, "input buffer already free, idx=%zu", i);
            continue;
        }
        MY_LOGD_IF(mLogLevel, "free input buffer, idx=%zu, surfaceId=%d", i, mInputBufferInfo.valueAt(i)->mSurfaceId);
    ...
    for(size_t i=0; i<mOutputBufferInfo.size(); i++)
    {
        ...
        {
            MY_LOGD_IF(mLogLevel, "output buffer already free, idx=%zu", i);
            continue;
        }
        MY_LOGD_IF(mLogLevel, "free output buffer, idx=%zu, surfaceId=%d", i, mOutputBufferInfo[i]->mSurfaceId);
        ...
        if (res != OK)
        {
            MY_LOGE("%s: Unable to cancelBuffer mSurfaceId:%d, bufferIdx:%zu, %s (%d)",
