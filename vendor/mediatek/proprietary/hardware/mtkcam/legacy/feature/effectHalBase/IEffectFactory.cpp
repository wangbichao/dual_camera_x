...
public:
    ...
    {
        FUNCTION_LOG_START;
        // avoid compile unused parameter warning.
        (void)effectName;
        (void)versions;
        //@todo implement this
        FUNCTION_LOG_END;
        ...
        uint32_t code, const Parcel& data, Parcel* reply, uint32_t flags)
{
    ...
    switch(code) {
        ...
        case GET_SUPPORTED_VERSION: {
            CHECK_INTERFACE(IEffectFactory, data, reply);
            String8 effectName = String8(data.readString16());
            Vector<EffectHalVersion> versions;
            status_t ret = getSupportedVersion(effectName, versions);
            reply->writeNoException();
            reply->writeInt32(ret);

            reply->writeInt32(versions.size());
            for(size_t i=0 ; i < versions.size() ; i++)
            {
                reply->writeInt32(1); // tell proxy that this object not null!
                reply->writeString16(String16(versions[i].effectName));
                reply->writeInt32(versions[i].major);
                reply->writeInt32(versions[i].minor);
            }
            return NO_ERROR;
        } break;
