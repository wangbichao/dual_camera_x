...
LscMgr2*
LscMgr2::
createInstance(ESensorDev_T const eSensorDev, MINT32 i4SensorIdx, NVRAM_CAMERA_ISP_PARAM_STRUCT& rIspNvram, MUINT8 fgType)
{
    ...
    case 2:
        ...
        case ESensorDev_Main:       //  Main Sensor
            _pLscMain = LscMgr2RtoCycle::createInstance(ESensorDev_Main, i4SensorIdx, rIspNvram);
            LSC_LOG_END("RTOC ESensorDev_Main(0x%p), i4SensorIdx(%d)", _pLscMain, i4SensorIdx);
            return _pLscMain;
        case ESensorDev_MainSecond: //  Main Second Sensor
            _pLscMain2 = LscMgr2RtoCycle::createInstance(ESensorDev_MainSecond, i4SensorIdx, rIspNvram);
            LSC_LOG_END("RTOC ESensorDev_MainSecond(0x%p), i4SensorIdx(%d)", _pLscMain2, i4SensorIdx);
            return _pLscMain2;
        case ESensorDev_Sub:        //  Sub Sensor
            _pLscSub = LscMgr2RtoCycle::createInstance(ESensorDev_Sub, i4SensorIdx, rIspNvram);
            LSC_LOG_END("RTOC ESensorDev_Sub(0x%p), i4SensorIdx(%d)", _pLscSub, i4SensorIdx);
            return _pLscSub;
        }
        break;
    case 1:
        ...
        default:
        case ESensorDev_Main:       //  Main Sensor
            _pLscMain = LscMgr2Rto::createInstance(ESensorDev_Main, i4SensorIdx, rIspNvram);
            LSC_LOG_END("RTO ESensorDev_Main(0x%p), i4SensorIdx(%d)", _pLscMain, i4SensorIdx);
            return _pLscMain;
        case ESensorDev_MainSecond: //  Main Second Sensor
            _pLscMain2 = LscMgr2Rto::createInstance(ESensorDev_MainSecond, i4SensorIdx, rIspNvram);
            LSC_LOG_END("RTO ESensorDev_MainSecond(0x%p), i4SensorIdx(%d)", _pLscMain2, i4SensorIdx);
            return _pLscMain2;
        case ESensorDev_Sub:        //  Sub Sensor
            _pLscSub = LscMgr2Rto::createInstance(ESensorDev_Sub, i4SensorIdx, rIspNvram);
            LSC_LOG_END("RTO ESensorDev_Sub(0x%p), i4SensorIdx(%d)", _pLscSub, i4SensorIdx);
            return _pLscSub;
        }
        break;
    case 0:
...
init()
{
    ...
    {
        if(m_u4LogEn & EN_LSC_LOG_SHOW_RESOLUTION_INFO)
        {
...
getSensorResolution()
{
    SensorHal* pIHalSensor;
     //ME14@@pIHalSensor = SensorHal::createInstance();
    if (pIHalSensor->init())
    {
        pIHalSensor->destroyInstance();
        pIHalSensor = NULL;
        LSC_ERR("Fail to init Sensor HAL");
        return MFALSE;
    }

    MINT32 u4NumSensorModes = MIN(3, (MINT32)ESensorMode_NUM);

    pIHalSensor->sendCommand((halSensorDev_e)m_eSensorDev, SENSOR_CMD_GET_SENSOR_PRV_RANGE,
            (int)&m_rSensorResolution.u4SensorPreviewWidth,
            (int)&m_rSensorResolution.u4SensorPreviewHeight,
            0);
    pIHalSensor->sendCommand((halSensorDev_e)m_eSensorDev, SENSOR_CMD_GET_SENSOR_FULL_RANGE,
            (int)&m_rSensorResolution.u4SensorCaptureWidth,
            (int)&m_rSensorResolution.u4SensorCaptureHeight,
            0);
    pIHalSensor->sendCommand((halSensorDev_e)m_eSensorDev, SENSOR_CMD_GET_SENSOR_VIDEO_RANGE,
            (int)&m_rSensorResolution.u4SensorVideoWidth,
            (int)&m_rSensorResolution.u4SensorVideoHeight,
            0);
    if(m_u4LogEn & EN_LSC_LOG_SHOW_RESOLUTION_INFO)
    {
        LSC_LOG("u4NumSensorModes(%d)", u4NumSensorModes);
    ...
    MINT32 i;
    m_rSensorCropWin[ESensorMode_Preview].u4W    = m_rSensorResolution.u4SensorPreviewWidth;
    m_rSensorCropWin[ESensorMode_Preview].u4H    = m_rSensorResolution.u4SensorPreviewHeight;
    m_rSensorCropWin[ESensorMode_Capture].u4W    = m_rSensorResolution.u4SensorCaptureWidth;
    m_rSensorCropWin[ESensorMode_Capture].u4H    = m_rSensorResolution.u4SensorCaptureHeight;
    m_rSensorCropWin[ESensorMode_Video].u4W      = m_rSensorResolution.u4SensorVideoWidth;
    m_rSensorCropWin[ESensorMode_Video].u4H      = m_rSensorResolution.u4SensorVideoHeight;
    for (i = 0; i < u4NumSensorModes; i++)
    {
        convertSensorCrop(!m_fgSensorCropInfoNull, m_rSensorCropWin[ESensorMode_Capture], m_rSensorCropWin[i], m_rSensorCropCfg[i]);
    }
...
{
    MUINT32 u4HwAddr = IspDebug::getInstance().readLsciAddr();
