...
init()
{
    ...
    if  ( mpPreviewCmdQueThread == 0 || OK != (status = mpPreviewCmdQueThread->run("PreviewCmdQueThread")) )
    {
        MY_LOGE(
    ...
    if  ( mpCaptureCmdQueThread == 0 || OK != (status = mpCaptureCmdQueThread->run("CaptureCmdQueThread") ) )
    {
        MY_LOGE(
...
sendCommand(int32_t cmd, int32_t arg1, int32_t arg2)
{
    ...
    case CAMERA_CMD_ENABLE_FOCUS_MOVE_MSG:
        CAM_LOGD("[sendCommand] CAMERA_CMD_ENABLE_FOCUS_MOVE_MSG (%d)", arg1);
        enableAFMove(arg1);
        return OK;
        break;
