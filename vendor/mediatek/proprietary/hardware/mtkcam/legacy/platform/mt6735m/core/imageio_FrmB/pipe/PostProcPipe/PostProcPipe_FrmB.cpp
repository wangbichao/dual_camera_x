...
//            numPortId = mCropPathInfo[idx].u4NumPortId;
            ...
            for (j = 0 ; j < DMA_PORT_TYPE_NUM; j++)/*DMA_OUT_PORT_NUM?*/
            {
...
configPipe(vector<PortInfo const*>const& vInPorts, vector<PortInfo const*>const& vOutPorts, PipePackageInfo *pPipePackageInfo)
{
    ...
    MUINT32 i, tableSize, mdpCheck1 = 0, mdpCheck2 = 0;
    //
    ...
    if(pPipePackageInfo->isV3==true){
        ...
        if(pPipePackageInfo->pTuningQue != NULL){
            ...
            PIPE_DBG("[V3_Tuning]en1(0x%08x),en2(0x%08x),dma(0x%08x)",pIspPhyReg->CAM_CTL_EN1.Raw,pIspPhyReg->CAM_CTL_EN2.Raw,pIspPhyReg->CAM_CTL_DMA_EN.Raw);
        ...
        PIPE_DBG("[hrz]crop(%d,%d),src(%d,%d),dst(%d,%d), pixIdP2(%d)", \
    ...
    PIPE_DBG("drvScenario(%d),EN1(0x%x),EN2(0x%x),DMAEN(0x%x),FMTSEL(0x%x),CTLSEL(0x%x),MUXSEL(0x%x),MUXSEL2(0x%x),SRAM_MUX(0x%x),p2SubMode(%d)", \
            drvScenario, \
