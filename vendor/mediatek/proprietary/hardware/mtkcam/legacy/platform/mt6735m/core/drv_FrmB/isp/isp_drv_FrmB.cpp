...
MBOOL IspDrvImp::dumpP2DebugLog(IspDumpDbgLogP2Package* pP2Package)
{
    ...
    if (pP2Package==NULL) {
        LOG_ERR("[Error] pP2Package==NULL");
        return MFALSE;
    }

    if (pP2Package->tpipeTableVa==NULL || pP2Package->pIspReg==NULL
		|| pP2Package->pIspTpipeCfgInfo==NULL) {
        LOG_ERR("[Error] Some Pointer==NULL!");
        return MFALSE;
    }
