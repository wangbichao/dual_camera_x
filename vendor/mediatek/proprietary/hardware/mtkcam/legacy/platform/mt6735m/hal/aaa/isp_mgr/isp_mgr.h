...
#define REG_ADDR(reg)\
        ((MUINTPTR)(&getIspReg()->reg) - (MUINTPTR)getIspReg())
...
protected:
     ...
     , m_eSensorDev(eSensorDev)
    {
        ...
        if (m_eSensorDev == ESensorDev_Main) {
            INIT_ISP_DRV_SCENARIO(EIspProfile_NormalPreview, eSoftwareScenario_Main_Normal_Stream);
            INIT_ISP_DRV_SCENARIO(EIspProfile_ZsdPreview_CC, eSoftwareScenario_Main_Normal_Stream);
            INIT_ISP_DRV_SCENARIO(EIspProfile_ZsdPreview_NCC, eSoftwareScenario_Main_Normal_Stream);
            INIT_ISP_DRV_SCENARIO(EIspProfile_NormalCapture, eSoftwareScenario_Main_Normal_Stream);
            INIT_ISP_DRV_SCENARIO(EIspProfile_VideoPreview, eSoftwareScenario_Main_Normal_Stream);
            INIT_ISP_DRV_SCENARIO(EIspProfile_VideoCapture, eSoftwareScenario_Main_Normal_Stream);
            INIT_ISP_DRV_SCENARIO(EIspProfile_MFCapPass1, eSoftwareScenario_Main_Normal_Stream);
            INIT_ISP_DRV_SCENARIO(EIspProfile_MFCapPass2, eSoftwareScenario_Main_Normal_Stream);
        }
        else /*if (m_eSensorDev == ESensorDev_Sub)*/ {
            INIT_ISP_DRV_SCENARIO(EIspProfile_NormalPreview, eSoftwareScenario_Sub_Normal_Stream);
            INIT_ISP_DRV_SCENARIO(EIspProfile_ZsdPreview_CC, eSoftwareScenario_Sub_Normal_Stream);
            INIT_ISP_DRV_SCENARIO(EIspProfile_ZsdPreview_NCC, eSoftwareScenario_Sub_Normal_Stream);
            INIT_ISP_DRV_SCENARIO(EIspProfile_NormalCapture, eSoftwareScenario_Sub_Normal_Stream);
            INIT_ISP_DRV_SCENARIO(EIspProfile_VideoPreview, eSoftwareScenario_Sub_Normal_Stream);
            INIT_ISP_DRV_SCENARIO(EIspProfile_VideoCapture, eSoftwareScenario_Sub_Normal_Stream);
            INIT_ISP_DRV_SCENARIO(EIspProfile_MFCapPass1, eSoftwareScenario_Sub_Normal_Stream);
            INIT_ISP_DRV_SCENARIO(EIspProfile_MFCapPass2, eSoftwareScenario_Sub_Normal_Stream);
        }
    }
...
protected:
    ...
    getIspReg() const
    {
        return reinterpret_cast<isp_reg_t*>(NS3Av3::IspDrvMgr::getInstance().getIspReg());
    }

    inline
    MBOOL
    readRegs(RegInfo_T*const pRegInfo, MUINT32 const u4RegInfoNum) const
    {
        return NS3Av3::IspDrvMgr::getInstance().readRegs(pRegInfo, u4RegInfoNum);
    }

    inline
    MBOOL
    writeRegs(RegInfo_T*const pRegInfo, MUINT32 const u4RegInfoNum, isp_reg_t* pReg)
    {
        //return IspDrvMgr::getInstance().writeRegs(pRegInfo, u4RegInfoNum);
        if (pReg)
        {
            MUINT32 i;
...
public: // Interfaces
    ...
    reset()
    {
        ...
        MBOOL err = readRegs(static_cast<RegInfo_T*>(m_pRegInfo), m_u4RegInfoNum);
...
protected:
    ISP_MGR_CTL_EN(ESensorDev_T const eSensorDev)
...
protected:
    ISP_MGR_OBC(ESensorDev_T const eSensorDev)
        : ISP_MGR_BASE_T(m_rIspRegInfo, ERegInfo_NUM, m_u4StartAddr, eSensorDev)
        , m_bEnable(MTRUE)
        , m_u4IspAEGain(512)
...
protected:
    ISP_MGR_LSC(ESensorDev_T const eSensorDev)
        : ISP_MGR_BASE_T(m_rIspRegInfo, ERegInfo_NUM, m_u4StartAddr, eSensorDev)
        , m_u4StartAddr(REG_ADDR(CAM_LSCI_BASE_ADDR))
        , m_fgOnOff(MTRUE)
    {
        INIT_REG_INFO_ADDR(CAM_LSCI_BASE_ADDR); // 0x426C
        INIT_REG_INFO_ADDR(CAM_LSCI_OFST_ADDR); // 0x4270
        INIT_REG_INFO_ADDR(CAM_LSCI_XSIZE);     // 0x4274
        INIT_REG_INFO_ADDR(CAM_LSCI_YSIZE);     // 0x4278
        INIT_REG_INFO_ADDR(CAM_LSCI_STRIDE);    // 0x427C
...
public: // Interfaces.
    ...
    MBOOL setIspAWBGain(AWB_GAIN_T& rIspAWBGain);
    /*
    {
...
protected:
    ISP_MGR_PCA(ESensorDev_T const eSensorDev)
...
public: //    Interfaces
    MBOOL config(AWB_STAT_CONFIG_T& rAWBStatConfig, MINT32 i4SensorIndex);
...
protected:
    ISP_MGR_AE_RAWPREGAIN2(ESensorDev_T const eSensorDev)
        : ISP_MGR_BASE_T(m_rIspRegInfo, ERegInfo_NUM, m_u4StartAddr, eSensorDev)
...
public: //    Interfaces
    ...
    inline AWB_GAIN_T getRAWPregain2()
    {
...
public:
    ...
    ISP_MGR_AE_RAWPREGAIN2_DEV()
        : ISP_MGR_AE_RAWPREGAIN2_T(eSensorDev)
    {}
...
protected:
    ISP_MGR_AE_STAT_HIST_CONFIG(ESensorDev_T const eSensorDev)
        : ISP_MGR_BASE_T(m_rIspRegInfo, ERegInfo_NUM, m_u4StartAddr, eSensorDev)
        , m_u4StartAddr(REG_ADDR(CAM_AE_RAWLIMIT2_0))
        , m_i4SensorIndex(0)
    {
        INIT_REG_INFO_ADDR(CAM_AE_HST_CTL);       // 0x4650
...
public: //    Interfaces
    MBOOL config(AE_STAT_PARAM_T &rAEStatConfig, MINT32 i4SensorIndex);
...
public:
    ...
    ISP_MGR_AE_STAT_HIST_CONFIG_DEV()
        : ISP_MGR_AE_STAT_HIST_CONFIG_T(eSensorDev)
    {}
