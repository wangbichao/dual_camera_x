...
#include <mtkcam/drv_common/tpipe_config.h>
//
...
int ret = 0;
    ...
    if (!m_fgIsGdmaMode)    // Normal Mode (i.e. not GDMA).
    {
        ...
        //tile buffer for mdp mgr strcture
        for(int q=0;q<ISP_DRV_CQ_NUM;q++)
        {
            tpipeBufInfo[q].size = MAX_ISP_TILE_TDR_HEX_NO;
            tpipeBufInfo[q].useNoncache = 1; //alloc non-cacheable mem.
            //
            if ( m_pIMemDrv->allocVirtBuf(&tpipeBufInfo[q]) ) {
                ISP_FUNC_ERR("ERROR:m_pIMemDrv->allocVirtBuf(tpipeBufInfo)");
            }
            g_MdpMgrCfgData[q].ispTpipeCfgInfo.drvinfo.tpipeTableVa = (MUINT32*)( (tpipeBufInfo[q].virtAddr+7) & (~7) );
            //
            if ( m_pIMemDrv->mapPhyAddr(&tpipeBufInfo[q]) ) {
                ISP_FUNC_ERR("ERROR:m_pIMemDrv->mapPhyAddr(tpipeBufInfo)");
            }
            //
            g_MdpMgrCfgData[q].ispTpipeCfgInfo.drvinfo.tpipeTablePa =  ( (tpipeBufInfo[q].phyAddr+7) & (~7) );
            ISP_FUNC_INF("[Tpipe alloc]q(%d),va/pa(0x%08x, 0x%08x)",q,g_MdpMgrCfgData[q].ispTpipeCfgInfo.drvinfo.tpipeTableVa,g_MdpMgrCfgData[q].ispTpipeCfgInfo.drvinfo.tpipeTablePa);
        }
    }
    else    // GDMA mode.
    {
    ...
    if (!m_fgIsGdmaMode)    // Normal Mode (i.e. not GDMA).
    {
        for (MINT32 cq = 0; cq<ISP_DRV_CQ_NUM; cq++ ) {
            ...
            //descriptor buffer
            ISP_FUNC_DBG("[mpIspCQDescriptorPhy[cq%d]]:0x%p ",cq,m_pIspDrv->getCQDescBufPhyAddr((ISP_DRV_CQ_ENUM)cq));
...
//js_test move to _disable.
    ...
    for(int q=0;q<ISP_DRV_CQ_NUM;q++)
    {
        if ( m_pIMemDrv->unmapPhyAddr(&tpipeBufInfo[q]) ) {
            ISP_FUNC_ERR("ERROR:q(%d),m_pIMemDrv->unmapPhyAddr (tpipeBufInfo)",q);
        }
        //LOCAL_PROFILING_LOG_PRINT("m_pIMemDrv->unmapPhyAddr(tpipeBufInfo)");
        //
        if ( m_pIMemDrv->freeVirtBuf(&tpipeBufInfo[q]) ) {
            ISP_FUNC_ERR("ERROR:q(%d),m_pIMemDrv->freeVirtBuf(tpipeBufInfo)",q);
        }
        g_MdpMgrCfgData[q].ispTpipeCfgInfo.drvinfo.tpipeTableVa = NULL;
        g_MdpMgrCfgData[q].ispTpipeCfgInfo.drvinfo.tpipeTablePa = 0;

        ISP_FUNC_INF("[Tpipe free]q(%d),va/pa(0x%08x, 0x%08x)",q,g_MdpMgrCfgData[q].ispTpipeCfgInfo.drvinfo.tpipeTableVa,g_MdpMgrCfgData[q].ispTpipeCfgInfo.drvinfo.tpipeTablePa);
    }
...
MINT32 cq
    ...
    switch(cq) {
        case CAM_ISP_CQ0:
            ...
            {
                ISP_IOCTL_WRITE_REG(m_pPhyIspDrv_bak, m_pPhyIspDrv_bak->getRegAddrMap(),CAM_CTL_CQ0_BASEADDR, (unsigned long)m_pPhyIspDrv_bak->getCQDescBufPhyAddr((ISP_DRV_CQ_ENUM)cq));
            }
            else
            {
                ISP_WRITE_REG(m_pPhyIspReg_bak,CAM_CTL_CQ0_BASEADDR, (unsigned long)m_pPhyIspDrv_bak->getCQDescBufPhyAddr((ISP_DRV_CQ_ENUM)cq));
            }
            //
        ...
        case CAM_ISP_CQ0B:
            if(m_pPhyIspDrv_bak->IsReadOnlyMode())
            {
                ISP_IOCTL_WRITE_REG(m_pPhyIspDrv_bak,m_pPhyIspDrv_bak->getRegAddrMap(),CAM_CTL_CQ0B_BASEADDR, (unsigned long)m_pPhyIspDrv_bak->getCQDescBufPhyAddr((ISP_DRV_CQ_ENUM)cq));
            }
            else
            {
                ISP_WRITE_REG(m_pPhyIspReg_bak,CAM_CTL_CQ0B_BASEADDR, (unsigned long)m_pPhyIspDrv_bak->getCQDescBufPhyAddr((ISP_DRV_CQ_ENUM)cq));
            }
            //
        ...
        case CAM_ISP_CQ0C:
            if(m_pPhyIspDrv_bak->IsReadOnlyMode())
            {
                ISP_IOCTL_WRITE_REG(m_pPhyIspDrv_bak,m_pPhyIspDrv_bak->getRegAddrMap(),CAM_CTL_CQ0C_BASEADDR, (unsigned long)m_pPhyIspDrv_bak->getCQDescBufPhyAddr((ISP_DRV_CQ_ENUM)cq));
            }
            else
            {
                ISP_WRITE_REG(m_pPhyIspReg_bak,CAM_CTL_CQ0C_BASEADDR, (unsigned long)m_pPhyIspDrv_bak->getCQDescBufPhyAddr((ISP_DRV_CQ_ENUM)cq));
            }
            //
            //m_pIspDrv->setCQTriggerMode((ISP_DRV_CQ_ENUM)cq,CQ_SINGLE_EVENT_TRIGGER,CQ_TRIG_BY_PASS1_DONE);
            break;
        case CAM_ISP_CQ1:
            if(m_pPhyIspDrv_bak->IsReadOnlyMode())
            {
                ISP_IOCTL_WRITE_REG(m_pPhyIspDrv_bak,m_pPhyIspDrv_bak->getRegAddrMap(),CAM_CTL_CQ1_BASEADDR, (unsigned long)m_pPhyIspDrv_bak->getCQDescBufPhyAddr((ISP_DRV_CQ_ENUM)cq));
            }
            else
            {
                ISP_WRITE_REG(m_pPhyIspReg_bak,CAM_CTL_CQ1_BASEADDR, (unsigned long)m_pPhyIspDrv_bak->getCQDescBufPhyAddr((ISP_DRV_CQ_ENUM)cq));
            }
            break;
        case CAM_ISP_CQ2:
            if(m_pPhyIspDrv_bak->IsReadOnlyMode())
            {
                ISP_IOCTL_WRITE_REG(m_pPhyIspDrv_bak,m_pPhyIspDrv_bak->getRegAddrMap(),CAM_CTL_CQ2_BASEADDR, (unsigned long)m_pPhyIspDrv_bak->getCQDescBufPhyAddr((ISP_DRV_CQ_ENUM)cq));
            }
            else
            {
                ISP_WRITE_REG(m_pPhyIspReg_bak,CAM_CTL_CQ2_BASEADDR, (unsigned long)m_pPhyIspDrv_bak->getCQDescBufPhyAddr((ISP_DRV_CQ_ENUM)cq));
            }
            break;
        case CAM_ISP_CQ3:
            if(m_pPhyIspDrv_bak->IsReadOnlyMode())
            {
                ISP_IOCTL_WRITE_REG(m_pPhyIspDrv_bak,m_pPhyIspDrv_bak->getRegAddrMap(),CAM_CTL_CQ3_BASEADDR, (unsigned long)m_pPhyIspDrv_bak->getCQDescBufPhyAddr((ISP_DRV_CQ_ENUM)cq));
            }
            else
            {
                ISP_WRITE_REG(m_pPhyIspReg_bak,CAM_CTL_CQ3_BASEADDR, (unsigned long)m_pPhyIspDrv_bak->getCQDescBufPhyAddr((ISP_DRV_CQ_ENUM)cq));
            }
            break;
        default:
...
int DMAI_B::_config( void )
{
    ...
    MUINT32 mdpmgrIdx = 0;
    MUINT32 mdpCropReg=0xC000;
    MUINT32 mdpCropXReg = 0x0;
    MUINT32 mdpCropYReg =0x0;
     ...
     switch(this->id())
     {
        ...
        case ISP_DMA_IMGI:
            if(m_pIspDrv->IsReadOnlyMode())
            {
                ...
                ISP_IOCTL_WRITE_REG(m_pIspDrv, m_pIspDrv->getRegAddrMap(),CAM_IMGI_XSIZE,( (this->dma_cfg.size.w * this->dma_cfg.pixel_byte)>> CAM_ISP_PIXEL_BYTE_FP ) - 1);
            ...
            else
            {
                ...
                ISP_WRITE_REG(m_pIspReg,CAM_IMGI_XSIZE,( (this->dma_cfg.size.w * this->dma_cfg.pixel_byte)>> CAM_ISP_PIXEL_BYTE_FP ) - 1);
...
//TEST 0418               ISP_WRITE_REG(m_pPhyIspReg, CAM_IMGI_CON2, 0x00000000);// TEST_MDP by m_pPhyIspReg
            ...
            ///////********///////
            //for D2, we need update mdp_crop_x and mdp_crop_y in stead of updating in wrapper_isp (cuz wrapper_isp and mdp driver do not separates for v1 and v3)
            //           and only direct link mode is support in cam1
            mdpCropReg=0xC000;
            mdpCropXReg = (this->dma_cfg.crop.x & 0x1fff)|(mdpCropReg)|((this->dma_cfg.crop.x+this->dma_cfg.crop.w-1)<<16 & 0x1FFF0000);
            mdpCropYReg = (this->dma_cfg.crop.y & 0x1fff)|(mdpCropReg)|((this->dma_cfg.crop.y+this->dma_cfg.crop.h-1)<<16 & 0x1FFF0000);
            ISP_FUNC_DBG("mdpCropXReg(0x%08x),mdpCropyReg(0x%08x)",mdpCropXReg,mdpCropYReg);
            if(m_pIspDrv->IsReadOnlyMode())
            {
                ISP_IOCTL_WRITE_REG(m_pIspDrv, m_pIspDrv->getRegAddrMap(),CAM_CTL_CROP_X,mdpCropXReg);
                ISP_IOCTL_WRITE_REG(m_pIspDrv, m_pIspDrv->getRegAddrMap(),CAM_CTL_CROP_Y,mdpCropYReg);
            }
            else
            {
                ISP_WRITE_REG(m_pIspReg,CAM_CTL_CROP_X,mdpCropXReg);
                ISP_WRITE_REG(m_pIspReg,CAM_CTL_CROP_Y,mdpCropYReg);
            }
...
int DMAI_B::_write2CQ( int cq )
{
    ...
    if (0xFF != cq_module_id) {
        ...
        if(cq_module_id == CAM_DMA_IMGI)
        {   //add mdp_crop info
            this->m_pIspDrvShell->cqAddModule((ISP_DRV_CQ_ENUM)this->CQ,(CAM_MODULE_ENUM)CAM_ISP_MDP_CROP);
        }
    }
     ...
     switch(this->id())
     {
...
#elif 1
                ISP_IOCTL_WRITE_REG(m_pIspDrv, m_pIspDrv->getRegAddrMap(), CAM_IMGO_CON, 0x08100850);  // ultra-
                ISP_IOCTL_WRITE_REG(m_pIspDrv, m_pIspDrv->getRegAddrMap(), CAM_IMGO_CON2, 0x00100800);
#endif
...
#elif 1
                ISP_WRITE_REG(m_pIspReg, CAM_IMGO_CON, 0x08100850); // ultra-
                ISP_WRITE_REG(m_pIspReg, CAM_IMGO_CON2, 0x00100800);
#endif
        ...
        case ISP_DMA_IMG2O:
            if(m_pIspDrv->IsReadOnlyMode())
            {
                ...
                ISP_IOCTL_WRITE_REG(m_pIspDrv, m_pIspDrv->getRegAddrMap(), CAM_IMG2O_CON, 0x08100840);  // ultra-high for pass1, not for 3DNR
                ISP_IOCTL_WRITE_REG(m_pIspDrv, m_pIspDrv->getRegAddrMap(), CAM_IMG2O_CON2, 0x00100800);
            ...
            else
            {
                ...
                ISP_WRITE_REG(m_pIspReg, CAM_IMG2O_CON, 0x08100840); //ultra-high for pass1, not for 3DNR
                ISP_WRITE_REG(m_pIspReg, CAM_IMG2O_CON2, 0x00100800);
...
#define CAM_ISP_FRAME_DONE_SEL_BIT 0x1   //bit 23 in 401c should be high for pass2b done event to GCE(66),
...
unsigned int turnEn1Tag=0x00, turnEn2Tag=0x00, turnDmaTag=0x00; //unable to touch sn
    ...
    ISP_FUNC_INF("path(%d),CQ(%d),en1(0x%08x),en2(0x%08x),dma(0x%08x),fmt(0x%08x),ctl(0x%08x),tcm_en(0x%08x),isIspOn(0x%x),muxS2(0x%x)", \
                                                          this->path, \
                                                          this->CQ, \
                                                          this->en_Top.enable1, \
                                                          this->en_Top.enable2, \
...
//            this->tcm_en |= ((turningEn1&CAM_CTL_EN1_LSC_EN)?0x00800000:0); // TEST MDP
            ...
            ISP_FUNC_DBG("cq(%d),turEn1(0x%08x),turEn2(0x%08x),turDma(0x%08x),dmaMerg(0x%08x),tcm(0x%08x)\n",this->CQ,turningEn1,turningEn2,turningDma,dmaMerg,this->tcm_en);
        }
        else
        {
            applyTurn = 0;
...
//TEST 0418            ISP_WRITE_REG(m_pPhyIspReg ,CAM_CTL_TILE, 0x7A03001E);  // TEST_MDP // hardware do not read both tpipe width and height
        ...
        if(m_pIspDrv->IsReadOnlyMode())
        {
            ...
            ISP_FUNC_DBG("[Pass2]0 AppTurn(%d),EN1:Set(0x%08x),Clr(0x%08x),En2:Set(0x%08x),Clr(0x%08x),DMA:Set(0x%08x),Clr(0x%08x)\n", \
        ...
        else
        {
            ...
            ISP_FUNC_DBG("[Pass2]AppTurn(%d) EN1-Set(0x%x),Clr(0x%x)-En2:Set(0x%x),Clr(0x%x)-DMA:Set(0x%x),Clr(0x%x)\n", \
    ...
    if(m_pIspDrv->IsReadOnlyMode())
    {
        ...
        //isp_frame_done_sel should be set as 1 for isp send pass2b done signal to GCE event(66) per tile(when irq mode=2)
        //and pass2a no need this
        //0: isp_frame_done means all TPIPE are done; isp_frame_done2 is always 0.
        //1: isp_frame_done means cq1 TPIPE is done;   isp_frame_done2 means cq2 TPIPE is done.
        //ISP_IOCTL_WRITE_BITS(m_pIspDrv, m_pIspDrv->getRegAddrMap(), CAM_CTL_PIX_ID_SET, ISP_FRAME_DONE_SEL_SET, CAM_ISP_FRAME_DONE_SEL_BIT);
        ...
        ISP_FUNC_DBG("1 CAM_CTL_PIX_ID_SET(0x%8x),pix_id(0x%8x),CAM_CTL_PIX_ID_CLR(0x%8x)",ISP_IOCTL_READ_REG(m_pIspDrv, m_pIspDrv->getRegAddrMap(),  CAM_CTL_PIX_ID_SET),\
            pix_id,ISP_IOCTL_READ_REG(m_pIspDrv, m_pIspDrv->getRegAddrMap(),  CAM_CTL_PIX_ID_CLR));
    }
    else
    {
        ...
        //and pass2a no need this
        //0: isp_frame_done means all TPIPE are done; isp_frame_done2 is always 0.
        //1: isp_frame_done means cq1 TPIPE is done;   isp_frame_done2 means cq2 TPIPE is don
        //ISP_WRITE_BITS(m_pIspReg, CAM_CTL_PIX_ID_SET, ISP_FRAME_DONE_SEL_SET, CAM_ISP_FRAME_DONE_SEL_BIT);
        ...
        ISP_FUNC_DBG("2 CAM_CTL_PIX_ID_SET(0x%8x),pix_id(0x%8x),CAM_CTL_PIX_ID_CLR(0x%8x)",ISP_READ_REG(m_pIspReg,  CAM_CTL_PIX_ID_SET),\
            pix_id,ISP_IOCTL_READ_REG(m_pIspDrv, m_pIspDrv->getRegAddrMap(),  CAM_CTL_PIX_ID_CLR));
    }
    //
...
int ISP_TOP_CTRL::_enable( void* pParam  )
{
    ...
    if( CAM_ISP_PASS1_START == (*start) )
    {
        ...
        ISP_FUNC_INF("ctrl(%d),id(%d),ptr(0x%p)",buf_ctrl.ctrl,buf_ctrl.buf_id,buf_ctrl.pExtend);
        ...
        if( 1 < size )
        {
            unsigned long cq0cDescBufAddr = (unsigned long)this->m_pIspDrvShell->getPhyIspDrv()->getCQDescBufPhyAddr(ISP_DRV_CQ0C);
            ISP_FUNC_DBG("ENABLE CAM_ISP_CQ0C(0x%p)",cq0cDescBufAddr + sizeof(CQ_RING_CMD_ST));
            if(m_pPhyIspDrv->IsReadOnlyMode())
            {
                ISP_IOCTL_WRITE_REG(m_pPhyIspDrv, m_pPhyIspDrv->getRegAddrMap(),CAM_CTL_CQ0C_BASEADDR,
                                (unsigned long)(cq0cDescBufAddr + sizeof(CQ_RING_CMD_ST) ));
            }
            else
            {
        ...
        {
            ISP_FUNC_DBG("ENABLE CAM_ISP_CQ0C(0x%p)",this->m_pIspDrvShell->getPhyIspDrv()->getCQDescBufPhyAddr(ISP_DRV_CQ0C));
            if(m_pPhyIspDrv->IsReadOnlyMode())
            {
                ISP_IOCTL_WRITE_REG(m_pPhyIspDrv,m_pPhyIspDrv->getRegAddrMap(),CAM_CTL_CQ0C_BASEADDR,
                                (unsigned long)( this->m_pIspDrvShell->getPhyIspDrv()->getCQDescBufPhyAddr(ISP_DRV_CQ0C) ));
            }
            else
            {
                ISP_WRITE_REG(this->m_pPhyIspReg,CAM_CTL_CQ0C_BASEADDR,
    ...
    ISP_FUNC_DBG("path(%d),start(%d),SCIO(%d),En1(0x%x),En2(0x%x),Dma(0x%x)",
    this->path, \
    *((MUINT32*)pParam), \
...
int ISP_TOP_CTRL::_disable( void )
{
    ...
    {
        //do not need this due to mdp control flow is changed
        #if 0
    ...
    for ( int rt_dma =_imgi_; rt_dma < _rt_dma_max_ ;rt_dma++ )
    {
        ...
        if ((_imgo_==rt_dma) || (_img2o_==rt_dma))
        {
            ...
            buf_ctrl.data_ptr = (ISP_RT_BUF_INFO_STRUCT *)&dummy;
...
int CAM_TDRI_PIPE::_config( void )
{
    ...
    if ( this->enTdri )
    {
        ...
        else // Add
        {
            ...
            ///////////////////////////////////////////////////////////////////////////////////////////
            //extra information for mdp
            g_MdpMgrCfgData[g_MdpMgrCqIdx].ispTpipeCfgInfo.drvinfo.DesCqPa = (unsigned long)(this->m_pIspDrvShell->getPhyIspDrv()->getCQDescBufPhyAddr((ISP_DRV_CQ_ENUM)(g_MdpMgrCqIdx)));
            g_MdpMgrCfgData[g_MdpMgrCqIdx].ispTpipeCfgInfo.drvinfo.DesCqVa = (unsigned int*)(this->m_pIspDrvShell->getPhyIspDrv()->getCQDescBufVirAddr((ISP_DRV_CQ_ENUM)(g_MdpMgrCqIdx)));
            g_MdpMgrCfgData[g_MdpMgrCqIdx].ispTpipeCfgInfo.drvinfo.VirCqVa = (unsigned int*)(this->m_pIspDrvShell->getPhyIspDrv()->getCQVirBufVirAddr((ISP_DRV_CQ_ENUM)(g_MdpMgrCqIdx)));
            //tile buffer for mdp mgr strcture
            g_MdpMgrCfgData[g_MdpMgrCqIdx].ispTpipeCfgInfo.drvinfo.tpipeTableVa = (MUINT32*)( (this->m_pIspDrvShell->tpipeBufInfo[g_MdpMgrCqIdx].virtAddr+7) & (~7) );
            g_MdpMgrCfgData[g_MdpMgrCqIdx].ispTpipeCfgInfo.drvinfo.tpipeTablePa =  ( (this->m_pIspDrvShell->tpipeBufInfo[g_MdpMgrCqIdx].phyAddr+7) & (~7) );
            g_MdpMgrCfgData[g_MdpMgrCqIdx].ispTpipeCfgInfo.top.mdp_crop_en=tdri.top.mdp_crop_en;
            g_MdpMgrCfgData[g_MdpMgrCqIdx].ispTpipeCfgInfo.top.mdp_sel=tdri.top.mdp_sel;
            g_MdpMgrCfgData[g_MdpMgrCqIdx].ispTpipeCfgInfo.top.rsp_en=tdri.top.mdp_crop_en;
            g_MdpMgrCfgData[g_MdpMgrCqIdx].ispTpipeCfgInfo.sw.log_en=1;
            g_MdpMgrCfgData[g_MdpMgrCqIdx].ispTpipeCfgInfo.sw.tpipe_irq_mode=tdri.tdriPerformCfg.irqMode;
            g_MdpMgrCfgData[g_MdpMgrCqIdx].ispTpipeCfgInfo.sw.tpipe_width=tdri.tdriPerformCfg.tpipeWidth;
            g_MdpMgrCfgData[g_MdpMgrCqIdx].ispTpipeCfgInfo.sw.tpipe_height=tdri.tdriPerformCfg.tpipeHeight;
            g_MdpMgrCfgData[g_MdpMgrCqIdx].ispTpipeCfgInfo.imgo.imgo_crop_en=tdri.imgo.imgo_crop_en;
            g_MdpMgrCfgData[g_MdpMgrCqIdx].ispTpipeCfgInfo.imgo.imgo_mux=tdri.imgo.imgo_mux;
            g_MdpMgrCfgData[g_MdpMgrCqIdx].ispTpipeCfgInfo.imgo.imgo_mux_en=tdri.imgo.imgo_mux_en;
            g_MdpMgrCfgData[g_MdpMgrCqIdx].ispTpipeCfgInfo.imgo.imgo_stride=tdri.imgo.imgo_stride;
            g_MdpMgrCfgData[g_MdpMgrCqIdx].ispTpipeCfgInfo.imgo.imgo_xoffset=tdri.imgo.imgo_xoffset;
            g_MdpMgrCfgData[g_MdpMgrCqIdx].ispTpipeCfgInfo.imgo.imgo_xsize=tdri.imgo.imgo_xsize;
            g_MdpMgrCfgData[g_MdpMgrCqIdx].ispTpipeCfgInfo.imgo.imgo_yoffset=tdri.imgo.imgo_yoffset;
            g_MdpMgrCfgData[g_MdpMgrCqIdx].ispTpipeCfgInfo.imgo.imgo_ysize=tdri.imgo.imgo_ysize;
            g_MdpMgrCfgData[g_MdpMgrCqIdx].ispTpipeCfgInfo.img2o.img2o_crop_en=tdri.img2o.img2o_crop_en;
            g_MdpMgrCfgData[g_MdpMgrCqIdx].ispTpipeCfgInfo.img2o.img2o_mux=tdri.img2o.img2o_mux;
            g_MdpMgrCfgData[g_MdpMgrCqIdx].ispTpipeCfgInfo.img2o.img2o_mux_en=tdri.img2o.img2o_mux_en;
            g_MdpMgrCfgData[g_MdpMgrCqIdx].ispTpipeCfgInfo.img2o.img2o_stride=tdri.img2o.img2o_stride;
            g_MdpMgrCfgData[g_MdpMgrCqIdx].ispTpipeCfgInfo.img2o.img2o_xoffset=tdri.img2o.img2o_xoffset;
            g_MdpMgrCfgData[g_MdpMgrCqIdx].ispTpipeCfgInfo.img2o.img2o_xsize=tdri.img2o.img2o_xsize;
            g_MdpMgrCfgData[g_MdpMgrCqIdx].ispTpipeCfgInfo.img2o.img2o_yoffset=tdri.img2o.img2o_yoffset;
            g_MdpMgrCfgData[g_MdpMgrCqIdx].ispTpipeCfgInfo.img2o.img2o_ysize=tdri.img2o.img2o_ysize;

            ISP_FUNC_DBG("g_MdpMgrCqIdx(%d),top.mdp_crop_en/rsp_en(%d/%d),tpipe_irq_mode(%d)",g_MdpMgrCqIdx,\
                    g_MdpMgrCfgData[g_MdpMgrCqIdx].ispTpipeCfgInfo.top.mdp_crop_en,g_MdpMgrCfgData[g_MdpMgrCqIdx].ispTpipeCfgInfo.top.rsp_en,\
                    g_MdpMgrCfgData[g_MdpMgrCqIdx].ispTpipeCfgInfo.sw.tpipe_irq_mode);
...
MINT32 ISP_BUF_CTRL::enqueueHwBuf( MUINT32 dmaChannel, stISP_BUF_INFO bufInfo )
{
    ...
    ISP_FUNC_DBG("dma(%d),id(%d),size(0x%x),VA(0x%lx),PA(0x%x),S/C(%d/%d)",
                rt_dma,
                buf_info.memID,
                buf_info.size,
                buf_info.virtAddr,
...
//            ISP_FUNC_DBG("Keep going first wo mapphy error");
    ...
    if(ISP_PASS1 == this->path)
    {
        ...
        ISP_FUNC_DBG("rt_buf_info.ID(%d)/size(0x%x)/vAddr(0x%lx)/pAddr(0x%x)",rt_buf_info.memID,rt_buf_info.size,rt_buf_info.base_vAddr,rt_buf_info.base_pAddr);
        ...
        buf_ctrl.buf_id = (_isp_dma_enum_)rt_dma;
        buf_ctrl.data_ptr = (ISP_RT_BUF_INFO_STRUCT *)&rt_buf_info;
        buf_ctrl.ex_data_ptr = (ISP_RT_BUF_INFO_STRUCT *)0;
        ISP_FUNC_DBG("[rtbc][ENQUQ]+(%d)/id(%d)/ptr(0x%p)",buf_ctrl.ctrl,buf_ctrl.buf_id,buf_ctrl.data_ptr);
        ...
        if( NULL != bufInfo.next )
        {
            ...
            ISP_FUNC_INF("exchange 1st buf. by 2nd buf. and enque it.ID(%d)/size(0x%x)/vAddr(0x%lx)/pAddr(0x%x)",ex_rt_buf_info.memID,ex_rt_buf_info.size,ex_rt_buf_info.base_vAddr,ex_rt_buf_info.base_pAddr);
            ...
            buf_ctrl.ex_data_ptr = (ISP_RT_BUF_INFO_STRUCT *)&ex_rt_buf_info;

            ISP_FUNC_INF("[rtbc][ENQUQ]+(%d)/id(%d)/ptr(0x%p)/ex_ptr(0x%p)",buf_ctrl.ctrl,buf_ctrl.buf_id,buf_ctrl.data_ptr,buf_ctrl.ex_data_ptr);
        }
...
MINT32 ISP_BUF_CTRL::dequeueHwBuf( MUINT32 dmaChannel )
{
    ...
    if( ISP_PASS1 == this->path )
    {
        if( _imgo_ == rt_dma || _img2o_ == rt_dma )
        {
            ...
            {
                ISP_FUNC_DBG("i(%d),dma(%d),id(0x%x),size(0x%x),VA(0x%lx),PA(0x%x),size(%d)",i,rt_dma,deque_buf.data[i].memID,deque_buf.data[i].size,deque_buf.data[i].base_vAddr,deque_buf.data[i].base_pAddr,m_swbufL[rt_dma].bufInfoList.size());
        ...
        while(count--)
        {
            ...
            if(m_hwbufL[rt_dma].bufInfoList.size() && ISP_PASS2 == this->path)
            {
                ...
                if(rt_dma==_dispo_)
                {
                    ...
                    {
                        ISP_FUNC_ERR("CQ1 g_pMdpMgr->dequeueBuf for dispo fail");
                        return -1;
                    }
                }
                ...
                if(rt_dma==_vido_)
                {
                    ...
                    {
                        ISP_FUNC_ERR("CQ1 g_pMdpMgr->dequeueBuf for vido fail");
                        return -1;
                    }
                }
                ...
                if(rt_dma==_imgi_)
                {
                    ...
                    {
                        ISP_FUNC_ERR("CQ1 g_pMdpMgr->dequeueBuf for src fail");
                        return -1;
                    }

                     //add to call dequeFrameEnd
                    if(MDPMGR_NO_ERROR != g_pMdpMgr->dequeueFrameEnd(g_MdpMgrCfgData[CAM_ISP_CQ1]))
                    {
                        ISP_FUNC_ERR("CQ1 g_pMdpMgr->dequeueFrameEnd fail");
                        return -1;
                    }
                }
            ...
            else if(ISP_PASS2B == this->path)
            {
                ...
                if(rt_dma==_dispo_)
                {
                    ...
                    {
                        ISP_FUNC_ERR("CQ2 g_pMdpMgr->dequeueBuf for dispo fail");
                        return -1;
                    }
                ...
                if(rt_dma==_vido_)
                {
                    ...
                    {
                        ISP_FUNC_ERR("CQ2 g_pMdpMgr->dequeueBuf for vido fail");
                        return -1;
                    }
                ...
                if(rt_dma==_imgi_)
                {
                    ...
                    {
                        ISP_FUNC_ERR("CQ2 g_pMdpMgr->dequeueBuf for src fail");
                        return -1;
                    }

                    //add to call dequeFrameEnd
                    if(MDPMGR_NO_ERROR != g_pMdpMgr->dequeueFrameEnd(g_MdpMgrCfgData[CAM_ISP_CQ2]))
                    {
                        ISP_FUNC_ERR("CQ2 g_pMdpMgr->dequeueFrameEnd fail");
                        return -1;
                    }
    ...
    ISP_FUNC_DBG("path(%d),start(NA),SCIO(NA),En1(0x%x),En2(0x%x),En2(0x%x),fmt_sel(0x%x),ctl_sel(0x%x),tcm_en(0x%x),mux_sel(0x%x),mux_sel2(0x%x),mux_conf(0x%x)",this->path,
    this->m_pIspDrvShell->getPhyIspDrv()->readReg(0x00004004),
    this->m_pIspDrvShell->getPhyIspDrv()->readReg(0x00004008),
    this->m_pIspDrvShell->getPhyIspDrv()->readReg(0x0000400C),
...
MINT32 ISP_BUF_CTRL::dequeueSwBuf( MUINT32 dmaChannel ,stISP_FILLED_BUF_LIST& bufList )
{
    ...
    while ( m_swbufL[rt_dma].bufInfoList.size() )
    {
        ...
        //
        ISP_FUNC_DBG("05_dma(%d)/memID(0x%x)/size(%d)/vaddr(0x%lx)/paddr(0x%x)",rt_dma,
...
MUINT32 ISP_BUF_CTRL::getCurrHwBuf( MUINT32 dmaChannel )
{
    ...
    ISP_FUNC_DBG("dma:(%d)/memID(0x%x)/vAddr:(0x%lx)/pAddr:(0x%x) ",rt_dma, \
    ...
    //revise, cause we set imgi base address by isp CQ, not by mdp
    if(m_hwbufL[rt_dma].bufInfoList.size()>1)
    {
        if(m_hwbufL[rt_dma].bufInfoList.size()==2)
        {   //max should be two.
             //in the case, the contents should be vss buffer+prv buffer, and user are setting current buffer for prv cause vss buffer has not been dequed
             //otherwise, prv buffer only
             list<stISP_BUF_INFO>::iterator it = m_hwbufL[rt_dma].bufInfoList.begin();
             it++;
             ISP_FUNC_INF("set prv buffer while vss buffer has not beed dequed, get prv (%d)_pa(0x%x)",rt_dma,(*it).base_pAddr);
             return ((*it).base_pAddr);
        }
        else
        {
            ISP_FUNC_ERR("too many imgi buffers (%d)",m_hwbufL[rt_dma].bufInfoList.size());
            return -1;
        }
    }
    else
...
debugPrint( MUINT32 dmaChannel )
{
    ...
    for ( it = m_hwbufL[rt_dma].bufInfoList.begin(); it != m_hwbufL[rt_dma].bufInfoList.end(); it++ ) {
        ISP_FUNC_DBG("m_hwbufL[%d].base_vAddr:[0x%lx]/base_pAddr:[0x%x] ",rt_dma,it->base_vAddr,it->base_pAddr);
    }
    //
...
{
    IspEventThreadImp(): mInitCount(0) {}
    //
