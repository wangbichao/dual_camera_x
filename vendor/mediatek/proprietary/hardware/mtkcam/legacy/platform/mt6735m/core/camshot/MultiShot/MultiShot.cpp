...
MultiShot(
    ...
    , mu4JpegWriteSize(0)
    , mu4JpegReadySize(0)
    , mu4ThumbnailWriteSize(0)
    , mu4ThumbnailReadySize(0)
    , mYuvReadyBufMtx()
    , mJpegReadyBufMtx()
    , semStartEnd()
    , semJpeg()
...
onCreateThumbnailImage()
{
    ...
    //(3) thumbnail done
    mu4ThumbnailWriteSize = u4ThumbnailSize;
...
initImageCreateThread()
{
    ...
    if  (
        ...
        ||  OK != (status = mpYuvImageCreateThread->run(LOG_TAG))
        )
    {
        MY_LOGE(
...
convertImage(ImgBufInfo const & rSrcImgBufInfo, ImgBufInfo const & rDstImgBufInfo, int rot)
{
    ...
    switch(rSrcImgBufInfo.eImgFmt) {
        case eImgFmt_YV12:
            ...
            src_addr_list[1] = (MUINTPTR)(src_yp + src_ysize * 5 / 4);
            src_addr_list[2] = (MUINTPTR)(src_yp + src_ysize);
    ...
    switch(rSrcImgBufInfo.eImgFmt) {
        case eImgFmt_YV12:
            ...
            dst_addr_list[1] = (MUINTPTR)(dst_yp + dst_ysize * 5 / 4);
            dst_addr_list[2] = (MUINTPTR)(dst_yp + dst_ysize);
