...
uninit()
{
    ...
    if (m_pIspDrvShell->m_pIMemDrv)    // Only do following steps when m_pIspDrvShell->m_pIMemDrv is not NULL (which implicitly imply it's non-GDMA mode.).
    {
        ...
        buf_info.virtAddr = (MUINTPTR)pTdriVir;
        ...
        buf_info.virtAddr = (MUINTPTR)pTpipeConfigVa;
        if ( m_pIspDrvShell->m_pIMemDrv->freeVirtBuf(&buf_info) ) {
            PIPE_ERR("ERROR:m_pIMemDrv->freeVirtBuf");
            return MFALSE;
        }
        ...
        if(tdriRingPhy && pTdriRingVir) {
            ...
            buf_info.virtAddr = (MUINTPTR)pTdriRingVir;
...
*/
    ...
    //
    m_camPass2Param.tdri.memBuf.base_vAddr = (MUINTPTR)pTdriVir;
    ...
    //
    m_camPass2Param.cqi.memBuf.base_vAddr = (MUINTPTR)cq_vir;
    ...
    if (-1 != idx_imgi ) {
        ...
        if(m_camPass2Param.imgi.ring_en) {
            ...
            m_camPass2Param.tdri.memBuf.base_vAddr = (MUINTPTR)pTdriRingVir + portInfo_imgi.u4RingTdriBufOffset;
            ...
            if(settingStage == eConfigSettingStage_Init) {
                ...
                m_camPass2Param.capTdriCfg.segSimpleConfBufVa = (MUINTPTR)pTpipeConfigVa;
    ...
    if( ret != 0 ) {
        ...
        if(m_camPass2Param.capTdriCfg.isRunSegment && settingStage == eConfigSettingStage_Init) {
            ...
            MUINTPTR pSetSimpleConfIdxNumVa = (MUINTPTR)portInfo_imgi.u4SegNumVa;
            segmSimpleConfIdxNum = (MUINT32)pSetSimpleConfIdxNumVa;
        }
    }
