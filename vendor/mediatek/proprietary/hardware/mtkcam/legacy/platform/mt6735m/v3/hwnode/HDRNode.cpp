...
#include <mtkcam/feature/hdr/1.0/IHdrProc.h>
#include <mtkcam/feature/hdr/1.0/utils/ImageBufferUtils.h>
...
// TODO: replace MY_LOG with HDR_LOG
...
MVOID HDRNodeImp::onProcessFrame(android::sp<IPipelineFrame> const& pFrame)
{
    ...
    MY_LOGD("mNodeEnqueIndex(%d) frameNo(%u) nodeId(%08" PRIxPTR ")",
    ...
    // image
    {
        ...
        {
            MY_LOGD("ImageInfoIOMapSet(%zu) frameNo(%u) In(%zu) Out(%zu)",
    ...
    // metadata
    {
        ...
        for (size_t i = 0; i < mapSet.size(); i++)
        {
            MY_LOGD("MetaInfoIOMapSet(%zu) frameNo(%u) In(%zu) Out(%zu)",
                    i, FRAME_NO, mapSet[i].vIn.size(), mapSet[i].vOut.size());
    ...
    for (size_t i = 0; i < mvFrameParams[mNodeEnqueIndex].vpIn.size(); i++)
    {
        ...
        {
            MY_LOGD("found full raw at %zu", i);
            fullRawIndex = i;
            break;
        }
    }
...

    {
        ...
        MY_LOGD("Input mPortID %08" PRIxPTR, reinterpret_cast<intptr_t>(&src.mPortID));
    ...
    MY_LOGD("dst mPortID %08" PRIxPTR, reinterpret_cast<intptr_t>(&dst.mPortID));
    MY_LOGD("dst mBuffer %08" PRIxPTR, reinterpret_cast<intptr_t>(&dst.mBuffer));
    MY_LOGD("dst mBuffer VA %08" PRIxPTR, dst.mBuffer != NULL ?
    ...
    MY_LOGD("dst2 mPortID %08" PRIxPTR, reinterpret_cast<intptr_t>(&dst2.mPortID));
    MY_LOGD("dst2 mBuffer %08" PRIxPTR, reinterpret_cast<intptr_t>(&dst2.mBuffer));
    MY_LOGD("dst2 mBuffer VA %08" PRIxPTR, dst2.mBuffer != NULL ?
    ...
    // add input/output frames into HDR proc
    {
        ...
        if (mNodeEnqueIndex == 0)
        {
            ...
            for (size_t i = 0; i < frameParams.vpOut.size(); i++)
            {
                ...
                MY_LOGD("add output frame(%zu) buffer(%p)",
    ...
    if ((mNodeEnqueIndex + 1) == mHDRFrameNum)
    {
        ...
        for (MINT32 i = 0; i < mHDRFrameNum; i++)
        {
            ...
            {
                MY_LOGD("release input image buffer i(%d)j(%zu)", i, j);
            ...
            {
                MY_LOGD("release output image buffer i(%d)j(%zu)", i, j);
            ...
            // apply buffers to be released
            {
                ...
                MY_LOGD("[applyRelease] frameNo(%u) nodeId(%08" PRIxPTR ")", FRAME_NO, getNodeId());
        ...
        MINT32 /*frameIndex*/) const
{
    ...
    for (size_t i = 0; i < imageIOMapSet.size(); i++)
    {
        ...
        MY_LOGD_IF(mLogLevel >= 1, "ImageInfoIOMapSet(%zu) frameNo(%u) In(%zu) Out(%zu)",
    ...
    for (size_t i = 0; i < metaIOMapSet.size(); i++)
    {
        ...
        MY_LOGD_IF(mLogLevel >= 1, "MetaInfoIOMapSet(%zu) frameNo(%u) In(%zu) Out(%zu)",
    ...
    sp<IImageBuffer>& imageBuffer)
{
    ...
    {
        MY_LOGE("    node(%" PRIdPTR ") type(%d) stream buffer(%s): cannot get imageBufferHeap",
    ...
    if (imageBuffer == NULL)
    {
        MY_LOGE("    node(%" PRIdPTR ") type(%d) stream buffer(%s): cannot create imageBuffer",
                getNodeId(), type, streamBuffer->getName());
        return INVALID_OPERATION;
    }
    ...
    MY_LOGD_IF(mLogLevel >= 1, "    streamId(%08" PRIxPTR ") buffer(%p) usage(0x%x) type(0x%x)",
    ...
    IMetadata*& metadata)
{
    ...
    if (metadata == NULL)
    {
        MY_LOGE("      node(%" PRIdPTR ") stream buffer(%s): cannot get app metadata",
    ...
    sp<IImageStreamBuffer> const pStreamBuffer) const
{
    ...
    {
        MY_LOGE("pFrame(%08" PRIxPTR ") or pStreamBuffer(%08" PRIxPTR ") should not be NULL",
...
bool HDRNodeImp::HDRProcThread::threadLoop()
{
    ...
    for (MINT32 i = 0; i < FRAME_NUM; i++)
    {
        ...
        MY_LOGD("alloc input working buffer(%d) addr(%p) width(%d) height(%d) format(0x%x)",
                i, imageBuffer, inputImageWidth, inputImageHeight, inputImageFormat);
    }
