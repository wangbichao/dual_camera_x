...
tsfBatchCap()
{
    ...
    if (fgDumpEn)
    {
        ...
        if (dumpSdblk("/sdcard/tsf0/tsfInput", rLscCfg, reinterpret_cast<MUINT32*>(m_rTsfSetProc.ShadingTbl)) != 0)
            LSC_ERR("dumpSdblk error");
    }
    ...
    tsfSetProcInfo(m_rTsfSetProc, fgDumpEn, "/sdcard/tsf0/tsfAwbStat.bin");
...
tsfRun()
{
    ...
    if (m_rTsfGetProc.TsfState == MTKTSF_STATE_READY)
    {
        ...
        updateLsc();
        IspTuningMgr::getInstance().forceValidate((MINT32)m_eSensorDev);
    }
