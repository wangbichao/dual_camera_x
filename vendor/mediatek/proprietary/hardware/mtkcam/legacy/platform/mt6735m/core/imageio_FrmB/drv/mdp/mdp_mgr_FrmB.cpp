...
MINT32 _startMdp(MDPMGR_DPSTREAM_TYPE dpStreamtype,MDPMGR_CFG_STRUCT_FRMB cfgData,T* pDpStream)
{
    ...
    // format convert
    errMDP = DpColorFmtConvert(cfgData.mdpSrcFmt, &srcFmt);
    ...
    errDP = pDpStream->setSrcConfig(cfgData.mdpSrcW ,cfgData.mdpSrcH, \
    ...
    errDP = pDpStream->queueSrcBuffer((void **)srcVirList,&srcPhyList[0],srcSizeList,cfgData.mdpSrcPlaneNum);
    if(errDP != DP_STATUS_RETURN_SUCCESS)
    {
    ...
    errDP = pDpStream->setSrcCrop(cfgData.srcCropX,
            ...
            switch(cfgData.dstDma[index].capbility)
               {
                ...
                case NSImageio_FrmB::NSIspio_FrmB::EPortCapbility_Disp:
                    ...
                    //
                    errDP = pDpStream->setSharpness(index, 1);
                    if(errDP != DP_STATUS_RETURN_SUCCESS)
                    {
                        LOG_ERR("setSharpness fail(%d)",errDP);
                        return MDPMGR_API_FAIL;
                    }
                    break;
                default:
