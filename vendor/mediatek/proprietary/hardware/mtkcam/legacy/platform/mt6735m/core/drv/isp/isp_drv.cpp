...
=   { {CAM_TOP_CTL,               0x4050, 2  },
      ...
      {CAM_ISP_MDP_CROP,            0x4110,2  },
              {CAM_DMA_TDRI,              0x4208 , 2  }, //remove CAM_REG_TDRI_BASE_ADDR          CAM_TDRI_BASE_ADDR;       // 4204 //              {CAM_DMA_TDRI,              0x4204, 3  }, //SL TEST_MDP
              {CAM_DMA_IMGI,              0x4230, 7  }, //0x4240/*0x4230*/, 3 /*8*/  },//0x4234/*0x4230*/, 6 /*8*/  }, //remove BASE/OFST/XS/YX/RSV from 4230 to 4244 just IMGI_CON.CON2 {CAM_DMA_IMGI,              0x4230, 8  }, //SL TEST_MDP
...
{
    //LOG_DBG("");
    return IspDrvVirImp::getInstance(cq, mpIspVirRegAddrVA[cq]);
}
...
MBOOL IspDrv::cqAddModule(ISP_DRV_CQ_ENUM cq, CAM_MODULE_ENUM moduleId)
{
    ...
    MUINTPTR dummyaddr = (MUINTPTR)((MUINTPTR)mpIspVirRegAddrPA[cq] + gIspCQModuleInfo[moduleId].addr_ofst);

    cmd = (gIspCQModuleInfo[moduleId].addr_ofst&0xffff) | (((gIspCQModuleInfo[moduleId].reg_num - 1) & 0x3ff) << 16) | ((ISP_CQ_WRITE_INST) << 26);

    mpIspCQDescriptorVirt[gIspDrvCqMappinig[cq].descriptorCq][moduleId].v_reg_addr = (MUINT32)dummyaddr & 0xFFFFFFFF;
...
MUINT32 *IspDrv::getCQDescBufPhyAddr(ISP_DRV_CQ_ENUM cq)
{
    ...
    return mpIspCQDescriptorPhy[gIspDrvCqMappinig[cq].descriptorCq];
}

/*******************************************************************************
*
********************************************************************************/
MUINT32 *IspDrv::getCQDescBufVirAddr(ISP_DRV_CQ_ENUM cq)
{
    Mutex::Autolock lock(cqPhyDesLock);

    return (MUINT32*)mpIspCQDescriptorVirt[gIspDrvCqMappinig[cq].descriptorCq];
}

/*******************************************************************************
*
********************************************************************************/
MUINT32 *IspDrv::getCQVirBufVirAddr(ISP_DRV_CQ_ENUM cq)
{
    Mutex::Autolock lock(cqPhyDesLock);

    return mpIspVirRegAddrVA[cq];
}
...
{
    LOG_DBG("cb(0x%x),cookie(0x%x)", (unsigned long) pCameraIOCB, (unsigned long)user);
...
{
    LOG_DBG("updateCqdes(%d) CtlEn1(0x%x) CtlEn2(0x%x) CtlMma(0x%x)\n",tdriMgrInfoCq1.updateCqDescriptorNum,
    ...
    if(tdriMgrInfoCq1.updateCqDescriptorNum)
    {
        ...
        {
            LOG_DBG("tdriMgrInfoCq1.mTurUpdTable[%d]=%d ",i,tdriMgrInfoCq1.mTurUpdTable[i]);
            if(tdriMgrInfoCq1.mTurUpdTable[i] == MTRUE)
            {
                ...
                LOG_DBG("i(%d) cqDescOffset(0x%x) cqDescRegNum(%d) pSrc(0x%x) pDst(0x%x)"
                                ,i,cqDescOffset,cqDescRegNum,(unsigned long)pSrc,(unsigned long)pDst);    //Vent@20121107: Fix build warning: format '%x' expects argument of type 'unsigned int', but argument 8/9 has type 'MUINT32* {aka unsigned int*}' [-Wformat].
...
{
    LOG_DBG("updateCqdes(%d) CtlEn1(0x%x) CtlEn2(0x%x) CtlMma(0x%x)",tdriMgrInfoCq2.updateCqDescriptorNum,
...
*
********************************************************************************/
MUINT32 IspDrv::getTpipeMgrVaCq1(unsigned long *pDescArray, unsigned long *pDescNum, unsigned long *pCtlEn1, unsigned long *pCtlEn2, unsigned long *pCtlDma)
{
    LOG_DBG("+");

    *pDescArray = (unsigned long)((MUINTPTR)tdriMgrInfoCq1.mTurUpdTable);
    *pDescNum = (unsigned long)((MUINTPTR)&tdriMgrInfoCq1.updateCqDescriptorNum);
    *pCtlEn1 = (unsigned long)((MUINTPTR)&tdriMgrInfoCq1.topCtlEn1);
    *pCtlEn2 = (unsigned long)((MUINTPTR)&tdriMgrInfoCq1.topCtlEn2);
    *pCtlDma = (unsigned long)((MUINTPTR)&tdriMgrInfoCq1.topCtlDma);

    LOG_DBG("cq1 mTurUpdTable(0x%lx),updateCqDescriptorNum(0x%lx),topCtlEn1(0x%lx),topCtlEn2(0x%lx),topCtlDma(0x%lx)",\
            (unsigned long)tdriMgrInfoCq1.mTurUpdTable, (unsigned long)&tdriMgrInfoCq1.updateCqDescriptorNum,\
            (unsigned long)&tdriMgrInfoCq1.topCtlEn1,(unsigned long)&tdriMgrInfoCq1.topCtlEn2,(unsigned long)&tdriMgrInfoCq1.topCtlDma);
    return MTRUE;
}
...
*
********************************************************************************/
MUINT32 IspDrv::getTpipeMgrVaCq2(unsigned long *pDescArray, unsigned long *pDescNum, unsigned long *pCtlEn1, unsigned long *pCtlEn2,  unsigned long *pCtlDma)
{
    LOG_DBG("+");

    *pDescArray = (unsigned long)((MUINTPTR)tdriMgrInfoCq2.mTurUpdTable);
    *pDescNum = (unsigned long)((MUINTPTR)&tdriMgrInfoCq2.updateCqDescriptorNum);
    *pCtlEn1 = (unsigned long)((MUINTPTR)&tdriMgrInfoCq2.topCtlEn1);
    *pCtlEn2 = (unsigned long)((MUINTPTR)&tdriMgrInfoCq2.topCtlEn2);
    *pCtlDma = (unsigned long)((MUINTPTR)&tdriMgrInfoCq2.topCtlDma);

    LOG_DBG("cq2 mTurUpdTable(0x%lx),updateCqDescriptorNum(0x%lx),topCtlEn1(0x%lx),topCtlEn2(0x%lx),topCtlDma(0x%lx)",\
            (unsigned long)tdriMgrInfoCq2.mTurUpdTable, (unsigned long)&tdriMgrInfoCq2.updateCqDescriptorNum,\
            (unsigned long)&tdriMgrInfoCq2.topCtlEn1,(unsigned long)&tdriMgrInfoCq2.topCtlEn2,(unsigned long)&tdriMgrInfoCq2.topCtlDma);
...
MBOOL IspDrvImp::init()
{
    ...
    else    // 1st time open success.   // Sometimes GDMA will go this path, too. e.g. File Manager -> Phone Storage -> Photo.
    {
        if(!m_fgIsGdmaMode)    // Normal Mode
        {
            ...
            LOG_DBG("m_RTBufTblSize(0x%x),m_pRTBufTbl(0x%p)",m_RTBufTblSize, m_pRTBufTbl);
    ...
    if(!m_fgIsGdmaMode)    // Normal Mode
    {
        ...
        LOG_DBG("[m_pIMemDrv]:0x%p", m_pIMemDrv);    //Vent@20121107: Fix build warning: format '%x' expects argument of type 'unsigned int', but argument 5 has type 'IMemDrv*' [-Wformat]
        ...
        for(MINT32 i = 0; i < ISP_DRV_CQ_NUM; i++)
        {
            ...
            LOG_INF("cq:[%d],virtIspAddr:virt[0x%p]/phy[0x%p]",i, mpIspVirRegAddrVA[i], mpIspVirRegAddrPA[i] );
        }
        ...
        LOG_VRB("cq(0x%x)(0x%x)(0x%x)(0x%x)(0x%p)(0x%p)",mIspCQDescSize,
        ...
        {
            //gIspCQDescInit[i].v_reg_addr = (MUINT32)&mpIspVirRegAddrPA[0][ISP_DRV_CQ_DUMMY_WR_TOKEN/4];
            gIspCQDescInit[i].v_reg_addr = (MUINT32)dummyaddr & 0xFFFFFFFF;
        }
        ...
        for(MINT32 i = 0; i < ISP_DRV_DESCRIPTOR_CQ_NUM; i++)
        {
            ...
            LOG_INF("cq:[%d],mpIspCQDescriptor:Virt[0x%08x]/Phy[0x%p]",i,mpIspCQDescriptorVirt[i],mpIspCQDescriptorPhy[i]);
        }
...
{
    LOG_DBG("mpIspHwRegAddr(0x%08X)",mpIspHwRegAddr);

    if(mpIspHwRegAddr != NULL)
    {
        return mpIspHwRegAddr;
    }
    else
    {
        return 0;
...
MBOOL IspDrvImp::cqRingBuf(MVOID *pBuf_ctrl)
{
    ...
    LOG_DBG("[rtbc]pcqrtbcring_va(0x%p)/pa(0x%p)",pcqrtbcring_va,pcqrtbcring_pa);
    LOG_DBG("[rtbc]ctrl(%d),dma(%d),or_size(%d),2or_size(%d)",pbuf_ctrl->ctrl,
                                                                pbuf_ctrl->buf_id,
    ...
    switch(pbuf_ctrl->ctrl)
    {
        case ISP_RT_BUF_CTRL_ENQUE:
            ...
            {
                LOG_INF("[rtbc]va(0x%p),pa(0x%p),ctrl(%d),dma(%d),PA(0x%x),or_size(%d),2or_size(%d)", \
                        pcqrtbcring_va, \
                        pcqrtbcring_pa, \
                        pbuf_ctrl->ctrl, \
                        pbuf_ctrl->buf_id, \
                ...
                pcqrtbcring_va->rtbc_ring[i].next_pa = (unsigned long)RTBC_GET_PA_FROM_VA(pcqrtbcring_va->rtbc_ring[i].pNext,pcqrtbcring_va,pcqrtbcring_pa);
                ...
                pcqrtbcring_va->rtbc_ring[i].cq_rtbc.imgo.data_ptr_pa = \
                    (unsigned int)RTBC_GET_PA_FROM_VA(&pcqrtbcring_va->rtbc_ring[i].cq_rtbc.imgo_base_pAddr,pcqrtbcring_va,pcqrtbcring_pa);
                ...
                pcqrtbcring_va->rtbc_ring[i].cq_rtbc.img2o.data_ptr_pa = \
                    (unsigned int)RTBC_GET_PA_FROM_VA(&pcqrtbcring_va->rtbc_ring[i].cq_rtbc.img2o_base_pAddr,pcqrtbcring_va,pcqrtbcring_pa);
                ...
                pcqrtbcring_va->rtbc_ring[i].cq_rtbc.next_cq0ci.data_ptr_pa = \
                    (unsigned int)RTBC_GET_PA_FROM_VA(&pcqrtbcring_va->rtbc_ring[i].next_pa,pcqrtbcring_va,pcqrtbcring_pa);
                ...
                if(i > 0)
                {
                    ...
                    pcqrtbcring_va->rtbc_ring[i-1].cq_rtbc.next_cq0ci.data_ptr_pa = \
                        (unsigned int)RTBC_GET_PA_FROM_VA(&pcqrtbcring_va->rtbc_ring[i-1].next_pa,pcqrtbcring_va,pcqrtbcring_pa);
                }
...

                {
                    ...
                    {
                        LOG_DBG("[rtbc](0x%p)(0x%08x)",pdata+i,pdata[i]);
                    }
                }
            }
            else    //VF is ON
...
{
    LOG_INF("cq(%d),ispVirRegAddr(0x%p)", cq, ispVirRegAddr);
...
{
    LOG_VRB("mpIspVirRegBuffer(0x%p)", mpIspVirRegBuffer);
