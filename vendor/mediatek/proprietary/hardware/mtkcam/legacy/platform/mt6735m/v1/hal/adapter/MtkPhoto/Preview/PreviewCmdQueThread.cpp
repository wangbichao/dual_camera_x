...
start()
{
    ...
    //
    if( ! (ret = mSensorInfo.init(SENSOR_SCENARIO_ID_NORMAL_PREVIEW)))
    {
    ...
    mp3AHal->setSensorMode(SENSOR_SCENARIO_ID_NORMAL_PREVIEW);
    CAM_TRACE_END();
...
delay(EQueryType_T type)
{
    ...
    switch (type)
    {
        case EQueryType_Init:
        {
            ...
            {
                //PTLogStrEx(Event_Hal_Adapter_MtkPhotoPreview_start_stable, CPTFlagSeparator, i, 0, "dropFrame");
                if ( ! dropFrame() )
                {
                    delayCnt++;
                    failCnt++;
...
lbExit:
   ...
   MY_LOGD("P2(0x%X),FC(%d),Time(%3lld/%3lld/%3lld)ms",
            flag,
            mFrameCnt,
            passTime/1000000,
            pass1Time/1000000,
