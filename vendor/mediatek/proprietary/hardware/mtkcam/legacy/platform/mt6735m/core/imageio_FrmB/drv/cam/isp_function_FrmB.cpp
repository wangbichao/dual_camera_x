...
unsigned long IspFunction_B::dec_Id( void )//Decimal ID number
{
    ...
    return (dec_Id>0)?(dec_Id-1):((unsigned long)-1);
}
...
int ISP_TOP_CTRL::checkBusy(  unsigned long* param  )
{
    ...
    return 1; /* for coverity, remove dead code */
}
    ...
    if(this->pP1Tuning){
        ...
        if(_eis_cfg.bEIS_Bypass == 0){
            ...
            if(_eis_cfg.bEIS_EN){
                ...
                if (_targetVirDrv->getIspCQModuleInfo(CAM_ISP_EIS, outInfo) == MFALSE) {
                    ISP_FUNC_ERR("getIspCQModuleInfo failed...\n");
                }
...
int ISP_RAW_PIPE::setP1TuneCfg(MUINT32 mNum, MINT32 senDev)
{
    ...
    //sanford.lin
    if (eIspUpdateTgtDev_Main2== senDev) {
        softScenario = eSoftwareScenario_Main2_Normal_Stream;
    }
    ...
    if ( hrz_cfg.isTwinMode) {
        ...
        return -1;
            ...
            */
            ...
            //
            if (_targetVirDrv == NULL) {
                ISP_FUNC_ERR("_targetVirDrv == NULL\n");
            }
            else {
