...
#define FUNCTION_IN               MY_LOGV("+")
#define FUNCTION_OUT              MY_LOGV("-")
...
public:
    ...
    void                reset()
    {
        MY_LOGD("mpSensorHal=0x%x",mpSensorHal);
        if (mpSensorHal)
        {
            MY_LOGD("mpSensorHal->reset()");
            mpSensorHal->reset();
        }
    }
//++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
    ...
    , miLogLevel(0)
{
    ...
    ::property_get("debug.camera.prvcq.loglevel", cLogLevel, "0");
...
{
    CAM_TRACE_NAME("PrvCQT_DEF::requestExit");
    FUNCTION_IN;
...
{
    CAM_TRACE_NAME("PrvCQT_DEF::postCommand");
    FUNCTION_IN;
    //
    bool ret = true;
    //
...
start()
{
    ...
    #else
    EIspProfile_T eIspProfile = EIspProfile_NormalPreview;
    #endif
    ...
    int32_t eScenarioId = ( mspParamsMgr->getRecordingHint() )
                ? SENSOR_SCENARIO_ID_NORMAL_VIDEO : SENSOR_SCENARIO_ID_NORMAL_PREVIEW;
    #else
    int32_t eScenarioId = SENSOR_SCENARIO_ID_NORMAL_PREVIEW;
    #endif
    ...
    //
    if ( ! (ret = mSensorInfo.init(eScenarioId)))
    {
...
delay(EQueryType_T type)
{
    ...
    switch (type)
    {
        case EQueryType_Init:
        {
            ...
            int failCnt = 0;
            CAM_TRACE_BEGIN("PrvCQT_DEF::delay::init");
        ...
        case EQueryType_Effect:
        {
            int32_t delay3A = mp3AHal->getDelayFrame(type);
            int32_t count = 0;
            CAM_TRACE_BEGIN("PrvCQT_DEF::delay::Effect");
        ...
        case EQueryType_AWB:
        {
            int32_t delay3A = mp3AHal->getDelayFrame(type);
            int32_t count = 0;
            CAM_TRACE_BEGIN("PrvCQT_DEF::delay::AWB");
            for (count; count < delay3A; count++)
            {
...
setParameters()
{
    CAM_TRACE_NAME("PrvCQT_DEF::setParameters");
    FUNCTION_IN;
...
#define UPDATE_PARAMS(param, eMapXXX, key) \
    ...
    //(7) Zoom
    {
        int32_t newParam = getParamsMgr()->getInt(CameraParameters::KEY_ZOOM);
        int32_t oldParam = mCurZoomIdx;
        if (newParam != oldParam)
        {
            mCurZoomIdx = newParam;
            MY_LOGD("zoom idx changed (%d) --> (%d)", oldParam, newParam);
        }
    }
...
getZoomValue()
{
    ...
    if( ! mvZoomIdx.empty() )
    {
        MY_LOGD_IF(miLogLevel, "Size(%d)", mvZoomIdx.size());
        zoomIdx = *mvZoomIdx.begin();
        mvZoomIdx.erase(mvZoomIdx.begin());

        MY_LOGD_IF(miLogLevel, "Idx(%d)", zoomIdx);
    }
    else
    {
...
stop()
{
    CAM_TRACE_NAME("PrvCQT_DEF::stop");
    FUNCTION_IN;
    //
    bool ret = true;
...
precap()
{
    CAM_TRACE_NAME("PrvCQT_DEF::precap");
    FUNCTION_IN;
...
update()
{
    ...
    do{
        ...
        MY_LOGD_IF(miLogLevel, "frameCnt(%d)", mFrameCnt);
        mFrameCnt++;
...
updateOne()
{
    CAM_TRACE_NAME("PrvCQT_DEF::updateOne");
...
lbExit:
    ...
    if(miLogLevel||(passTime/1000000)>1000)
    {
        MY_LOGI("Time(%3lld/%3lld/%3lld/%3lld)ms",
            passTime/1000000,
            pass1Time/1000000,
            pass2Time/1000000,
            vssTime/1000000);
...
updateZoom(vector<IhwScenario::PortImgInfo> &rImgIn)
{
    ...
    if(mspParamsMgr->getVideoStabilization())
    {
        mpEisHal->getEISResult(
                    ...
                    EisTarWidth,
                    EisTarHeight);
        MY_LOGD_IF(miLogLevel, "EIS:X(%d/%10d),Y(%d/%10d),S(%d/%d)",
                    EisXInt, EisXFlt,
                    EisYInt, EisYFlt,
        ...
        Crop.floatX = EisXFlt;
        Crop.floatY = EisYFlt;
        MY_LOGD_IF(miLogLevel, "Crop(EIS):X(%d/%10d),Y(%d/%10d),S(%d/%d)",
                    Crop.x, Crop.floatX,
                    Crop.y, Crop.floatY,
                    Crop.w, Crop.h);
    }
...
doCrop(IhwScenario::Rect_t const &rSrc, IhwScenario::Rect_t const &rDst, uint32_t ratio)
{
    ...
    MY_LOGD_IF(miLogLevel, "S(%d/%d),D(%d/%d),Z(%d),C(%d,%d,%d,%d)",
                rSrc.w, rSrc.h,
                rDst.w, rDst.h,
                ratio,
    ...
    uint32_t filenum)
{
    for (MUINT32 i = 0; i < src.size(); i++)
    {
        ...
        MY_LOGD("addr: 0x%x, size: %d, time: %f",
