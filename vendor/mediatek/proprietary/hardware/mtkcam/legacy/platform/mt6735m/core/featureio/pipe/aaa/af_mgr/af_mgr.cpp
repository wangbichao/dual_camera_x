...
MRESULT AfMgr::SingleAF_CallbackNotify()
{
    ...
    {
        MY_LOG("[%s()] AutoFocus %d\n", __FUNCTION__, m_i4AutoFocus);
        if(m_i4AutoFocus != FALSE)
        {
            m_pAFCallBack->doNotifyCb(I3ACallBack::eID_NOTIFY_AF_FOCUSED, m_sAFOutput.i4IsFocused, 0, 0);
            m_i4AutoFocus = FALSE;
            MY_LOG("[%s()] autofocus callback in auto mode %d", __FUNCTION__, m_eLIB3A_AFMode);
        }
    }
...
MVOID AfMgr::setCurFrmNum(MUINT32 frm_num)
{
    ...
    MY_LOG("[m_cur_frm_num] %d", m_cur_frm_num);
}
