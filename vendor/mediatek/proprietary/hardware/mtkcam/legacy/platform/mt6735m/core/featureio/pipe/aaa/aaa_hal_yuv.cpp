...
init(MINT32 i4SensorDev)
{
    ...
    if (!m_pSensorHal)   {
        ...
        m_pSensorHal = pIHalSensorList->createSensor("aaa_hal_yuv", i4SensorDev);
...
MBOOL Hal3AYuv::getSupportedParams(FeatureParam_T &rFeatureParam)
{
    ...
    m_pSensorHal->sendCommand(m_i4SensorDev,SENSOR_CMD_GET_YUV_AE_AWB_LOCK,(MUINTPTR)&ae_lock,(MUINTPTR)&awb_lock,0);
    ...
    m_pSensorHal->sendCommand(m_i4SensorDev,SENSOR_CMD_GET_YUV_AF_MAX_NUM_FOCUS_AREAS,(MUINTPTR)&max_focus,0,0);
    m_pSensorHal->sendCommand(m_i4SensorDev,SENSOR_CMD_GET_YUV_AE_MAX_NUM_METERING_AREAS,(MUINTPTR)&max_meter,0,0);
...
MINT32 Hal3AYuv::getDelayFrame(EQueryType_T const eQueryType) const
{
    ...
    m_pSensorHal->sendCommand(m_i4SensorDev,SENSOR_CMD_GET_YUV_DELAY_INFO,(MUINTPTR)&pDelay,0,0);
...
MINT32 Hal3AYuv::getCaptureParams(MINT8 index, MINT32 i4EVidx, CaptureParam_T &a_rCaptureInfo)
{
    ...
    if (m_rParam.u4CamMode == eAppMode_FactoryMode){
         ...
         m_pSensorHal->sendCommand(m_i4SensorDev,SENSOR_CMD_SET_YUV_AUTOTEST,(MUINTPTR)&yuvCmd,(MUINTPTR)&yuvParam,0);
