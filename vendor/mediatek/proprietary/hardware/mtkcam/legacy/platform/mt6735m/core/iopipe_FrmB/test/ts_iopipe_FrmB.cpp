    ...
    else if( mSensorInfo.sensorType == SENSOR_TYPE_RAW )
    {
        if(port == PORT_IMGO) //imgo
        {
            switch( bitDepth )
            {
                ...
                default:
                printf("imgo bitdepth not supported, 0x%x\n", bitDepth);
                goto lbExit;
                break;
            }
        }
        else // img2o
        {
            switch( bitDepth)
            {
                ...
                case_Format( 14, eImgFmt_BAYER14, pFmt);
                default:
                printf("img2o bitdepth not supported, 0x%x\n", bitDepth);
                goto lbExit;
                break;
            }
        }
...
int main_iopipe(int testcaseType,int testcaseNum)
{
    ...
    NSImageio_FrmB::NSIspio_FrmB::E_ISP_PIXMODE e_PixMode = NSImageio_FrmB::NSIspio_FrmB::ISP_QUERY_1_PIX_MODE;
