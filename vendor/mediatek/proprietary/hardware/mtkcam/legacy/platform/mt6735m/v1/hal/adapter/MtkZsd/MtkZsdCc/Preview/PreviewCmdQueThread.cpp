...
{
    CAM_TRACE_NAME("PrvCQT_ZC::requestExit");
    FUNCTION_IN;
...
{
    CAM_TRACE_NAME("PrvCQT_ZC::postCommand");
    FUNCTION_IN;
    //
    bool ret = true;
    //
...
start()
{
    CAM_TRACE_NAME("PrvCQT_ZC::start");
    FUNCTION_IN;
    //
    bool ret = false;
...
delay(EQueryType_T type)
{
    ...
    switch (type)
    {
        case EQueryType_Init:
        {
            ...
            int failCnt = 0;
            CAM_TRACE_BEGIN("PrvCQT_ZC::delay::init");
        ...
        case EQueryType_Effect:
        {
            int32_t delay3A = mp3AHal->getDelayFrame(type);
            int32_t count = 0;
            CAM_TRACE_BEGIN("PrvCQT_ZC::delay::Effect");
        ...
        case EQueryType_AWB:
        {
            int32_t delay3A = mp3AHal->getDelayFrame(type);
            int32_t count = 0;
            CAM_TRACE_BEGIN("PrvCQT_ZC::delay::AWB");
            for (count; count < delay3A; count++)
            {
...
setParameters()
{
    CAM_TRACE_NAME("PrvCQT_ZC::setParameters");
    FUNCTION_IN;
...
precap()
{
    CAM_TRACE_NAME("PrvCQT_ZC::precap");
    FUNCTION_IN;
    uint32_t bFlashOn = 0;
...
updateOne()
{
    ...
    CPTLogStrEx(Event_Hal_Adapter_MtkZsdPreview_proc, CPTFlagSeparator,(MUINTPTR)capNode.mainImgNode->getVirAddr(),0, "dequeProvider");
/*
    updateCfg();
