...
#define REG_ADDR(reg)\
    ((MUINTPTR)(&getIspReg(ISPDRV_MODE_ISP)->reg) - (MUINTPTR)getIspReg(ISPDRV_MODE_ISP))
...
public: // Interfaces.
    ...
    MBOOL isEnable(void);
    MVOID setLogOnOff(MBOOL fgOnOff);
...
public: // Interfaces.
    ...
    MBOOL setIspPregain();
    MVOID setLogOnOff(MBOOL fgOnOff){m_fgLogEn = fgOnOff;}
