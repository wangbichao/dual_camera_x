...
MBOOL CamIOPipe::init()
{
    ...
    //temp mark to avoid build err, need revise later
...
//                ctl_mux_sel2 = 0xC0084108;//0x40380148;   //TEST_MDP ctl_mux_sel2 = 0x40080108;   // 0x15004078, Bin out after LSC
        ...
        case eScenarioID_VSS:       //video snap shot
            ...
            if ( eScenarioFmt_YUV == meScenarioFmt )
            {
                ...
                ctl_sram_mux_cfg = 0x010054F7; //TEST_VSS_YUV ctl_sram_mux_cfg = 0x00021000;  // 407C.    // SGG_HRZ_SEL = 0, PREGAIN_SEL = 0, ESFKO_SOF_SEL_EN = 1, ESFKO_SOF_SEL = 1.
            }
...
sendCommand(MINT32 cmd, MINTPTR arg1, MINTPTR arg2, MINTPTR arg3)
{
    ...
    switch ( cmd ) {
        ...
        case EPIPECmd_SET_IMGO_RAW_TYPE:
            {
                if ( eRawImageType_PreProc == (ERawImageType)arg1 ) {
                    m_RawType = 1;
                }
            }
            break;
