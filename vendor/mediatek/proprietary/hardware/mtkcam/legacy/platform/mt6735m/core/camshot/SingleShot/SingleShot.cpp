...
{
    CAM_TRACE_NAME("SingleShot::startOne 1");
    FUNCTION_LOG_START;
    ...
    IHalSensor* sensor_hal = NULL;

    querySensorResolution(mSensorParam.u4DeviceID, mSensorParam.u4Scenario, u4TgInW, u4TgInH);

    if(pHalSensorList != NULL)
    {
        MUINT i, sensorOpenId = 0, sensorNum = pHalSensorList->queryNumberOfSensors();
        ...
        pHalSensorList->querySensorStaticInfo(
                            rSensorParam.u4DeviceID,
                            &sensorStaticInfo);
        sensor_hal = pHalSensorList->createSensor(
                                        LOG_TAG,
                                        sensorOpenId);
        if (NULL == sensor_hal)
        {
    ...
    if(sensorStaticInfo.fakeOrientation)
    {
        ...
        {
            mu4DiffOri = ANGLE_SUB(90, sensorStaticInfo.orientationAngle);
        }
    }
    else
    {
...
{
    CAM_TRACE_NAME("SingleShot::startOne 2");
    FUNCTION_LOG_START;
...
#warning [TODO] this should callback from pipe
    ...
    //
    p3AObj->setSensorMode(rSensorParam.u4Scenario);
...
{
    CAM_TRACE_NAME("SingleShot::createSensorRawImg");
    FUNCTION_LOG_START;
    ...
    CAM_TRACE_BEGIN("CamIOPipe::configPipe");
    ...
    CAM_TRACE_BEGIN("CamIOPipe::enqueBuf");
    ...
    ret = ret && p3AObj->setIspProfile(static_cast<EIspProfile_T>(meProfile));
     CAM_TRACE_END();
    CPTLogStr(Event_SShot_createSensorRawImg, CPTFlagSeparator, "3A CapStart");
    CAM_TRACE_BEGIN("3A::ECmd_CaptureStart");
    ...
    CAM_TRACE_BEGIN("CamIOPipe::dequeBuf");
...
{
    CAM_TRACE_NAME("SingleShot::createYuvRawImg 1");
    FUNCTION_LOG_START;
    ...
    CAM_TRACE_BEGIN("PostProcPipe::configPipe");
    ...
    CAM_TRACE_BEGIN("PostProcPipe::enqueBuf in");
    ...
    CAM_TRACE_BEGIN("PostProcPipe::enqueBuf out");
    ...
    CAM_TRACE_BEGIN("PostProcPipe::start");
    pPostProcPipe->start();
    CAM_TRACE_END();
    ...
    CAM_TRACE_BEGIN("PostProcPipe::dequeBuf out");
...
{
    CAM_TRACE_NAME("SingleShot::createYuvRawImg 2");
    FUNCTION_LOG_START;
    AutoCPTLog cptlog(Event_SShot_createYuvRawImg);
    if (!lock(mbCCmode ? RES_MGR_DRV_SCEN_HW_VSS : RES_MGR_DRV_SCEN_HW_IP,
              PIPE_MGR_DRV_PIPE_MASK_POST_PROC, 3000))
...
{
    CAM_TRACE_NAME("SingleShot::rotatePostview");
    FUNCTION_LOG_START;
...
{
    CAM_TRACE_NAME("SingleShot::createJpegImg");
    FUNCTION_LOG_START;
    AutoCPTLog cptlog(Event_SShot_createJpegImg);
    MtkCamUtils::CamProfile profile("createJpegImg", "SingleShot");
    //
...
preAllocMem()
{
    FUNCTION_LOG_START;
    //
    pthread_attr_t const attr = {0, NULL, 1024*1024, 4096, SCHED_OTHER, NICE_CAMERA_CAPTURE - 1};
