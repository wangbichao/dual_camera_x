...
)
{
    ...
    IspTuningMgr::getInstance().setISPAEGain((ESensorDev_T)m_eSensorDev,
                                                           MFALSE, m_rAEOutput.rPreviewMode.u4IspGain>>1);
    //ISP_MGR_OBC_T::getInstance((ESensorDev_T)m_eSensorDev).setIspAEGain(m_rAEOutput.rPreviewMode.u4AfeGain);
    ...
    //Allen: IspTuningMgr::getInstance().setAEInfo((ESensorDev_T)m_eSensorDev, rAEInfo2ISP);
    IspTuningMgr::getInstance().setAEInfo(m_eSensorDev, rAEInfo2ISP);
...
)
{
...
#if 0
    {
        static int i = 0;
        char fileName[64];
        sprintf(fileName, "/data/AEPline_apply_%d.bin",i++);
...
)
{
...
#if 0
    {
        static int i = 0;
        char fileName[64];
        sprintf(fileName, "/data/AEPline_get_%d.bin", i++);
        FILE *fp = fopen(fileName, "w");
        if (NULL == fp)
        {
            MY_ERR("fail to open file to save img: %s", fileName);
...
)
{
...
#if 0
    {
        static int i = 0;
        char fileName[64];
        sprintf(fileName, "/data/AEPline_save_%d.bin", i++);
        FILE *fp = fopen(fileName, "w");
        if (NULL == fp)
        {
            MY_ERR("fail to open file to save img: %s", fileName);
...
)
{
    ...
    //ISP_MGR_OBC_T::getInstance((ESensorDev_T)m_eSensorDev).setIspAEGain(a_IspRAWGain>>1);
    return S_AE_OK;
}
