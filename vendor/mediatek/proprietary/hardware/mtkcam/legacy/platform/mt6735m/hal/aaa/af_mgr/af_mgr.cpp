...
MRESULT AfMgr::init(MINT32 eSensorDev, MINT32 i4SensorIdx)
{
    ...
    if((m_NVRAM_LENS.rAFNVRAM.i4SensorEnable > 0) && (m_AndroidServiceState==TRUE) )
    {
...
MRESULT AfMgr::setAFMode(MINT32 eAFMode)
{
    ...
    MY_LOG_IF(m_bDebugEnable, "[setAFMode] %d to %d\n", m_eCurrAFMode,eAFMode);
...
{
    MY_LOG_IF(m_bDebugEnable, "getAFState, trigger %d, mode %d\n", m_isTriggerAFS, m_eLIB3A_AFMode);
    ...
    if (m_pIAfAlgo)
    {
        ...
        {
            if (!isFocusFinish() || m_isTriggerAFS)
            {
                  af_state=E_AF_ACTIVE_SCAN;
            }
            else
            {
                af_state = isFocused() ? E_AF_FOCUSED_LOCKED : E_AF_NOT_FOCUSED_LOCKED;
            }
        }
        else
    #endif
...
MRESULT AfMgr::setZoomWinInfo(MUINT32 u4XOffset, MUINT32 u4YOffset, MUINT32 u4Width, MUINT32 u4Height)
{
    ...
    memset(&sAFArea,   0, sizeof(CameraFocusArea_T));
    sAFArea.u4Count = 0;
    setAFArea(sAFArea);
...
MRESULT AfMgr::doAF(MVOID *pAFStatBuf)
{
    ...
    if (m_pMcuDrv)
    {
        MY_LOG_IF(m_bDebugEnable, "[AFStatH]%lld [AFStatV]%lld [moveMCU] %d, [afstate] %d ", m_sAFInput.sAFStat.i8Stat24, m_sAFInput.sAFStat.i8StatV,m_sAFOutput.i4AFPos,m_af_state);
