...
#define logI(fmt, arg...) {MY_LOG_IF(mLogLevel, fmt, ##arg);}
...
int FlashMgrM::initSub(int sensorId, int isTemp)
{
    ...
    /*log control*/
    mLogLevel = FlashUtil::getPropInt("debug.camera.log.flash_mgr", 0);
...
int FlashMgrM::doPreviewOneFrame()
{
    ...
    {
        Mutex::Autolock lock(mLock);
        logI("getFlashState ln=%d ison=%d %d %d %d", __LINE__, isOn, mFlashStateStack[0], mFlashStateStack[1], mFlashStateStack[2]);
        int i;
        for(i=9;i>=1;i--)
        {
...
int FlashMgrM::start(FlashExePara* in)
{
    ...
    int i = 0;
    int j = 0;
    logI("logEng dutyN=%d", prjPara.dutyNum);
...
int FlashMgrM::setEvComp(int ind, float ev_step)
{
    if (m_evComp != ind*ev_step)
        logI("setEvComp ind=%d evs=%f origin=%f", ind, ev_step, m_evComp);
...
{
    if (mCamMode != mode)
        logI("setCamMode mode=%d old_mode=%d", mode, mCamMode);
    mCamMode = mode;
    return 0;
}
int FlashMgrM::setAeFlashMode(int aeMode, int flashMode)
{
    ...
    if (mFlash1Mode != fm)
        logI("setAeFlashMode ae_mode=%d flash_mode=%d f1_mode=%d old=%d", aeMode, flashMode, fm, mFlash1Mode);
...
int FlashMgrM::setFlashMode(int mode)
{
    ...
    {
        logI("setFlashMode- ln=%d capturing", __LINE__);
        return 0;
    }
...
int FlashMgrM::updateOneFrame()
{
...

    {
        ...
        if (mFlashStateStack[0] != curState)
            logI("updateOneFrame ln=%d ison=%d %d %d %d", __LINE__, isOn, mFlashStateStack[0], mFlashStateStack[1], mFlashStateStack[2]);
        int i;
        for(i=9;i>=1;i--)
        {
            mFlashStateStack[i]=mFlashStateStack[i-1];
