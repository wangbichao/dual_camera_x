...
)
{
...

    {
        ...
        if (fd < 0) {
            MY_LOGE("devinfo open fail, errno(%d):%s",errno,strerror(errno));
        }
        else
        {
            ...
            {
                isp_bonding = (devinfo.data[3] >> 29) & 0x3;
                MY_LOGD("Bound(%d) CapWidth(%d)", isp_bonding, CapWidth);
                if(isp_bonding != 0x0)
                {
                    switch(isp_bonding){
                        case 1: // 16M
        ...
        #if 0
            /*print Devinfo*/
            MY_LOGE("Total devinfo number: %d\n", devinfo.entry_num);
            for(index = 0; index < devinfo.entry_num; index++)
            {
                MY_LOGE("Devinfo Index:%d\n", index);
                MY_LOGE("Get Devinfo Data:0x%x\n", devinfo.data[index]);
            }
        #endif
            close(fd);
        }
