...
//
#include <mtkcam/drv_common/hwutils.h>
//
#include <sys/prctl.h>
//
...
public:
    ...
    readyToRun()
    {
        ...
        int const expect_priority   = NICE_CAMERA_SHUTTER_CB;
...
)
{
    if  ( msgTypeEnabled(CAMERA_MSG_SHUTTER) )
    {
        ...
        if  ( pThread == 0 || pThread->run("CamAdapter::onCB_Shutter") != OK )
        {
...
#warning "[TODO] Magic number: need to enumerize"
    switch (mpsz8RawSaveMode[0])
    {
        ...
        case '5': // 4: "Slim Video 1 Preview Mode"
        case '6': // 4: "Slim Video 2 Preview Mode"
        default:
...
)
{
    ...
    if  ( image )
    {
        ...
        if  ( pThread == 0 || pThread->run("CamAdapter::onCB_CompressedImage") != OK )
        {
