...
MINT32 AcdkUtility::ImageallocBuffer(IImageBuffer*& ppBuf, MUINT32 w, MUINT32 h, MUINT32 fmt, IMEM_BUF_INFO Imem)
{
    ...
    if (pBuf == NULL) {
        ACDK_LOGD("Null createImageBuffer\n");
        return  MFALSE;
    }
    pBuf->incStrong(pBuf);
                                       ...
                                       MUINT32 a_Stride)
{
    ...
    ACDK_LOGD("srcImem : VA(0x%p),PA(0x%p),ID(%d),SZ(%u)",srcImem.virtAddr,
                                                           srcImem.phyAddr,
                                                           srcImem.memID,
                                                           srcImem.size);

    ACDK_LOGD("dstImem : VA(0x%p),PA(0x%p),ID(%d),SZ(%u)",dstImem.virtAddr,
                                                           dstImem.phyAddr,
                                                           dstImem.memID,
                                                           dstImem.size);
