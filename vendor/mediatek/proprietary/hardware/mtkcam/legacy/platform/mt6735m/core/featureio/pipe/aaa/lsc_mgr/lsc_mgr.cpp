    ...
    if (m_pIMemDrv)
    {
        ...
        if (bSceneChange == MTRUE)   // changed
        {
            ...
            if (m_bMetaMode == MTRUE) {
                ...
                //m_bTSF = tsfEnableOpt() ? MTRUE : MFALSE;
            }
        }
#endif
    }
...
#endif
{
    ...
    MY_LOG("[%s] m_pIMemDrv(0x%08x), m_rIspLscCfg(0x%08x), m_rIspShadingLut(0x%08x), m_rIspSl2Cfg(0x%08x), m_rIspNvram(0x%08x)",
            __FUNCTION__,
            (MUINT32)m_pIMemDrv,
    ...
    MY_LOG("[%s] Shading Version(%d) SensorId(0x%x) PreviewSVDSize(%d) VideoSVDSize(%d) CaptureSVDSize(%d)",
            __FUNCTION__,
...
createInstance(ESensorDev_T const eSensorDev, NVRAM_CAMERA_ISP_PARAM_STRUCT& rIspNvram)
{
    ...
    int err;
    err = NvBufUtil::getInstance().getBufAndRead(CAMERA_NVRAM_DATA_SHADING, eSensorDev, (void*&)pNvram_Shading);
    if(err!=0)
    {
        MY_ERR("Fail to init NvramDrvMgr, use default LSC table");
    }
...
tsfEnableOpt() const
{
    return m_rTsfCfgTbl.TSF_CFG.isTsfEn;
}
...
getTsfCfgTbl(void)
{
    MY_LOG("[%s]+ m_eSensorDev(0x%02x)", __FUNCTION__, (MINT32)m_eActive);

    CAMERA_TSF_TBL_STRUCT*      pDftTsf     = NULL;
    MBOOL           fgLoadOK                = MFALSE;
    MINT32          i4TsfEn                 = isEnableTSF(m_eActive);
    ...
    int err;
    err = NvBufUtil::getInstance().getBufAndRead(CAMERA_DATA_TSF_TABLE, m_eActive, (void*&)pDftTsf);
    if(err!=0)
    {
        MY_ERR("Fail to read tsf data");
    }
    else
    {
        fgLoadOK = MTRUE;
    ...
    if (fgLoadOK)
    {
        ...
        MY_LOG("[%s] Load TSF table OK, TSF(%d), CtIdx(%d), data(0x%08x), para(0x%08x)",
            __FUNCTION__,
    ...
    else
    {
        ...
        ::memcpy(m_rTsfCfgTbl.TSF_PARA, pi4TsfPara, sizeof(MINT32)*409);
        ::memcpy(m_rTsfCfgTbl.TSF_DATA, pu4TsfData, sizeof(MUINT32)*16000);
        MY_ERR("Load TSF table Fail, use default: TSF(%d), CtIdx(%d), data(0x%08x), para(0x%08x)",
            m_rTsfCfgTbl.TSF_CFG.isTsfEn, m_rTsfCfgTbl.TSF_CFG.tsfCtIdx,
            m_rTsfCfgTbl.TSF_DATA,
            m_rTsfCfgTbl.TSF_PARA);
    }
...
fillTSFInitParams(MTK_TSF_ENV_INFO_STRUCT &params)
{
    ...
    params.Para             = m_rTsfCfgTbl.TSF_DATA;
    params.pTuningPara      = m_rTsfCfgTbl.TSF_PARA;
    ...
    MY_LOG("[%s] ImgWidth(%d), ImgHeight(%d), BayerOrder(%d), ImgAddr(0x%08x), ShadingTbl(0x%08x), Raw16_9Mode(%d)",
            __FUNCTION__,
            params.ImgWidth,
            params.ImgHeight,
            params.BayerOrder,
    ...
    MY_LOG("[%s] raw_wd(%d), raw_ht(%d), block_wd(%d), block_ht(%d), xgrid(%d), ygrid(%d), wd_last(%d), ht_last(%d)",
            __FUNCTION__,
