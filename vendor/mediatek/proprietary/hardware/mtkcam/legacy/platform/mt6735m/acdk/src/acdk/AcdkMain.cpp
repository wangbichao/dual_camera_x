    ...
    ,mIsISPEng(MFALSE)
{
    ...
    if (m_regMgrDrvObj) {
        ...
        ACDK_LOGE("m_regMgrDrvObj->CreateInstance() error");
    }
...
MINT32 AcdkMain::init()
{
    ...
    mpIHal3A = Hal3ABase::createInstance(mSensorDev);
        ...
        if (!mpIHal3A->setParams(cam3aParam)) {
            ACDK_LOGE("IHal3A setParams error");
        }
#endif
#if FIXED ==1
...
MINT32 AcdkMain::startPreview(Func_CB prvCb)
{
    ...
    if (m_pSensorHalObj == NULL) {
        ACDK_LOGD("createSensor fail, null sensorObj ptr");
        err = ACDK_RETURN_INVALID_SENSOR;
...
MINT32 AcdkMain::handlePreviewCB(MVOID *param)
{
    ...
    else
    {
        ...
        memcpy((MUINT8 *)(mDispIMemInfo[OVERLAY_BUFFER_CNT-1].virtAddr), pVirBufin, displaySize);
        ISrcMemInfo.memID = mDispIMemInfo[OVERLAY_BUFFER_CNT-1].memID;
        ISrcMemInfo.virtAddr = mDispIMemInfo[OVERLAY_BUFFER_CNT-1].virtAddr;
        ISrcMemInfo.phyAddr = mDispIMemInfo[OVERLAY_BUFFER_CNT-1].phyAddr;
