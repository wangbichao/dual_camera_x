...
int CamPath_B::_registerIrq( NSImageio_FrmB::NSIspio_FrmB::Irq_t irq )
{
    return 0;
}

int CamPath_B::_waitIrq( NSImageio_FrmB::NSIspio_FrmB::Irq_t irq )
{
    return 0;
}
...
int CamPath_B::setDMACurrBuf( MUINT32 const dmaChannel)
{
    ...
    //
    if ( CAM_ISP_CQ_NONE != this->CQ ) {
        this->ispTopCtrl.ispDrvSwitch2Virtual(this->CQ,0,0,0);
    }
    //
    //->for pass2
    //
    ...
    //
    if ( ISP_DMA_WDMAO & dmaChannel ) {
        cdpPipe.wdmao_out.memBuf.base_pAddr = this->ispBufCtrl.getCurrHwBuf((MUINT32) ISP_DMA_WDMAO);
//SL TEST_MDP removed. Should no used    ISP_PATH_DBG("SL : this->ispBufCtrl.getCurrHwBuf((MUINT32) ISP_DMA_WDMAO)\n ");
//SL TEST_MDP removed. Should no used    ISP_PATH_DBG("SL : should GET SW ADDR now ; 0x%8x \n ",cdpPipe.wdmao_out.memBuf.base_pAddr);
//SL TEST_MDP removed. Should no used        ret = this->DMADispo.config(); //SL
        //
        if ( 0 == cdpPipe.wdmao_out.uv_plane_swap ) {
            cdpPipe.wdmao_out.memBuf_c.base_pAddr = cdpPipe.wdmao_out.memBuf.base_pAddr + \
                                                    cdpPipe.wdmao_out.memBuf_c.ofst_addr;
            cdpPipe.wdmao_out.memBuf_v.base_pAddr = cdpPipe.wdmao_out.memBuf.base_pAddr + \
                                                    cdpPipe.wdmao_out.memBuf_v.ofst_addr;
        }
        else {
            cdpPipe.wdmao_out.memBuf_c.base_pAddr = cdpPipe.wdmao_out.memBuf.base_pAddr + \
                                                    cdpPipe.wdmao_out.memBuf_v.ofst_addr;
        ...
        this->ispTopCtrl.m_pIspDrvShell->m_pP2CrzDrv->DISPO_SetOutputAddr(
                        this->cdpPipe.wdmao_out.memBuf.base_pAddr,
                        this->cdpPipe.wdmao_out.memBuf.ofst_addr,
                        this->cdpPipe.wdmao_out.size.stride * this->cdpPipe.wdmao_out.pixel_byte, //byte input
                        this->cdpPipe.wdmao_out.memBuf_c.base_pAddr,
                        this->cdpPipe.wdmao_out.memBuf_c.ofst_addr,
                        this->cdpPipe.wdmao_out.size_c.stride * this->cdpPipe.wdmao_out.pixel_byte, //byte input
                        this->cdpPipe.wdmao_out.memBuf_v.base_pAddr,
                        this->cdpPipe.wdmao_out.memBuf_v.ofst_addr,
                        this->cdpPipe.wdmao_out.size_v.stride * this->cdpPipe.wdmao_out.pixel_byte); //byte input

        ISP_PATH_DBG("[DISPO]base(0x%x)\n",this->cdpPipe.wdmao_out.memBuf.base_pAddr);

    }
    //
    if ( ISP_DMA_WROTO & dmaChannel ) {
        cdpPipe.wroto_out.memBuf.base_pAddr = this->ispBufCtrl.getCurrHwBuf((MUINT32) ISP_DMA_WROTO);
        //
        if ( 0 == cdpPipe.wroto_out.uv_plane_swap ) {
            cdpPipe.wroto_out.memBuf_c.base_pAddr = cdpPipe.wroto_out.memBuf.base_pAddr + \
                                                    cdpPipe.wroto_out.memBuf_c.ofst_addr;
            cdpPipe.wroto_out.memBuf_v.base_pAddr = cdpPipe.wroto_out.memBuf.base_pAddr + \
                                                    cdpPipe.wroto_out.memBuf_v.ofst_addr;
        }
        else {
            cdpPipe.wroto_out.memBuf_c.base_pAddr = cdpPipe.wroto_out.memBuf.base_pAddr + \
                                                    cdpPipe.wroto_out.memBuf_v.ofst_addr;
        ...
        //ret = this->cdpPipe.config(); //this func will induce sysram realloc.
        this->ispTopCtrl.m_pIspDrvShell->m_pP2CrzDrv->VIDO_SetOutputAddr(
                        this->cdpPipe.wroto_out.memBuf.base_pAddr,
                        this->cdpPipe.wroto_out.memBuf.ofst_addr,
                        this->cdpPipe.wroto_out.size.stride * this->cdpPipe.wroto_out.pixel_byte, //byte input
                        this->cdpPipe.wroto_out.memBuf_c.base_pAddr,
                        this->cdpPipe.wroto_out.memBuf_c.ofst_addr,
                        this->cdpPipe.wroto_out.size_c.stride * this->cdpPipe.wroto_out.pixel_byte, //byte input
                        this->cdpPipe.wroto_out.memBuf_v.base_pAddr,
                        this->cdpPipe.wroto_out.memBuf_v.ofst_addr,
                        this->cdpPipe.wroto_out.size_v.stride * this->cdpPipe.wroto_out.pixel_byte); //byte input

        ISP_PATH_INF("[VIDO]base(0x%x)-0x%x-0x%x\n",this->cdpPipe.wroto_out.memBuf.base_pAddr,
                this->cdpPipe.wroto_out.memBuf_c.base_pAddr,this->cdpPipe.wroto_out.memBuf_v.base_pAddr);
...
int CamPath_B::flushCqDescriptor( MUINT32 cq )
{
    ...
    if(cq < ISP_DRV_BASIC_CQ_NUM) {
        for(int i=0; i<NSIspDrv_FrmB::CAM_DUMMY_; i++)
            this->ispTopCtrl.m_pIspDrvShell->cqDelModule((NSIspDrv_FrmB::ISP_DRV_CQ_ENUM)cq,0,0, (NSIspDrv_FrmB::CAM_MODULE_ENUM)i);
    } else {
...
int CamPath_B::setDMANextBuf( MUINT32 const dmaChannel )
{
    ...
    //
    if ( ISP_DMA_WDMAO & dmaChannel ) {
        cdpPipe.wdmao_out.memBuf.base_pAddr = this->ispBufCtrl.getNextHwBuf((MUINT32) ISP_DMA_WDMAO);
        ret = this->cdpPipe.config();
    }
    //
    if ( ISP_DMA_WROTO & dmaChannel ) {
        cdpPipe.wroto_out.memBuf.base_pAddr = this->ispBufCtrl.getNextHwBuf((MUINT32) ISP_DMA_WROTO);
        ret = this->cdpPipe.config();
    }
    //
    if ( ISP_DMA_FDO & dmaChannel ) {
        //FD can use either port, choose one.
        cdpPipe.wdmao_out.memBuf.base_pAddr = this->ispBufCtrl.getNextHwBuf((MUINT32) ISP_DMA_FDO);
        ret = this->cdpPipe.config();
    }
    //
    if ( 0 != ret ) {
