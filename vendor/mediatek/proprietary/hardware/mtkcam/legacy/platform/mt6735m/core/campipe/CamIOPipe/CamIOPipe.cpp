...
configPipe(vector<PortInfo const*>const& vInPorts, vector<PortInfo const*>const& vOutPorts)
{
    ...
    if (NULL == mpSensorHal)
    {
        ...
        {
            MUINT i, sensorNum = pHalSensorList->queryNumberOfSensors();

            for(i=0; i<sensorNum; i++)
            {
                if(pHalSensorList->querySensorDevIdx(i) == static_cast<MUINT32>(pSensorPort->u4DeviceID))
                {
                    mSensorOpenId = i;
                    MY_LOGD("sensorOpenId(%d)",mSensorOpenId);
                    break;
                }
            }

            pHalSensorList->querySensorStaticInfo(
                                pSensorPort->u4DeviceID,
                                &mSensorStaticInfo);
...
#warning [TODO] Should check the port config by scenario
    ...
    {
        mu4SkipFrame = mSensorStaticInfo.captureDelayFrame;
    }
...
MBOOL
CamIOPipe::
sendCommand(MINT32 cmd, MINT32 arg1, MINT32 arg2, MINTPTR arg3)
{
    //FUNCTION_LOG_START;
...
#define scenario_case(scenario, KEY)       \
                case scenario:                            \
                    u4Width = mSensorStaticInfo.KEY##Width;  \
                    u4Height = mSensorStaticInfo.KEY##Height; \
                    break;
                switch(u4Scenario)
                {
...
#undef scenario_case
    ...
    switch(mSensorStaticInfo.sensorType)
    {
        case SENSOR_TYPE_RAW:
            eFmt = mapRawFormat(u4BitDepth);
            u4RawPixelID = mapRawPixelID(mSensorStaticInfo.sensorFormatOrder);
        break;
        case SENSOR_TYPE_YUV:
            eFmt = mapYUVFormat(mSensorStaticInfo.sensorFormatOrder);
        break;
        break;
...
configSensor(
    ...
    MUINT32 const u4Scenario,
    MUINT32 const u4Width,
    MUINT32 const u4Height,
    MUINT32 const byPassDelay,
    MUINT32 const u4ByPassScenario,
    MBOOL const fgIsContinuous)
{
    ...
    {
        index               : mSensorOpenId,
        crop                : MSize(u4Width, u4Height),
        scenarioId          : u4Scenario,
        isBypassScenario    : static_cast<MBOOL>(u4ByPassScenario),
        isContinuous        : fgIsContinuous,
        HDRMode             : 0,
        framerate           : 0, //default frame rate
        twopixelOn          : 0,
        debugMode           : 0,
    };
    return mpSensorHal->configure(1, &configParam[0]);
}
