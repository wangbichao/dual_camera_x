...
INormalPipe_FrmB::createInstance(MUINT32 *pSensorIndex, MUINT32 sensorCnt,char const* szCallerName, MINT32 burstQnum)
{
    ...
    if (1 == sensorCnt) {//NPIPE_Sensor_0 ||  NPIPE_Sensor_1
        ...
        if (pSensorIndex[0] > NPIPE_Sensor_0And1){
...
{
    MUINT32 d = this->GetOpenedPortInfo(NormalPipe_FrmB::_GetPort_OpendNum);
    if (d == 0) {
        PIPE_ERR("Opened port num is 0");
        return 0;
    }
    return (this->mRingBufDepth / d);
}
