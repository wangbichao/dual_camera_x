            ...
            switch (imgFmt)
             {
                ...
                case eImgFmt_BAYER10:         //= 0x0002,   //Bayer format, 10-bit
                    ...
                    stride = stride + 8;    //add dummy stride to avoid tpipe/hw contraint
                    break;
...
typedef enum _QUERYOP{
    ...
    ISP_QUERY_CROP_START_X = 0x0040,   //query when using cropping, (crop size need to satisfy cropping rule)
}E_ISP_QUERY;
...
static MUINT32 _queryCropStart(EImageFormat imgFmt,MUINT32 input,MUINT32 &rst)
{
    switch(imgFmt){
        case eImgFmt_BAYER8:
            rst = input;
...
static MUINT32 _queryXsize_rrzo(EImageFormat imgFmt,MUINT32& xsize, MUINT32& outWidth,MUINT32 pixMode){
    ...
    if( (outWidth > RRZ_LIMITATION) && (pixMode == ISP_QUERY_1_PIX_MODE) ){
        ISPUTIL_ERR("rrzo output over 3072, support no 1 pix mode, 2 pix mode will be enabled\n");
        ...
        switch(imgFmt){
            ...
            case eImgFmt_BLOB:
                ...
                if(e_Op & ISP_QUERY_CROP_START_X){
                    if(_queryCropStart(imgFmt,QueryInput,QueryRst.crop_x) == 0){
            ...
            case eImgFmt_FG_BAYER10:
            case eImgFmt_FG_BAYER12:
                ISPUTIL_ERR("FullG/UFO formats r not supported\n",portId,imgFmt);
                return 0;
            break;
        default:
            {
...
#else
    ISPUTIL_ERR("error:rrzo is not supported\n");
    return 0;
#endif
}
