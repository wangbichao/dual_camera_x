...
LscMgr::UpdateSL2Param(void)
{
    ...
    if (i4SL2En != -1)
    {
        ...
        ISP_MGR_SL2_T::getInstance(m_eSensorDev).setEnable(i4SL2En ? MTRUE : MFALSE);
    }
...
//            ISP_MGR_SL2_T::getInstance(m_eSensorDev).apply(getIspProfile()); //Apply to ISP.

            MY_LOG_IF(_bEnableMyLog, "[%s] TSF SL2 = %d, cen=0x%08x, max0_rr=0x%08x, max1_rr=0x%08x, max2_rr=0x%08x\n",
                __FUNCTION__, ISP_MGR_SL2_T::getInstance(m_eSensorDev).isEnable(),
...
getSensorResolution()
{
    ...
    MINT32 u4NumSensorModes = 3; //MIN(rSensorStaticInfo.SensorModeNum, 3);
...
//    m_SensorDev = (halSensorDev_s)m_eSensorDev;
#if 1
    ...
    if (m_pIMemDrv)
    {
        ...
        if (bSceneChange == MTRUE)   // changed
        {
            ...
            if (m_bMetaMode == MTRUE) {
                ...
                //m_bTSF = isEnableTSF(m_eSensorDev) ? MTRUE : MFALSE;
            }
        }
#endif
    }
...
enableLscWoVariable(MBOOL const fgEnable)
{
    ...
    MY_LOG("[LscMgr] -enableLsc(enableLscWoVariable)"
            "  --> %d\n", fgEnable);
    // (1) get hw setting
    ISP_MGR_LSC_T::getInstance(m_eSensorDev).get(tmp);

    //  (2) Change the state of hw data
    tmp.lsc_en.bits.LSC_EN = tmp.lsci_en.bits.LSCI_EN = (MUINT32)fgEnable;

    //  (3) Apply to ISP.
    ISP_MGR_LSC_T::getInstance(m_eSensorDev).put(tmp); //put to ispmgr
    fgRet = ISP_MGR_LSC_T::getInstance(m_eSensorDev).apply(getIspProfile()); //Apply to ISP.

    MY_LOG("[LscMgr] enable %d", ISP_MGR_LSC_T::getInstance(m_eSensorDev).isEnable());
...
enableLsc(MBOOL const fgEnable)
{
    ...
    //  (3) Apply to ISP.
    fgRet = 1; //ISP_MGR_LSC_T::getInstance(m_eSensorDev).reset();
    ...
    if (fgRet) {
        ...
        ISP_MGR_LSC_T::getInstance(m_eSensorDev).put(m_rIspLscCfg[m_eLscScenario]); //put to ispmgr_mt6573
        fgRet = ISP_MGR_LSC_T::getInstance(m_eSensorDev).apply(getIspProfile()); //Apply to ISP.
...
getDebugInfo(DEBUG_RESERVEA_INFO_T &rShadingDbgInfo)
{
    ...
    MY_LOG("[%s] + size=%d\n", __FUNCTION__, sizeof(DEBUG_RESERVEA_INFO_T));
...
#endif
{
    ...
    {
        ISP_NVRAM_LSC_T debug;
        ISP_MGR_LSC_T::getInstance(m_eSensorDev).reset();
        ISP_MGR_LSC_T::getInstance(m_eSensorDev).get(debug);
        MY_LOG("[LscMgr]  %s"
...
LscMgr::init() {
    ...
    ISP_MGR_LSC_T::getInstance(m_eSensorDev).put(lsc_off);
    ISP_MGR_LSC_T::getInstance(m_eSensorDev).apply(EIspProfile_NormalPreview);
    ISP_MGR_LSC_T::getInstance(m_eSensorDev).apply(EIspProfile_NormalCapture);

    ISP_MGR_SL2_T::getInstance(m_eSensorDev).put(sl2_off);
    //ISP_MGR_SL2_T::getInstance(m_eSensorDev).apply(EIspProfile_NormalPreview);
    //ISP_MGR_SL2_T::getInstance(m_eSensorDev).apply(EIspProfile_NormalCapture);
...
fillTSFInitParams(MTK_TSF_ENV_INFO_STRUCT &params)
{
    ...
    if (MERR_OK != NvramDrvMgr::getInstance().init(m_eSensorDev))
    {
...
//                    }
                ...
                {  // normal preview case
                    ...
                    {
                        MINT32 i4Batch = 1;
                        //GET_PROP("debug.lsc_mgr.tsf_pv_batch", "0", i4Batch);
                        ...
                        {
                            MY_LOG("[%s] Batch run +", __FUNCTION__);
                            ret = tsf->TsfFeatureCtrl(MTKTSF_FEATURE_BATCH, &TSFInput, &TSFOutput);
                            if (ret == S_TSF_OK)
                            {
                                ::memcpy((void*)TSFInput.ShadingTbl,
                                ...
                                lsc->enableLsc(isLscActive);
                                lsc->mTSFState = LSCMGR_TSF_STATE_DO;
                                MY_LOG("[%s] Batch run done -", __FUNCTION__);
                            }
                            else
                            {
                                MY_ERR("Batch run error(%d)", ret);
                            }
                        }
                }
                }
