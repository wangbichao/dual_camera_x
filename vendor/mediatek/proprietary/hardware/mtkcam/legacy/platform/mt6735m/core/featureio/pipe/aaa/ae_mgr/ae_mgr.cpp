...
MRESULT AeMgr::PreviewAEInit(MINT32 i4SensorIdx, Param_T &rParam)
{
    ...
    // AE statistics and histogram config
     err = ISP_MGR_AE_STAT_HIST_CONFIG_T::getInstance(m_eSensorDev).config(m_rAEStatCfg);
    if (FAILED(err)) {
...
MRESULT AeMgr::camcorderPreviewInit(MINT32 i4SensorIdx, Param_T &rParam)
{
    ...
    MY_LOG("[%s()] m_eSensorDev: %d, i4SensorIdx = %d, m_eSensorMode = %d CamMode:%d m_eAEScene:%d\n", __FUNCTION__, m_eSensorDev, i4SensorIdx, m_eSensorMode, m_eAECamMode, m_eAEScene);

    // the same with preview initial


    if (m_eAEScene == LIB3A_AE_SCENE_NIGHT){
        err = setAEMinMaxFrameRate( 10000 , 15000 );
    } else {
        err = setAEMinMaxFrameRate( 10000 , 30000 );
    }
    m_bIsVideoRecording = MTRUE;
    m_eAECamMode = LIB3A_AECAM_MODE_VIDEO; // for board support package test used.
    PreviewAEInit(i4SensorIdx, rParam);
...
MRESULT AeMgr::setFDenable(MBOOL bFDenable)
{
    ...
    MY_LOG("[%s()] FDenable is %d\n", __FUNCTION__, bFDenable);
    return S_AE_OK;
}
   ...
   if(CUST_ENABLE_FACE_AE() == MTRUE) {
        ...
        // Define FD area number
           ...
           for(ifaceIdx=0; ifaceIdx < m_eAEFDArea.u4Count; ifaceIdx++) {
                ...
                if ((m_eAEFDArea.rAreas[ifaceIdx].i4Left   != pFaces->faces[ifaceIdx].rect[0])||
         ...
         MY_LOG_IF(m_3ALogEnable,"[%s()] i4SensorDev:%d line:%d Face Number:%d FaceAEChange:%d", __FUNCTION__, m_eSensorDev, __LINE__, m_eAEFDArea.u4Count, m_bFaceAEAreaChage);
         }
    } else {
...
MRESULT AeMgr::setAEMinMaxFrameRate(MINT32 i4NewAEMinFps, MINT32 i4NewAEMaxFps)
{
    ...
    if ( ((m_bIsVideoRecording == 0)||(m_bIsEISon == 1))&&((m_i4AEMinFps != i4NewMinFPS) || (m_i4AEMaxFps != i4NewMaxFPS)) ) {
    ...
    MY_LOG("[%s()] AEMgr block the framerate setting , IsVideoRecording: %d m_bIsEISon: %d m_i4AEMinFps: %d m_i4AEMaxFps:%d\n", __FUNCTION__, m_bIsVideoRecording ,m_bIsEISon , i4NewMinFPS, i4NewMaxFPS);
    }
...
#if 0
        if (bAFAELock == MFALSE){
              if (m_strAECycleInfo.m_i4FrameCnt>=2){
                      m_strAECycleInfo.m_i4FrameCnt = 4;
...
MRESULT AeMgr::doPvAEmonitor(MINT32 i4FrameCount, MVOID *pAEStatBuf, MINT32 i4ActiveAEItem, MUINT32 u4AAOUpdate, MBOOL bAAASchedule, MBOOL &bWillUpdateSensorbyI2C)
{
    ...
    memset(&rOutput, 0, sizeof(mVHDROutputParam_T));
    ...
    if(m_bEnableAE) {
        if ((i4ActiveItem & E_AE_AE_CALC) || (bCalculateAE == MTRUE)) {
            ...
            if(m_eZoomWinInfo.bZoomChange == MTRUE) {
                ...
                ISP_MGR_AE_STAT_HIST_CONFIG_T::getInstance(m_eSensorDev).config(m_rAEStatCfg);
            } else {
...
#if 0
    {
        MY_LOG("sizeof(AE_PLINETABLE_T) = %d", sizeof(AE_PLINETABLE_T));
        static int i = 0;
        char fileName[64];
        sprintf(fileName, "/sdcard/getNvramData_%d.bin",i++);
...
MRESULT AeMgr::AEInit(Param_T &rParam)
{
    ...
    if (m_bIsVideoRecording == MTRUE){
        m_rAEInitInput.i4SensorMode = ESensorMode_Video;
    } else{
        m_rAEInitInput.i4SensorMode = m_eSensorMode;
    }

    MY_LOG("[%s()] i4SensorDev:%d SensorIdx:%d AE max block width:%d heigh:%d AE meter:%d CamMode:%d AEScene:%d Flicker :%d %d EVcomp:%d ISO:%d %d MinFps:%d MaxFps:%d Limiter:%d SensorMode:%d\n", __FUNCTION__,
            ...
            m_i4AEMinFps, m_i4AEMaxFps, m_bAElimitor, m_rAEInitInput.i4SensorMode);
...
MRESULT AeMgr::getExposureInfo(ExpSettingParam_T &strHDRInputSetting)
{
    ...
    if(m_pIAeAlgo != NULL) {
        ...
        if(rSWHdrInfo.eAeTargetMode == AE_MODE_AOE_TARGET) {
            ...
            memset(&rOutput, 0, sizeof(mVHDR_SWHDR_OutputParam_T));
            if(m_rSensorVCInfo.u2VCModeSelect == 0)
    ...
    switch(eNewAEState)
    {
        ...
        case AE_MANUAL_FRAMERATE_STATE:
            if(m_pIAeAlgo != NULL) {
                ...
                MY_LOG_IF(m_3ALogEnable, "[%s()] The AE algo class is NULL  i4SensorDev:%d line:%d", __FUNCTION__, m_eSensorDev, __LINE__);
            }

            MY_LOG_IF(m_3ALogEnable, "[%s:s] i4SensorDev:%d VDNum %d, Prev %d/%d/%d, Cur %d/%d/%d, Output %d/%d/%d Smooth:%d\n", __FUNCTION__, m_eSensorDev, m_i4WaitVDNum,
                    m_u4PreExposureTime,
                    m_u4PreSensorGain,
                    m_u4PreIspGain,
...
//                        g_rAEOutput.rPreviewMode.u4IspGain);
...
#endif
                MY_LOG_IF(m_3ALogEnable, "[%s:e] i4SensorDev:%d VDNum %d, Delay %d/%d/%d, Prev %d/%d/%d, Cur %d/%d/%d, Output %d/%d/%d State:%d FrameRate:%d %d %d Flare Offset:%d", __FUNCTION__, m_eSensorDev, m_i4WaitVDNum,
                ...
                if(m_p3ANVRAM != NULL) {
                    ...
                    rAEInfo2ISP.u4RealISOValue = (85*((rAEInfo2ISP.u4AfeGain*rAEInfo2ISP.u4IspGain)>>10))>>10;
                    MY_LOG_IF(m_3ALogEnable, "[%s()] m_i4SensorDev:%d NVRAM data is NULL\n", __FUNCTION__, m_eSensorDev);
                }
        ...
        case AE_AF_STATE:
            ...
            m_i4WaitVDNum++;
            MY_LOG_IF(m_3ALogEnable, "[doAFAE] AF SensorDev:%d Shutter:%d Sensor gain:%d Isp gain:%d frame rate:%d flare:%d %d ISO:%d m_i4WaitVDNum:%d\n", m_eSensorDev, m_rAEOutput.rAFMode.u4Eposuretime,
            ...
            MY_LOG_IF(m_3ALogEnable, "[doAFAE] Capture i4SensorDev:%d Shutter:%d Sensor gain:%d Isp gain:%d frame rate:%d flare:%d %d ISO:%d\n", m_eSensorDev, m_rAEOutput.rCaptureMode[0].u4Eposuretime,
        ...
        case AE_CAPTURE_STATE:
            ...
            if( (m_bEnableAE == MTRUE) && (m_bMultiCap == MFALSE) &&  (mCaptureMode.u4Eposuretime > 50000000) && (m_rAEInitInput.rAEPARAM.strAEParasetting.bEnableLongCaptureCtrl == MTRUE)) {
        ...
        case AE_TWO_SHUTTER_FRAME_STATE:
            if(m_i4WaitVDNum == m_i4ShutterDelayFrames) {
                ...
                if(bUpdateSensorAWBGain) {
                    strSensorAWBGain rSensorAWBGain;
                    AWB_GAIN_T rCurrentIspAWBGain = {1024,1024,1024};//Allen: ISP_MGR_PGN_T::getInstance(m_eSensorDev).getIspAWBGain();
                rSensorAWBGain.u4R = rCurrentIspAWBGain.i4R;
                rSensorAWBGain.u4GR = rCurrentIspAWBGain.i4G;
                rSensorAWBGain.u4GB = rCurrentIspAWBGain.i4G;
                rSensorAWBGain.u4B = rCurrentIspAWBGain.i4B;
                ...
                //Allen: IspTuningMgr::getInstance().setISPAEGain((ESensorDev_T)m_eSensorDev, m_strIVHDROutputSetting.bEnableWorkaround, m_strIVHDROutputSetting.u4LEISPGain>>1);
                //Allen: IspTuningMgr::getInstance().setIspFlareGainOffset((ESensorDev_T)m_eSensorDev, m_rAEOutput.rPreviewMode.i2FlareGain, (-1*m_rAEOutput.rPreviewMode.i2FlareOffset));
                //MY_LOG("[%s()]Line(%d)setIspAEGain(%d)\n", __FUNCTION__, __LINE__, m_strIVHDROutputSetting.u4LEISPGain);
                ISP_MGR_OBC_T::getInstance((ESensorDev_T)m_eSensorDev).setIspAEGain(m_strIVHDROutputSetting.u4LEISPGain>>1);
                ISP_MGR_PGN_T::getInstance((ESensorDev_T)m_eSensorDev).setIspFlare(m_rAEOutput.rPreviewMode.i2FlareGain, (-1*m_rAEOutput.rPreviewMode.i2FlareOffset));
...
MRESULT AeMgr::monitorAndReschedule( MBOOL bAAASchedule, MUINT32 u4AvgYcur, MUINT32 u4AvgYStable, MBOOL *bAECalc, MBOOL *bAEApply )
{
    ...
    MY_LOG_IF(m_3ALogEnable, "[%s()] Calc:%d Apply:%d Cnt:%d Frame:%d ReSchedule:%d Exit:%d Skip:%d bStatChange:%d bAAASchedule:%d frameDelay:%d Stable:%d AEMonitorStable:%d VdCnt:%d ISPdelay:%d FaceArea:%d TouchArea:%d u4AvgYcur:%d u4AvgYpre:%d isneedupdatesensorisp:%d\n",
              ...
              m_bAEStable, m_bAEMonitorStable, m_i4WaitVDNum, m_i4IspGainDelayFrames, m_bFaceAEAreaChage, m_bTouchAEAreaChage,u4AvgYcur, u4AvgYStable, m_bWillUpdateSensorbyI2C);

    // If I2C didn't work in time
    if (IsNeedUpdateSensor() == 1){
    MY_LOG_IF(m_3ALogEnable, "[I2Cincorrect] m_bSetShutterValue:%d, m_bSetGainValue:%d, m_bSetFrameRateValue:%d \n",  m_bSetShutterValue, m_bSetGainValue, m_bSetFrameRateValue);
    }
...
MRESULT AeMgr::setEISon(MBOOL bISEISon)
{
    m_bIsEISon = bISEISon ;
    MY_LOG("[%s()] current:%d previous:%d \n", __FUNCTION__, m_bIsEISon, bISEISon);
    return S_AE_OK;
}
