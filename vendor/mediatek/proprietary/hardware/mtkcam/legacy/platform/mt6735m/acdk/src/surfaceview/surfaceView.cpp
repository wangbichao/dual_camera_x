        ...
        MUINT32 const orientation)
{
    ...
    ACDK_LOGD_DYN(g_overlayDebug,"phyAddr  = 0x%p", phyAddr);
    ACDK_LOGD_DYN(g_overlayDebug,"virtAddr = 0x%p", virtAddr);
...
MINT32 surfaceView::refresh()
{
    ...
    if(g_overlayFlag == 0)
    {
        ...
        ACDK_LOGD_DYN(g_overlayDebug,"src VA(0x%p), PA(0x%p)",(MUINTPTR)mLayerInfo[0].src_base_addr,(MUINTPTR)mLayerInfo[0].src_phy_addr );
    ...
    else
    {
        ...
        ACDK_LOGD_DYN(g_overlayDebug,"use mmap");
        ACDK_LOGD_DYN(g_overlayDebug,"src VA(0x%p), size(%u)",(MUINTPTR)mLayerInfo[0].src_base_addr,mLayerInfo[0].src_width * mLayerInfo[0].src_height * 2);
        ...
        for(MUINT32 i = 0; i < mLayerInfo[0].src_height; ++i)
        {
            memcpy((m_pFrameBuf + (mFBBufferIndex * mFBBufferSize))+ mLayerInfo[0].tgt_offset_x + (mVinfo.xres_virtual * mLayerInfo[0].tgt_offset_y * bytePrePixel) + (i * mVinfo.xres_virtual * bytePrePixel),
                    (void*)((MUINT8*)mLayerInfo[0].src_base_addr + (i * mLayerInfo[0].src_width * 2)), mLayerInfo[0].src_width * 2);
        }
...
*
*******************************************************************************/
MINT32 surfaceView::registerBuffer(MUINTPTR virtAddr, MUINT32 size)
{
    int ret = 0;
#if 0
    ACDK_LOGD("+");
    ...
    b.src_vir_addr = (MUINTPTR)virtAddr;
    b.size         = (MUINT32)size;
...
*
*******************************************************************************/
MINT32 surfaceView::unRegisterBuffer(MUINTPTR virtAddr)
{
    int ret = 0;
#if 0
    ACDK_LOGD("+");
