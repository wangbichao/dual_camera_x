...
{
    CAM_TRACE_NAME("ZsdShot::onYuv_capture");
    FUNCTION_LOG_START;
...
{
    CAM_TRACE_NAME("ZsdShot::onRaw_capture");
    FUNCTION_LOG_START;
    MBOOL ret = MTRUE;
...
onCmd_capture()
{
    ...
    CPTLogStrEx(Event_ZsdShot_capture, CPTFlagSeparator, (MUINTPTR)rNode.mainImgNode->getVirAddr(), 0, "dequeProcessor");
...
createJpegImg(ImgBufInfo const & rSrcImgBufInfo,
                    ...
                    MUINT32 & u4JpegSize)
{
    CAM_TRACE_NAME("ZsdShot::createJpegImg");
    //FUNCTION_LOG_START;
...
{
    CAM_TRACE_NAME("ZsdShot::imageTransform");
    FUNCTION_LOG_START;
    //return MFALSE;
    ...
    MUINTPTR src_addr_list[3];//
    unsigned int src_size_list[3];//
    int plane_num;//
    // set & register src buffer
    switch(rSrcImgBufInfo.eImgFmt) {
        case eImgFmt_YV12:
            src_addr_list[0] = (MUINTPTR)src_yp;
            src_addr_list[1] = (MUINTPTR)(src_yp + src_ysize);
            src_addr_list[2] = (MUINTPTR)(src_yp + src_ysize * 5 / 4);
    ...
    MUINTPTR dst_yp = (MUINTPTR)(rDstImgBufInfo.u4BufVA);
    ...
    MUINTPTR dst_addr_list[3];
    unsigned int dst_size_list[3];
    int plane_num_out;//
    switch(rDstImgBufInfo.eImgFmt) {
        case eImgFmt_YV12:
            dst_addr_list[0] = (MUINTPTR)dst_yp;
            dst_addr_list[1] = (MUINTPTR)(dst_yp + dst_ysize);
            dst_addr_list[2] = (MUINTPTR)(dst_yp + dst_ysize * 5 / 4);
...
mapNodeToImageBuf(ImgBufQueNode & rNode, ImgBufInfo & rBuf)
{
    ...
    rBuf.u4BufVA = (MUINTPTR)rNode->getVirAddr();
    rBuf.u4BufPA = (MUINTPTR)rNode->getPhyAddr();
