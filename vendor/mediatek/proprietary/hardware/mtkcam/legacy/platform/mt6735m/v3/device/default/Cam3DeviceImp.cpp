...
Cam3DeviceImp(
    ...
    , mpHalSensor(NULL)
    //
    , mpResourceMgr(IPipelineResourceMgr::create(i4OpenId))
    //
    , mHdrStatus(MFALSE)
    , mHdrStatusLock()
{
...
)
{
    ...
    //
    {
        ...
        params.pPipelineModel   = mAppContext.pPipeline = NULL;
        params.pipelineScene    = DEFAULT;
        mpResourceMgr = NULL;
        mpResourceMgr = IPipelineResourceMgr::create(getOpenId());
...
#if 1
{
    if( OK == find_camera_metadata_ro_entry(request->settings, ANDROID_CONTROL_CAPTURE_INTENT, &e) &&
        *e.data.u8 == ANDROID_CONTROL_CAPTURE_INTENT_STILL_CAPTURE &&
        OK == find_camera_metadata_ro_entry(request->settings, ANDROID_CONTROL_SCENE_MODE, &e) &&
        *e.data.u8 == ANDROID_CONTROL_SCENE_MODE_HDR )
    {
        if (mHdrStatus == MFALSE)
        {
            Mutex::Autolock _l(mHdrStatusLock);
            if (mHdrStatus == MFALSE){
                MERROR err = reConfigurePipeline(HDR);
                if  ( OK != err ) {
                    MY_LOGE("reConfigurePipeline error");
                    return err;
                }
                mHdrStatus = MTRUE;
            }
    ...
    else
    {
        if (mHdrStatus == MTRUE) {
            Mutex::Autolock _l(mHdrStatusLock);
            if (mHdrStatus == MTRUE){
                MERROR err = reConfigurePipeline(DEFAULT);
                if  ( OK != err ) {
                    MY_LOGE("reConfigurePipeline error");
                    return err;
                }
...
#if 0
    sp<IMetaStreamBuffer> pStreamBuffer  = appRequest.vInputMetaBuffers[0];
    ...
    if (!eSceneMode.isEmpty() && eSceneMode.tag() == MTK_CONTROL_SCENE_MODE_HDR &&
        !eCaptureIntent.isEmpty() && eCaptureIntent.tag() == MTK_CONTROL_CAPTURE_INTENT_STILL_CAPTURE)
    {
        if (mHdrStatus == MFALSE)
        {
            Mutex::Autolock _l(mHdrStatusLock);
...
status_t
Cam3DeviceImp::
reConfigurePipeline(PipelineScene scene)
{
    MY_LOGD("Configure pipeline(%d) +", scene);
    MERROR err = OK;
    {
        //waitUntilDrained(getSafeAppContext());
        if  ( mAppContext.pAppStreamManager != NULL ) {
            err = mAppContext.pAppStreamManager->waitUntilDrained(::ms2ns(5000));
            MY_LOGW_IF(OK!=err, "AppStreamManager::waitUntilDrained err:%d(%s)", err, ::strerror(-err));
        }
        if ( mAppContext.pPipeline != NULL ) {
            mAppContext.pPipeline = NULL;
        }
    }
    ...
    err = mAppContext.pAppStreamManager->queryConfiguredStreams(appStreams);
    if ( OK != err ) {
        MY_LOGE("mAppContext.pAppStreamManager error");
        return err;
    }

    {
        ...
        params.pipelineScene     = scene;
        err = ConfigurePipeline()(params);
        if ( OK != err ) {
            MY_LOGE("Configure pipeline failed");
            return err;
        }
        mAppContext.pPipeline = params.pPipelineModel;
        err = finishPipelineConfiguration(mAppContext.pPipeline);
