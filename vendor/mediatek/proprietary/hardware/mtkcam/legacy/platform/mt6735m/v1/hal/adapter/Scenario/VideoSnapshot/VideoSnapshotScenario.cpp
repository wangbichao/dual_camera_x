    ...
    ImageSize*          pImageSize)
{
    ...
    if(mSensorStaticInfo.sensorType == SENSOR_TYPE_YUV)
    {
    ...
    MY_LOGD("Sensor:Type(%d),BitDepth(%d),Order(%d),Format(%d)",
            ...
            mSensorStaticInfo.rawSensorBit,
    ...
    //
    run("VideoSnapshotScenario::init");
    //
    android_atomic_inc(&mUsers);
    //
    EXIT:
    ...
    MBOOL           enableSOI)
{
    ...
    // for YV12, the order is YVU, the the API for jpeg encode is (Y, U, V), so switch the U and V add for jpeg API
    ...
    pJpgEncoder->setSrcBufSize(
                    ...
                    yuvSize[2],
                    yuvSize[1],
                    ALIGN_SIZE(mJpgInfo[jpgType].width/2,YUV_IMG_STRIDE_U));
    // (3). set quality
...
readyToRun()
{
    ...
    int const priority  = NICE_CAMERA_VSS_PASS2;
    //
