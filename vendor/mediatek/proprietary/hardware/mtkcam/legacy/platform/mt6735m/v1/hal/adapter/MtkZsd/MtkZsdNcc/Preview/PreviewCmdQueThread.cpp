        ...
        switch (scenarioId)
        {
            ...
            case SENSOR_SCENARIO_ID_NORMAL_CAPTURE:
            {
                ...
                if(mpResMgrDrv)
                {
                    mpResMgrDrv->setItem(
                                    RES_MGR_DRV_ITEM_SENSOR_FPS,
                                    mSensorStaticInfo.captureFrameRate/10);
                }
                break;
            }
            default:
...
{
    CAM_TRACE_NAME("PrvCQT_ZNC::requestExit");
    FUNCTION_IN;
...
{
    CAM_TRACE_NAME("PrvCQT_ZNC::postCommand");
    FUNCTION_IN;
    //
    bool ret = true;
    //
...
start()
{
    CAM_TRACE_NAME("PrvCQT_ZNC::start");
    FUNCTION_IN;
    //
    bool ret = false;
    ...
    //
    if( ! (ret = mSensorInfo.init(SENSOR_SCENARIO_ID_NORMAL_CAPTURE)))
    {
    ...
    mp3AHal->setSensorMode(SENSOR_SCENARIO_ID_NORMAL_CAPTURE);
    CAM_TRACE_END();
...
delay(EQueryType_T type)
{
    ...
    switch (type)
    {
        case EQueryType_Init:
        {
            ...
            int failCnt = 0;
            CAM_TRACE_BEGIN("PrvCQT_ZNC::delay::init");
        ...
        case EQueryType_Effect:
        {
            ...
            int32_t delay3A = mp3AHal->getDelayFrame(type);
            int32_t count = 0;
            CAM_TRACE_BEGIN("PrvCQT_ZNC::delay::Effect");
        ...
        case EQueryType_AWB:
        {
            ...
            int32_t delay3A = mp3AHal->getDelayFrame(type);
            int32_t count = 0;
            CAM_TRACE_BEGIN("PrvCQT_ZNC::delay::AWB");
            for (count; count < delay3A; count++)
            {
...
setParameters()
{
    CAM_TRACE_NAME("PrvCQT_ZNC::setParameters");
    FUNCTION_IN;
...
stop()
{
    CAM_TRACE_NAME("PrvCQT_ZNC::stop");
    FUNCTION_IN;
    //
    bool ret = true;
...
precap()
{
    CAM_TRACE_NAME("PrvCQT_ZNC::precap");
    FUNCTION_IN;
    uint32_t bFlashOn = 0;
    ...
    if ( bFlashOn == 0 && (mShotMode == eShotMode_NormalShot || mShotMode == eShotMode_AsdShot) ) {
...
updateOne()
{
    CAM_TRACE_NAME("PrvCQT_ZNC::updateOne");
    ...
    for (MINT32 i =0; i< pass1RawReadyCnt; i++)
    {
        ...
        for (it = mvDequedImgBufNode.begin(); it != mvDequedImgBufNode.end(); ) {
            if ( (MUINTPTR)vDequedBuffer.at(i).virtAddr == (MUINTPTR)(*it).mainImgNode->getVirAddr()){
                    ...
                    list<CapBufQueNode> &vOutNode)
{
    ...
    for (MINT32 i =0; i < size; i++)
    {
        ...
        {
            if ( vQTBuf.at(0).bufInfo.vBufInfo[i].u4BufVA == (MUINTPTR)(*it).mainImgNode->getVirAddr())
            {
