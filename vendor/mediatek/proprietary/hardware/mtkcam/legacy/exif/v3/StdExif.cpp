...
)
{
    ...
    MY_LOGI("mpExifParam(0x%x) ImageSize(%dx%d) FNumber(%d/10) FocalLegth(%d/1000) AWBMode(%d) Strobe(%d) AEMeterMode(%d) AEExpBias(%d)",
    ...
    MY_LOGI("CapExposureTime(%d) AEISOSpeed(%d) LightSource(%d) ExpProgram(%d) SceneCapType(%d) Orientation(%d) ZoomRatio(%d) Facing(%d)",
        mExifParam.u4CapExposureTime, mExifParam.u4AEISOSpeed, mExifParam.u4LightSource, mExifParam.u4ExpProgram,
        mExifParam.u4SceneCapType, mExifParam.u4Orientation, mExifParam.u4ZoomRatio, mExifParam.u4Facing);
...
getHeaderSize() const
{
    ...
    MY_LOGI_IF(mLogLevel >= 1, "header size(%d) thumbSize(%d) getDbgExifSize(%d)", size, getThumbnailSize(), getDbgExifSize());
    return size;
}
