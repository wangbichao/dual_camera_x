...
//
#include <mtkcam/device/ExternalModuleBase.h>
...
getListener()
{
    ...
    if(mListener == NULL) {
        //mListener = IExternalDeviceListener::create();
 ...
 *
 ******************************************************************************/
/*IExternalDeviceListener*
IExternalDeviceListener::
create()
{
    return new ExternalDeviceListenerImp();
}*/
...
)
{
    ...
    //
    /*android::sp<CamDeviceManagerBase> mgr = mDeviceManager.promote();
    if( mgr == 0) {
        remoteID = -1;
        return -EINVAL;
    }*/
    ...
    return err;
#else
    MY_LOGE("Not support external devide");
    return -EINVAL;
#endif
}
...
)
{
    ...
    int32_t remoteID;
    //
    status_t err = ( mDeviceManager == 0) ? -EINVAL : mDeviceManager->removeExternalDeviceLocked(deviceID, remoteID);
    //
    if ( err == OK )
