...
#if 1
        {
            mvFutures.push_back(
                std::async(std::launch::async,
                    [ this ]() {
                        MY_LOGD("Previous pipeline(%s:%p) to wait drained",
                                mConfigParams.pPipelineModel->getName(), mConfigParams.pPipelineModel.get());
                        MERROR err = OK;
                        sp<IPipelineModel> pPipeline = mConfigParams.pPipelineModel;
                        if ( ! pPipeline.get() )
                            err = -ENODEV;
                        else {
