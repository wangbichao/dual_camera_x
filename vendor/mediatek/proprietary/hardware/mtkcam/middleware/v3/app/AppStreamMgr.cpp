...
{
    return new AppStreamMgr(openId, callback_ops, pMetadataProvider, isExternal);
}
...
AppStreamMgr(
    ...
    , mResultQueue()
    //
    , mFrameHandler(new FrameHandler(pMetadataProvider, isExternal))
    , mFrameHandlerLock()
    , mFrameHandlerCond()
...
checkStream(camera3_stream* stream) const
{
    ...
    if ( CAMERA3_STREAM_ROTATION_0 != stream->rotation ) {
        MY_LOGI("stream format:0x%x w/ rotation:%d", stream->format, stream->rotation);
...
checkStreams(camera3_stream_configuration_t *stream_list) const
{
    ...
    //
    KeyedVector<int, size_t> outRotationNum;
    outRotationNum.add(CAMERA3_STREAM_ROTATION_0,      0);
    outRotationNum.add(CAMERA3_STREAM_ROTATION_90,     0);
    outRotationNum.add(CAMERA3_STREAM_ROTATION_180,    0);
    outRotationNum.add(CAMERA3_STREAM_ROTATION_270,    0);
    //
    ...
    {
        // we cannot decide whether it's resonalbe for isp to generate more than 1 oupout stream w/ rotation value here
        // it should be determined by pipelinemodel configuration step to predict pass2 worst case scenario
...
)   const
{
    ...
    MUINT usageForAllocator = usageForConsumer
                            ...
                            ;
#else // for reprocessing
    MUINT usageForAllocator = usageForConsumer
                            | (GRALLOC_USAGE_SW_READ_OFTEN|GRALLOC_USAGE_SW_WRITE_OFTEN)
                            | GRALLOC_USAGE_HW_CAMERA_WRITE | (stream->stream_type == CAMERA3_STREAM_INPUT ? GRALLOC_USAGE_HW_CAMERA_READ : 0)
    ...
    //
    MUINT32 transform = (stream->rotation==CAMERA3_STREAM_ROTATION_90)? HAL_TRANSFORM_ROT_270 :
                        (stream->rotation==CAMERA3_STREAM_ROTATION_180)? HAL_TRANSFORM_ROT_180 :
                        (stream->rotation==CAMERA3_STREAM_ROTATION_270)?  HAL_TRANSFORM_ROT_90 :
                        0;
    ...
    MY_LOGD_IF(
        1,
        "[%#" PRIxPTR " %s] stream:%p->%p %dx%d type:%d"
        ...
        pStream->getImgSize().w, pStream->getImgSize().h, stream->stream_type,
        stream->rotation, transform, stream->data_space,
...
performCallback(CallbackParcel const& cbParcel)
{
    ...
    if  ( ! cbParcel.vOutputImageItem.isEmpty() || ! cbParcel.vInputImageItem.empty() )
    {
        ...
        for (size_t i = 0; i < cbParcel.vOutputImageItem.size(); i++) {
            ...
            if  ( ATRACE_ENABLED() )
            {
                auto isTarget = [&](MUINT const usage) {
                    return  0 != (rDst.stream->usage & usage)
                        &&  CAMERA3_BUFFER_STATUS_OK == rDst.status
                        &&  -1 == rDst.release_fence
                        &&  0 != cbParcel.timestampShutter
                            ;
                };

                auto traceTarget = [&, this](char const* szPrefix, MUINT const usage, String8& str) {
                    if  ( isTarget(usage) ) {
                        str = String8::format(
                            "Cam:%d:Fwk:%s|timestamp(ns):%" PRId64 " duration(ns):%" PRId64 " u:%#x %dx%d request:%d",
                            mOpenId, szPrefix,
                            cbParcel.timestampShutter, ::systemTime()-cbParcel.timestampShutter,
                            rDst.stream->usage, rDst.stream->width, rDst.stream->height,
                            cbParcel.frameNo
                        );
                        return MTRUE;
                    }
                ...
                //Trace display
                (MVOID)(traceTarget("Hwc", GRALLOC_USAGE_HW_COMPOSER, s8Trace_HwComposer)
        ...
        if  ( ! s8Trace_HwComposer.isEmpty() ) {
            CAM_TRACE_BEGIN(s8Trace_HwComposer.string());
        }
        else if  ( ! s8Trace_HwTexture.isEmpty() ) {
            CAM_TRACE_BEGIN(s8Trace_HwTexture.string());
        }
        mpCallbackOps->process_capture_result(mpCallbackOps, &result);
        if  ( ! s8Trace_HwComposer.isEmpty() || ! s8Trace_HwTexture.isEmpty() ) {
            CAM_TRACE_END();
        }
    }
    //
