...
init()
{
    ...
    mpFlowControl = IFlowControl::createInstance(
                            "StereoCamAdapter",
                            getOpenId(),
                            IFlowControl::ControlType_T::CONTROL_STEREO,
                            mpParamsManagerV3,
    ...
    mpShot = new StereoShot("SShot", NSCam::eShotMode_ZsdShot, 312, mpFlowControl);
    ret = ((StereoFlowControl*)mpFlowControl.get())->setCallbacks(mpShot);
    mpShot->setCallback(cb);
lbExit:
    if(!ret)
    {
...
)
{
    switch(_msgType)
    {
        case MTK_CAMERA_MSG_EXT_DATA_AF:
        {
            ...
            camera_memory* pmem = mpCamMsgCbInfo->mRequestMemory(-1, sizeof(int32_t)*(_size+1), 1, NULL);
            if (pmem)
            {
