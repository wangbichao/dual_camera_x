...
execute()
{
    ...
    MY_LOGD_IF( 1, "[out]heapVA:0x%x, imgbufferVA:0x%x ",
                    mpEncodeFrame->mpOutImgBufferHeap->getBufVA(0),
...
)
{
    ...
    //
    {
        ...
        MY_LOGD( "heap size = %d, mainOffset = %d, mainMaxSize = %d",
                 rpImageBufferHeap->getBufSizeInBytes(0), mainOffset, mainMaxSize);
        //
...
)
{
    ...
    MY_LOGD( "thumbnail size: params(%dx%d) imagebuffer(%dx%d) exifsize(%d)",
...
)
{
    ...
    // do encode
    {
        ...
        if( err != OK ) {
            MY_LOGE( "thumb encode fail src %x, type 0x%x, dst %x, type 0x%x",
                     rParams.pSrc, rParams.pSrc->getImgFormat(),
                     rParams.pDst->getBufVA(0), rParams.pDst->getImgFormat());
...
{
    FUNC_START;
    MY_LOGD( "src(%p:0x%x) %dx%d, dst(%p:0x%x) %dx%d, format = 0x%x",
             rParams.pSrc, rParams.pSrc->getBufVA(0),
             rParams.pSrc->getImgSize().w, rParams.pSrc->getImgSize().h,
             rParams.pDst, rParams.pDst->getBufVA(0),
             rParams.pDst->getImgSize().w, rParams.pDst->getImgSize().h,
