...
void
ZipImageCallbackThread::
setShotMode(uint32_t const u4ShotMode, const IState::ENState nextState, String8 const s8AppMode)
{
    MY_LOGD("set shot mode %d->%d nextState(%d)", mu4ShotMode, u4ShotMode, nextState);
...
addCallbackData(callback_data* pData)
{
    ...
    MY_LOGD("addCallback type(%d)",pData->type);
...
processCallback(callback_data& one)
{
    ...
    switch(one.type)
    {
        ...
        case callback_type_shutter:
        {
            ...
            if(one.bMtkExt)
            {
                ret = handleShutter(one.i4PlayShutterSound);
            }
            else
            {
                ret = handleShutter(false);
            }
            MY_LOGD("(%d)[ShutterThread] -", ::gettid());
            return ret;
        }
        ...
        {
            return handleMetadataRaw16Callback(one);
        }
        case callback_type_postview:
        {
            return handlePostviewCallback(one);
        }
...
handleP2doneNotify()
{
          ...
          mu4ShotMode == eShotMode_FaceBeautyShotCc )
    {
        ...
        if( !mbForceExit )
        {
            ...
            {
                MY_LOGW("handleP2doneNotify do nothing due to fail to wait - ShotMode:%d", mu4ShotMode);
                return  false;
            }
        }
    }
...
bool
ZipImageCallbackThread::
handleMetadataRaw16Callback(callback_data & data)
{
    CAM_TRACE_CALL();
    if( mbForceExit )
    {
...
bool
ZipImageCallbackThread::
handlePostviewCallback(callback_data const& data)
{
    CAM_TRACE_CALL();
    if( mbForceExit )
    {
    ...
    {
        MY_LOGD("CAMERA_MSG_POSTVIEW_FRAME");
        mpCamMsgCbInfo->mDataCb(
