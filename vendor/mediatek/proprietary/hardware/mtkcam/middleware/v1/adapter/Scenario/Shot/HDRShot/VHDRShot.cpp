...
Mutex VHDRShot::sPostProcessingDoneLock;
...
bool VHDRShot::zsdCapture()
{
    ...
    Vector<MINT32> resultRequestNo;
    {
        ...
        selector->queryCollectImageStreamInfo(rawInputInfos);
...
status_t VHDRShot::createPipeline(const sp<StreamBufferProvider>& provider)
{
    ...
    else
    {
        ...
        for(size_t i = 0; i < rawInputInfos.size() ; i++)
        {
            ...
            sp<CallbackBufferPool> pPool = prepareCallbackBufferPool(mpCallbackHandler, pStreamInfo, false);
            bufferProvider->setImageStreamInfo(pStreamInfo);
            ...
            imgSrcParam.push_back(imageParam);
        }

        if (OK != builder->setSrc(imgSrcParam))
        {
        ...
        if (selector.get())
        {
            for (MfcSelector::ResultSet& resultSet : mpSelectedResult)
            {
                HDR_LOGD("return selected buffer heap(%s)",
                        ISelector::logBufferSets(resultSet.bufferSet));
