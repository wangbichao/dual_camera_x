...
{
    miOpenId = openId;
    MY_LOGD("ctor(0x%x) openId(%d)", this, miOpenId);
}
/******************************************************************************
 *
 ******************************************************************************/
...
)
{
    ...
    //
    MY_LOGD("name(%s) streamId(0x%x) size(%dx%d)",
...
)
{
    ...
    MY_LOGW("Can not find buffer provider. streamId(0x%x)", streamId);
    return nullptr;
}
/******************************************************************************
 *
...
{
    status_t ret = UNKNOWN_ERROR;
    for(ssize_t i = 0;i<mvBufferProviderPool.size();++i)
    {
...
)
{
    ...
    if(index >= 0)
    {
        MY_LOGD("edit buffer provider (0x%x)", streamId);
        mvBufferProviderPool.replaceValueAt(index, provider);
    }
    else
    {
        MY_LOGD("add new buffer provider (0x%x)", streamId);
