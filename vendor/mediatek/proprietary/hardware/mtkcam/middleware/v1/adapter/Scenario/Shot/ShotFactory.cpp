...
)
{
    // if refocus mode or vsdof mode is on, using stereo shot.
        MBOOL const isStereoShot = (::strcmp(pParamsMgr->getStr(MtkCameraParameters::KEY_STEREO_REFOCUS_MODE), MtkCameraParameters::ON) == 0)||
                                   (::strcmp(pParamsMgr->getStr(MtkCameraParameters::KEY_STEREO_VSDOF_MODE), MtkCameraParameters::ON) == 0) ? MTRUE : MFALSE;
    ...
    case eShotMode_HdrShot:
        rpShot = createHdrShotInstance(u4ShotMode, i4OpenId, pParamsMgr);
        break;
    //
