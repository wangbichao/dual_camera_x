...
updateUserTypeParams(String8 const& s8KeyName, NSCameraFeature::FeatureInfo const& rFeatureInfo)
{
    ...
    //
    if  ( s8KeyName == MtkCameraParameters::KEY_SINGLE_FRAME_CAPTURE_HDR_SUPPORTED )
    {
        return  updateSingleFrameCaptureHDRParams(rFeatureInfo);
    }
    //
    if  ( s8KeyName == MtkCameraParameters::KEY_SUPPORT_MZAF_FEATURE )
    {
        return  updateSensorSupportMZAF(rFeatureInfo);
    }
    //
    if  ( s8KeyName == MtkCameraParameters::KEY_HDR_DETECTION_SUPPORTED )
    {
        return  updateHDRDetectionParams(rFeatureInfo);
    }
    //
    return  false;
}
...
bool
ParamsManager::
updateSingleFrameCaptureHDRParams(NSCameraFeature::FeatureInfo const& rFeatureInfo)
{
    const char *rs8Supported = rFeatureInfo.getDefaultItem().string();
    //
    MY_LOGD_IF(1, "%s=%s",
...
bool
ParamsManager::
updateSensorSupportMZAF(NSCameraFeature::FeatureInfo const& rFeatureInfo)
{
    String8 const& rs8Supported = rFeatureInfo.getDefaultItem();
    //
    MY_LOGD_IF(1, "MZAF %s=%s", MtkCameraParameters::KEY_SUPPORT_MZAF_FEATURE, rs8Supported.string());
    //
    mParameters.set(MtkCameraParameters::KEY_SUPPORT_MZAF_FEATURE, rs8Supported.string());
    MY_LOGD_IF(mEnableDebugLog, "-");
    //
    return  true;
}
...
bool
ParamsManager::
updateHDRDetectionParams(NSCameraFeature::FeatureInfo const& rFeatureInfo)
{
    String8 const& rs8Supported = rFeatureInfo.getDefaultItem();
    //
    MY_LOGD_IF(1, "%s=%s", MtkCameraParameters::KEY_HDR_DETECTION_SUPPORTED, rs8Supported.string());
    //
    mParameters.set(MtkCameraParameters::KEY_HDR_DETECTION_SUPPORTED, rs8Supported.string());
    MY_LOGD_IF(mEnableDebugLog, "-");
    //
    return true;
}
...
updateDefaultParams0()
{
    ...
    mParameters.set(CameraParameters::KEY_SUPPORTED_JPEG_THUMBNAIL_SIZES, "0x0,160x128");
...
//    mParameters.set(MtkCameraParameters::KEY_ISP_MODE, 0);     //0: normal
    ...
    // Enable MZAF
    mParameters.set(MtkCameraParameters::KEY_MZAF_ENABLE, CameraParameters::FALSE);
    //
    MY_LOGD("MZAF set default false");
    mParameters.set(MtkCameraParameters::KEY_SUPPORT_MZAF_FEATURE, CameraParameters::FALSE);
    //----------------------------------------------------------------------------------------------
    //  MTK proprietary parameters
    //----------------------------------------------------------------------------------------------
    MY_LOGD_IF(mEnableDebugLog, "-");
...
evaluateHalAppModeUtility(String8& mode) const
{
    ...
    if  ( i4CamMode == MtkCameraParameters::CAMERA_MODE_NORMAL )
    {
        ...
        // zsd: only for test
        {
            ...
            MY_LOGD("CAMERA_MODE_NORMAL: zsd mode %d", i4ZsdMode);
