...
#include <pip/pip_hal.h>   // For PIP Hal GetMaxFrameRate().
...
#undef FALSE
        ...
        const int32_t debugHDRDetection =
            property_get_int32("mediatek.hdrdetection.debug", -1);
        ...
        const int32_t debugCaptureHDRMode =
            property_get_int32("mediatek.hdr.capture.mode", -1);
...
#undef FALSE
    ...
    MY_LOGD("u4MaxMeterAreaNum(%d)", r3ASupportedParam.u4MaxMeterAreaNum);
...

    {
    #if '1'==MTKCAM_HAVE_SENSOR_HAL
        int st, ed;
        IHalFlash*const pHalFlash = MAKE_HalFlash();
        pHalFlash->egGetDutyRange(getOpenId(), &st, &ed);
        ...
        pHalFlash->egGetStepRange(getOpenId(), &st, &ed);
        ...
        pHalFlash->egSetMfDutyStep(getOpenId(), MtkCameraParameters::KEY_ENG_FLASH_DUTY_DEFAULT_VALUE, mParameters.getInt(MtkCameraParameters::KEY_ENG_FLASH_STEP_MAX)); // Default values for flash
    #endif
    }
    //
