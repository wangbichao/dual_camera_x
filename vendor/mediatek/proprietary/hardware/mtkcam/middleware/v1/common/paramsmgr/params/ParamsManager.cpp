...
ParamsManager::updateLogStatus()
{
    ...
    int len = property_get("debug.paramsmgr.log", val, NULL);
    if(len > 0 &&
       !strncmp(val, "1", 1))
    {
...
#if 1
    const char* strSceneMode =
        mParameters.get(CameraParameters::KEY_SCENE_MODE);
    const char* strHDRAutoMode =
        mParameters.get(MtkCameraParameters::KEY_HDR_AUTO_MODE);
    const char* strVideoHDR =
        mParameters.get(MtkCameraParameters::KEY_VIDEO_HDR);

    // scene-mode ≠ SCENE_MODE_HDR
    if (!strSceneMode || strcmp(strSceneMode, CameraParameters::SCENE_MODE_HDR))
        return HDRMode::OFF;

    // hdr-auto-mode ≠ on
    if (!strHDRAutoMode || strcmp(strHDRAutoMode, MtkCameraParameters::ON))
        return
            !strVideoHDR ? HDRMode::ON :
            !strcmp(strVideoHDR, MtkCameraParameters::ON) ? HDRMode::VIDEO_ON :
            HDRMode::ON;

    // hdr-auto-mode = on
    return
        !strVideoHDR ? HDRMode::AUTO :
        !strcmp(strVideoHDR, MtkCameraParameters::ON) ? HDRMode::VIDEO_AUTO :
        HDRMode::AUTO;
#else
