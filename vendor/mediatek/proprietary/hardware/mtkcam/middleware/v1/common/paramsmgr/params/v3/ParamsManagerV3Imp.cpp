...
set(/*MtkCameraParameters const& paramString, */my_params& params) {
        ...
        else
        {
            if(mpParamsMgr->getRecordingHint() && mpParamsMgr->getVideoStabilization())
            {
                ...
                if (((videoWidth*videoHeight) >= (VR_UHD_W*VR_UHD_H)) &&
                    EISCustom::isEnabledEIS25_4k2k() &&
                    mpParamsMgr->getVHdr() == SENSOR_VHDR_MODE_NONE)
                {
                    if (EISCustom::isEnabledFixedFPS())
                    {
                        params.mPreviewFpsRange[0] = EISCustom::getEIS25_4k2k_VR_FPS();
                    }else
                    {
                        params.mPreviewFpsRange[0] = 5;
                    }
                    params.mPreviewFpsRange[1] = EISCustom::getEIS25_4k2k_VR_FPS();
                }
    ...
    mMZAFEnable = 0;
    MBOOL isMzafEnable = ( ::strcmp(newParams.get(MtkCameraParameters::KEY_MZAF_ENABLE), CameraParameters::TRUE) == 0 ) ? MTRUE : MFALSE;
    MBOOL isMzafSupport = ( ::strcmp(newParams.get(MtkCameraParameters::KEY_IS_SUPPORT_MZAF), CameraParameters::TRUE) == 0 ) ? MTRUE : MFALSE;
    if (isMzafEnable && isMzafSupport)
    {
        mMZAFEnable = 1;
    }
    MUINT32 forceEnableMZAF = ::property_get_int32("debug.enablemzaf.enable", -1);
...
status_t ParamsManagerV3Imp::calculateCropRegion(MINT32 sensorMode)
{
    ...
    mSensorSizeCrop.s.w = sensorSize.w * 100 / zoomRatio;
    mSensorSizeCrop.s.h = sensorSize.h * 100 / zoomRatio;
    mSensorSizeCrop.p.x = (sensorSize.w - mSensorSizeCrop.s.w) >> 1;
    mSensorSizeCrop.p.y = (sensorSize.h - mSensorSizeCrop.s.h) >> 1;
