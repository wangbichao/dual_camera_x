...
#if 1
    {
        String8 str = String8::format("requestNo:%u userId:%#" PRIxPTR " streamId:%#" PRIxPTR " error:%d"
...
)
{
    ...
    if  ( 0 <= index ) {
        MY_LOGW("requestNo:%u existed @ index:%zd frameEnd:%d", requestNo, index, result.bFrameEnd);
        //
...

    {
        for ( size_t i = 0; i < rvEarlyResult.size(); ++i ) {
            ...
            while ( item != mEarlyListenerList.end() ) {
                ...
                MY_LOGD_IF(
                    mLogLevel >= 1,
                    "[early meta] requestNo:%d streamId:%d error:%d",
...
)
{
    for (size_t i = 0; i < rvResult.size(); i++) {
        ...
        for (; item != callbackList.end(); item++) {
            ...
            //
            CAM_TRACE_FMT_BEGIN("PartCBtoListner No%d,ID%d",(*item)->requestNo, (*item)->streamId);
...
handleFullResultCallback()
{
    for (size_t i = 0; i < mFullResultQueue.size(); i++) {
        ...
        for (; item != callbackList.end(); item++) {
            ...
            //
            CAM_TRACE_FMT_BEGIN("FullCBtoListner No%d,ID%d",(*item)->requestNo, (*item)->streamId);
            (*item)->listener->onResultReceived(
                                    (*item)->requestNo,
                                    (*item)->streamId,
                                    (*item)->errorResult,
