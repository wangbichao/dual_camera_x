...
handleReturn()
{
    ...
    while ( i < mResultSetMap.size() ) {
        ...
        if( (r->heap != NULL && r->timestamp >= 0)
            ...
            if ( mpConsumer != 0 ) {
                MY_LOGD2("[%d] To selector %p timestamp:%lld hasTimeStampInfo:%d",
            ...
            {
                MY_LOGD1("i(%d/%d) release reqNo(%d) buf:%p TS:%lld err:%d TSInfo:%d",
                        i,
...
)
{
    ...
    if(!bFind)
    {
        ...
        for ( size_t i = 0; i < mConsumerMap.size(); ++i ) {
            MY_LOGW("Current: [%d/%d][%d] error:%d timestamp:%lld heap:%p",
                i, mConsumerMap.size(),
                mConsumerMap[i].requestNo,
                mConsumerMap[i].error,
                mConsumerMap[i].timestamp,
...
{
    MY_LOGD1("reqNo(%d),err(%d),TS(%lld)", requestNo, errorResult, timestamp);
    //
    Vector<ResultSet_T> tempSetMap;
    tempSetMap.push_back(
        ResultSet_T{
