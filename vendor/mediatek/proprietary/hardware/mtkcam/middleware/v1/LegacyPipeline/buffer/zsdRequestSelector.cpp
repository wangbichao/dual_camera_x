...
)
{
    ...
    MY_LOGD_IF( 1, "selectResult error:%d rRequestNo(%d) %s ", errorResult, rRequestNo, ISelector::logBufferSets(rvBuffers));
    ...
    for ( size_t i = 0; i < mWaitRequestNo.size(); ++i ) {
        ...
        if( req == rRequestNo) {
            ...
            MY_LOGD_IF( 1 , "get result No(%d) %s",rRequestNo, ISelector::logBufferSets(rvBuffers));
...
)
{
    ...
    if ( mResultSet.isEmpty() ) {
        ...
        if(status != OK) {
            MY_LOGE("wait result timeout.... get latest result %d %s", mLatestResult.requestNo, ISelector::logBufferSets(mLatestResult.bufferSet));
    ...
    MY_LOGD_IF( mLogLevel >= 1, "return mLatestResult buf:%p", ISelector::logBufferSets(mLatestResult.bufferSet));
    returnBuffersToPool(mLatestResult.bufferSet);

    mLatestResult.bufferSet.clear();
    mLatestResult.resultMeta.clear();

    MY_LOGD_IF( mLogLevel >= 1, "getResult %d %s", rRequestNo, ISelector::logBufferSets(rvBuffers));
