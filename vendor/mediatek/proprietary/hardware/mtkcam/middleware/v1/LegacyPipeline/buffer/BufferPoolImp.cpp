...
{
    MY_LOGI(
        "[%#" PRIxPTR " %s] stream:%dx%d format:%#x transform:%d",
    ...
    MY_LOGI("logLevel:%d Max:%d Min:%d future:%d inUse:%d available:%d",
...
)
{
    ...
    if ( minNumberOfInitialCommittedBuffers > maxNumberOfBuffers) {
        MY_LOGE("mMinBuffer:%d > mMaxBuffer:%d", minNumberOfInitialCommittedBuffers, maxNumberOfBuffers);
        return UNKNOWN_ERROR;
    }
