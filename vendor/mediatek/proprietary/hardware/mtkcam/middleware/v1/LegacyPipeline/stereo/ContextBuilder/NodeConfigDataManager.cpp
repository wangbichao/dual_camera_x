...
#include "NodeData/BMDeNoiseNodeConfigData.h"
#include "NodeData/BMDepthMapNodeConfigData.h"
#include "NodeData/BMPreProcessNodeConfigData.h"
#endif
//
/******************************************************************************
 *
...
createNodeActor(NodeId_T nodeId, DefaultKeyedVector<NodeId_T, sp<INodeConfigBase> >& nodeList)
{
    ...
    else if(eNODEID_DualYuvNode == nodeId)
        nodeList.add(nodeId, new DualYUVNodeConfigData(mSensorId, nodeId, "DualYUVNode"));
#if (MTKCAM_STEREO_DENOISE_SUPPORT == '1')
    else if(eNODEID_BMDeNoiseNode == nodeId)
        nodeList.add(nodeId, new BMDeNoiseNodeConfigData(mSensorId, nodeId, "BMDeNoiseNode"));
    else if(eNODEID_BMDepthMapNode == nodeId)
        nodeList.add(nodeId, new BMDepthMapNodeConfigData(mSensorId, nodeId, "BMDepthMapNode"));
    else if(eNODEID_BMPreProcessNode == nodeId)
        nodeList.add(nodeId, new BMPreProcessNodeConfigData(mSensorId, nodeId, "BMPreProcessNode"));
#endif
    else
