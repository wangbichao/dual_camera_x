            ...
            MBOOL                 /*errorResult*/)
{
    ...
    CAM_LOGD("selectResult requestNo(%d) heap(%s)", requestNo, ISelector::logBufferSets(rvBuffers));
    ...
    if (found)
    {
        ...
        CAM_LOGD("selectResult requestNo(%d) heap(%s) ts(%lld)",
                requestNo, ISelector::logBufferSets(rvBuffers), timestamp);

        {
            ...
            CAM_LOGD("selectResult requestNo(%d) heap(%s) BufferInUse(%zu)",
                    requestNo, ISelector::logBufferSets(rvBuffers), mBufferInUse);
        }
    }
    else
    {
            ...
            Vector<BufferItemSet>&  rvBuffers)
{
    ...
    if (mResultSet.isEmpty())
    {
...

        {
            ...
            {
                CAM_LOGE("no incoming result after %lld second",
                        ns2s(timeout_select_result_ns));
                return WOULD_BLOCK;
            }
        }

        CAM_LOGD("no result, start waiting %lld millisecond", ns2ms(timeout_ns));
        mCondResultSet.waitRelative(mResultSetLock, timeout_ns);
        if (CC_UNLIKELY(mResultSet.isEmpty()))
        {
            CAM_LOGE("timeout after %lld millisecond, no result can get",
                    ns2ms(timeout_ns));
            return TIMED_OUT;
        }
    }
...

    {
        ...
        CAM_LOGD("getResult heap(%s) BufferInUse(%zu)",
                ISelector::logBufferSets(rvBuffers), mBufferInUse);
    }
...
status_t MfcSelector::flushLocked()
{
    ...
    if (!mResultSet.isEmpty())
    {
        ...
        for (size_t i = 0; i < mResultSet.size(); ++i)
        {
            ...
            CAM_LOGD("flush requestNo(%d) heap(%s) ts(%lld)",
                    resultSet->requestNo, ISelector::logBufferSets(resultSet->bufferSet),
                    resultSet->timestamp);
