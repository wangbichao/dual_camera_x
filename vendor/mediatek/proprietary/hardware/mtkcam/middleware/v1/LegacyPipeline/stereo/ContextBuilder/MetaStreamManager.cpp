...
getStreamInfoByStreamId(StreamId_T id)
{
    if(mvMetadataContainer.size() > 0)
    {
        ...
        {
            MY_LOGW("StreamId_T: (%d) is not contained in mvMetadataContainer.", id);
            return NULL;
        }
    }
...
getPoolTypeByStreamId(StreamId_T id)
{
    if(mvMetadataPoolContainer.size() > 0)
    {
        ...
        {
            MY_LOGW("StreamId_T: (%d) is not contained in mvMetadataPoolContainer.", id);
...
dump()
{
    ...
    if(size > 0)
    {
        ...
        {
            MY_LOGD("StreamId (%d) MetaStreamInfo (0x%x) StreamType (%d)",
            ...
            if(temp != NULL)
            {
                MY_LOGD("Name(%s) StreamId(%d) StreamType(%d) MaxBuf(%d) MinInitBuf(%d)",
                        temp->getStreamName(),
                        temp->getStreamId(),
                        temp->getStreamType(),
                        temp->getMaxBufNum(),
