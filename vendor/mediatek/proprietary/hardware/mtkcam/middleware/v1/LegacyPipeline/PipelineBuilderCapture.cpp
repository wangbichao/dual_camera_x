...
#define LOG_TAG "MtkCam/PipeBuilderCapture"
...
struct PipelineBuilderCapture::myConfig
...
MBOOL updateYuv(Param& rParam) {
    ...
    //
    if( pYuv == 0 && pYuvJpeg == 0 && pYuvThumbnail == 0) return MTRUE;
    ASSERT_NOT_NULL(pImgoRaw, "should config imgo");
    //
    if(!rParam.bMemoryIn)
    {
    ...
    if ( pLcsoRaw != NULL)
        rParam.pMapP2->addIn  ( pLcsoRaw->getStreamId() );
    if ( pYuv != 0 ) rParam.pMapP2->addOut ( pYuv->getStreamId() );
    if ( pYuvJpeg != 0 ) rParam.pMapP2->addOut ( pYuvJpeg->getStreamId() );
    if ( pYuvThumbnail != 0 ) rParam.pMapP2->addOut ( pYuvThumbnail->getStreamId() );
    return MTRUE;
}
...
)
{
    ...
    // Node
    {
        ...
        NodeT::ConfigParams cfgParam;
        {
            ...
            cfgParam.enableCaptureFlow  = mConfigParams.optimizedCaptureFlow;
...
)
{
    ...
    // Node
    {
        ...
        {
            MY_LOGD("P2 time sharing");
            pNode = new MyNodeActorT( NodeT::createInstance(P2Node::PASS2_TIMESHARING) );
        }
        else
        {
            pNode = new MyNodeActorT( NodeT::createInstance() );
        }
        pNode->setInitParam(initParam);
        pNode->setConfigParam(cfgParam);
        //
...
create()
{
    ...
    MUINT32 configFlag = 0;
    {
            ...
            if(OK != configureP2Node(pPipelineContext, configFlag))
                MY_LOGW("configureP2Node failed");
    ...
    //
    {
        // Image input image
        {
            ...
            if ( !querySrcParam(params) ) {
                ...
                {
                    MY_LOGD("Src buffer provider StreamId:%d",params[i].pInfo->getStreamId() );
        ...
        // JPEG image
        {
            ...
            if ( !queryJpegDst(vParams) ) {
                ...
                for ( size_t i = 0; i < vParams.size(); ++i ) {
                    if ( vParams[i].pProvider != 0 ) {
                        MY_LOGD("JPEG buffer provider StreamId:%d",vParams[i].pInfo->getStreamId() );
                        pTimestampProcessor->registerCB(vParams[i].pProvider);
                    }
                }
            }
