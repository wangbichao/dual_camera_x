...
#define addBufPlane(planes, height, stride)                                      \
    ...
    switch( imgFormat ) {
        ...
        case eImgFmt_BLOB:
            ...
            break;
        default:
            MY_LOGE("streamId= %#" PRIxPTR " format not support yet %d", streamId, imgFormat);
            break;
    }
#undef  addBufPlane
...
getPoolTypeByStreamId(StreamId_T id)
{
    if(mvImageStreamPoolTypeContainer.size() > 0)
    {
        ...
        {
            MY_LOGW("StreamId_T: (%d) is not contained in mvImageStreamPoolTypeContainer.", id);
...
getBufProviderByStreamId(StreamId_T id)
{
    if(mvImageStreamPoolContainer.size() > 0)
    {
        ...
        {
            MY_LOGW("StreamId_T: (%d) is not contained in mvImageStreamPoolContainer.", id);
            return NULL;
        }
    }
...
getStreamInfoByStreamId(StreamId_T id)
{
    if(mvImageStreamContainer.size() > 0)
    {
        ...
        {
            MY_LOGW("StreamId_T: (%d) is not contained in mvImageStreamContainer.", id);
            return NULL;
        }
    }
...
dump()
{
    ...
    if(size > 0)
    {
        ...
        {
            MY_LOGD("StreamId (%x) ImageStreamInfo (0x%x) StreamType (%d) BufProvide(0x%x)",
            ...
            if(temp != NULL)
            {
                MY_LOGD("Name(%s) StreamId(%x) StreamType(%d) MaxBuf(%d) MinInitBuf(%d)",
                        temp->getStreamName(),
                        temp->getStreamId(),
                        temp->getStreamType(),
                        temp->getMaxBufNum(),
    ...
    for(MUINT32 i=0; i<size; i++)
    {
        ...
        {
            MY_LOGD(" - MetaStreamId(%d)", pSet->vMetaStream[j]);
        }
    }
    MY_LOGD("-");
}
...
)
{
    ...
    if((index = mvImageStreamContainer.indexOfKey(id)) < 0) {
        MY_LOGW("StreamID(%d) doest not exist in container!!", id);
        return BAD_VALUE;
    }
    if(getPoolTypeByIndex(index) != NSPipelineContext::eStreamType_IMG_HAL_PROVIDER)
    {
        MY_LOGW("StreamID(%d) buffer-type(%d) is not HAL_PROVIDER!!", id, getPoolTypeByIndex(index));
        return BAD_VALUE;
    }
...
)
{
    ...
    if(index>=0)
    {
        ...
        MY_LOGD("Update stream info stremaId(%d)", id);
        mvImageStreamContainer.replaceValueFor(id, pInfo);
    }
    else
    {
        MY_LOGE("StreamId(%d) is not exist in table.");
        return MFALSE;
    }
    return MTRUE;
}
