...
startOne(SensorParam const & rSensorParam, StreamBufferProvider* pProvider, IMetadata* rOverwrite)
{
    ...
    for(size_t i = 0; i < bufferSet.size() ; i++)
    {
        MY_LOGD("Ready to returnBuffer(0x%" PRIxPTR ")(%p)", bufferSet[i].id, bufferSet[i].heap.get());
...
createStreams(StreamBufferProvider* pProvider)
{
    ...
    else
    {
        ...
        {
            MY_LOGD("zsd raw(%zu) stream %#" PRIxPTR ", (%s) size(%dx%d), fmt 0x%x",
                i,
...
#if 1
    LPBConfigParams.optimizedCaptureFlow = (getShotMode() == eShotMode_NormalShot|| getShotMode() == eShotMode_EngShot) ? MTRUE : MFALSE;
#endif
    //
...
{
    CAM_TRACE_FMT_BEGIN("onDataReceived No%d,streamId%" PRIxPTR ,requestNo,streamId);
