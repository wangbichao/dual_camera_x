...
{
    SingleShot* pSingleShotImp = new SingleShot(eShotMode, szCamShotName, ePipelineMode);
    if  ( ! pSingleShotImp )
    {
...
{
    Mutex::Autolock _lock(mLock);
    return  getImp()->startOne(rSensorParam, pProvider, rOverwrite);
}
...
{
    Mutex::Autolock _lock(mLock);
    return  getImp()->getResultMetadata(rAppResultMetadata, rHalResultMetadata);
}
