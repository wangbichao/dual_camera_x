...
//
//#include <featureio/stereo_hal_base.h>
//
// buffer usage
//
// for debug dump
...
startOne(SensorParam const & rSensorParam)
{
    ...
    //query flash on/off
    {
        IHal3A* pHal3A = MAKE_Hal3A(mSensorParam.u4OpenID, LOG_TAG);
        if( pHal3A == NULL ) {
        ...
        MBOOL isYuv =
            MAKE_HalSensorList()->queryType( mSensorParam.u4OpenID ) == NSCam::NSSensorType::eYUV;
