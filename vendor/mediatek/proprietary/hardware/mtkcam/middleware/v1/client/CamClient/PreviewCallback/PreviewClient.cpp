...
#define PRVCB_WAIT_BUF_TIMEOUT      10
/******************************************************************************
*
*******************************************************************************/
...
{
    {
        ...
        //
        String8 const s8AppMode = PARAMSMANAGER_MAP_INST(eMapAppMode)->stringFor(mpParamsMgr->getHalAppMode());
        if( s8AppMode == MtkCameraParameters::APP_MODE_NAME_MTK_VT ||
                property_get_bool("debug.camera.vtmode", 0) )
        {
            int32_t sensor_orientation, rotation;
            //
            sp<IMetadataProvider> pMetadataProvider =
                NSMetadataProviderManager::valueFor(mpParamsMgr->getOpenId());
            if( pMetadataProvider.get() ) {
                sensor_orientation = pMetadataProvider->getDeviceWantedOrientation();
                rotation = (sensor_orientation + get_dummy_angle()) % 360;
            }
            //
            mi4Orientation =
                (rotation == 90) ? eTransform_ROT_90 :
                (rotation == 180) ? eTransform_ROT_180 :
                (rotation == 270) ? eTransform_ROT_270 :
                0;
            //
            MY_LOGI("vt mode: rotate %d (sensor %d, dummy %d)",
                    rotation, sensor_orientation, get_dummy_angle());
        }
    }
    //
    initBuffers();
...
stopPreview()
{
    ...
    {
        if(count >= PRVCB_WAIT_BUF_TIMEOUT)
        {
...
updateMsg(int32_t const oldMsgType, int32_t const newMsgType)
{
    ...
    {
        Mutex::Autolock _l(mModuleMtx);
        MY_LOGD_IF((1<=miLogLevel), "+ current mIsMsgEnabled=(%d -> 1)", mIsMsgEnabled);
    ...
    {
        Mutex::Autolock _l(mModuleMtx);
        MY_LOGD_IF((1<=miLogLevel), "+ current mIsMsgEnabled=(%d -> 0)", mIsMsgEnabled);
