...
prepareAllTodoBuffers(sp<IImgBufQueue>const& rpBufQueue, sp<ImgBufManager>const& rpBufMgr)
{
    ...
    for (int i = 0; i < iEnqCount; i++)
    {
        ...
        ret = rpBufQueue->enqueProcessor(
            ImgBufQueNode(pCameraImgBuf, ImgBufQueNode::eSTATUS_TODO, 0, 0, mi4Orientation)
        );
        if  ( ! ret ) {
...
handleReturnBuffers(Vector<ImgBufQueNode>const& rvQueNode)
{
    ...
    for (int32_t i = 0; i < queSize; i++)
    {
        ...
        sp<ICameraImgBuf>       pListImgBuf = NULL;
        {
            Mutex::Autolock _l(mModuleMtx);
            //
            List<ImgBufNode>::iterator iterImgBufNode;
            for(iterImgBufNode = mImgBufList.begin(); iterImgBufNode != mImgBufList.end(); iterImgBufNode++)
            {
                pListImgBuf = (*iterImgBufNode).getImgBuf();     //  ImgBuf in List.
                ...
                MY_LOGD_IF((1<=miLogLevel),"erase list va(%p)", pListImgBuf->getVirAddr());
                mImgBufList.erase(iterImgBufNode);
                break;
            }
        }
        //
