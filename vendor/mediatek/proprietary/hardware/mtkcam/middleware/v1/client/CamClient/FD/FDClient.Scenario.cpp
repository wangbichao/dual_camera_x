...
doFD(ImgBufQueNode const& rQueNode, bool &rIsDetected_FD, bool &rIsDetected_SD, bool rDoSD, bool &rIsDetected_GD, bool rDoGD)
{
    ...
    mpFDHalObj->halFTBufferCreateAsync(mpDDPBuffer, (MUINT8*)(rQueNode.getImgBuf()->getVirAddr()), mFDBufferPlanes, srcWidth, srcHeight);

    struct FD_Frame_Parameters Param;
    Param.pScaleImages = NULL;
    Param.pRGB565Image = (MUINT8 *)mpDDPBuffer;
    Param.pPureYImage  = (unsigned char *)rQueNode.getImgBuf()->getVirAddr();
    Param.pImageBufferVirtual  = (unsigned char *)rQueNode.getImgBuf()->getVirAddr();
    Param.pImageBufferPhyP0 = (unsigned char *)rQueNode.getImgBuf()->getPhyAddr();
    ...
    {
        if(rDoSD == 1 || rDoGD == 1) { // HW FD4.0 don't need extract Y buffer
            mpFDHalObj->halFDYUYV2ExtractY(mpExtractYBuffer, (MUINT8*)(rQueNode.getImgBuf()->getVirAddr()), srcWidth, srcHeight);
        }
