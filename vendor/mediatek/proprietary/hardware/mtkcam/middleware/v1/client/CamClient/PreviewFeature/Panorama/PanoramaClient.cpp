...
#define DUMP_FOLDER "/sdcard/autorama/"
...
PanoramaClient(int ShotNum)
    : PanoramaNum(ShotNum)
{
    MY_LOGI("+ this(%p) num %d", this, PanoramaNum);
    PanoramaClientObj = this;
    allocator = IImageBufferAllocator::getInstance();
    if(!allocator)
    {
        MY_LOGE("+ Memory allocator is Null");
    }
    userCount = 0;
    mDebugLevel = 0;

    mpPanoramaResult = new MTKPipeAutoramaResultInfo();
    PanoramaFuncThread = (pthread_t) NULL;
    sem_init(&PanoramaSemThread, 0, 0);
    sem_init(&PanoramamergeDone, 0, 0);
    mCancel = MFALSE;
    mPanoramaFrameWidth = 0;
    mPanoramaFrameHeight = 0;
    mPanoramaFrameSize = 0;
    mDataCb = (ImgDataCallback_t) NULL;
    mpPanoramaObj = NULL;
    mPanoramaFrameIdx = 0;
...
allocMem(IImageBufferAllocator::ImgParam &memBuf, sp<IImageBuffer> &imgBuf)
{
    ...
    MY_LOGI("imgBuf adr 0x%p imgBuf 0x%p\n",(void*)imgBuf->getBufVA(0),(void*)imgBuf.get());
    return MTRUE;
}
 ...
 *
 ******************************************************************************/
bool PanoramaClient::init(int bufwidth,int bufheight)
{
    ...
    //mpHal3A = MAKE_Hal3A(i4SensorDevId,"MTKPanorama"); //move to autorama_hal.cpp

    // (1) Create frame buffer buffer
    MY_LOGI("mPanoramaFrameWidth %d mPanoramaFrameHeight %d mPanoramaFrameSize %d PanoramaNum %d",
        mPanoramaFrameWidth,mPanoramaFrameHeight,mPanoramaFrameSize,PanoramaNum);
    ...
    MY_LOGD("MOTION_MAX_IN_WIDTH %d MOTION_MAX_IN_HEIGHT %d ",MOTION_MAX_IN_WIDTH,MOTION_MAX_IN_HEIGHT);
    ...
    if(mpPanoramaObj->mHalAutoramaInit(mAutoramaInitInData,mMotionInitInfo) <0)
    {
    ...
    property_get("mtkcam.panorama.debug_level", value, "0");
    mDebugLevel = atoi(value);
    MY_LOGD("mDebugLevel %d \n", mDebugLevel);
...
stopFeature(int cancel)
{
    ...
    bool ret = false;
   int err;
    MY_LOGI("CAM_CMD_STOP_AUTORAMA, do merge %d mPanoramaaddImgIdx %d PanoramaNum %d", cancel,mPanoramaaddImgIdx,PanoramaNum);
    mCancel = MTRUE;
    ...
    if(mpPanoramaObj)
    {
        if ((cancel == 1) || (mPanoramaaddImgIdx == PanoramaNum))
        {
            ...
            MY_LOGI("  CAM_CMD_STOP_AUTORAMA: Merge Accidently ");
        ...
        {
            MY_LOGI("  CAM_CMD_STOP_AUTORAMA: Cancel");
        }
    }
    else
    {
...
mHalCamFeatureAddImg()
{
    ...
    if (mCancel)
    {
        MY_LOGI("mHalCamPanoramaAddImg exit mCancel %d", mCancel);
        return err;
    }
    ...
    if ( err != NO_ERROR) {
         ...
         MY_LOGE("mHalAutoramaCalcStitch(): ret %d", err);
         return err;
     }
...
mHalCamFeatureMerge()
{
    ...
    MY_LOGI(" ImgWidth %d ImgHeight %d ImgBufferAddr 0x%x",
        mpPanoramaResult->ImgWidth, mpPanoramaResult->ImgHeight, mpPanoramaResult->ImgBufferAddr);

    if (mDebugLevel)
    {
        char sourceFiles[80];
        sprintf(sourceFiles, "%sfinal_%dx%d.yuv", DUMP_FOLDER,
            mpPanoramaResult->ImgWidth, mpPanoramaResult->ImgHeight);
        dumpBufToFile((char *) sourceFiles, (MUINT8 *)mpPanoramaResult->ImgBufferAddr ,
            (mpPanoramaResult->ImgWidth * mpPanoramaResult->ImgHeight * 2));
    }
...
{
    MY_LOGI("[mHalCamFeatureCompress]+");
    ...
    mDataCb((MVOID*)mpPanoramaWorkingBuf->getBufVA(0),
            (MINT32)mpPanoramaWorkingBuf->getFD(0),
            mpPanoramaResult->ImgWidth, mpPanoramaResult->ImgHeight);
...
*
********************************************************************************/
MVOID* PanoramaClient::PanoramathreadFunc(void *)
{
    MY_LOGI("[PanoramathreadFunc] +");
