...
dumpBufToFile(char const*const fname, MUINT8 *const buf, MUINT32 const size)
{
    ...
    CAM_LOGD("(name, buf, size) = (%s, %p, %d)", fname, buf, size);
...
{
    return new PreviewFeatureBase(pParamsMgr);
}

/******************************************************************************
 *
 ******************************************************************************/
sp<IFeatureClient> IFeatureClient::createInstance(PreFeatureObject_e eobject,int ShotNum)
{
 ...
 *
 ******************************************************************************/
PreviewFeatureBase::PreviewFeatureBase(sp<IParamsManager> pParamsMgr)
{
    MY_LOGD("+ this(%p)", this);
    BasObj = this;
    ...
    quality = 0;
    onClientThreadLoopThread = (pthread_t) NULL;
    mNotifyCb = (camera_notify_callback) NULL;
    mDataCb = (camera_data_callback) NULL;
    mDataCbTimestamp = (camera_data_timestamp_callback) NULL;
}
 ...
 *
 ******************************************************************************/
PreviewFeatureBase::~PreviewFeatureBase()
{
    MY_LOGD("- this(%p)", this);
}
 ...
 *
 ******************************************************************************/
bool PreviewFeatureBase::uninit()
{
    MY_LOGD("+");
    //
    //
    if  ( 0 != mi4CallbackRefCount )
    {
        ...
        MY_LOGW("Preview Callback: ref count(%d)!=0, the last callback before %d ms, timestamp:(the last, current)=(%d ms,%d  ms)",
                mi4CallbackRefCount,
                (int)(i8CurrentTimeInMs-mi8CallbackTimeInMs),
                (int)mi8CallbackTimeInMs,
                (int)i8CurrentTimeInMs);
    }
    //
    //
 ...
 *
 ******************************************************************************/
bool PreviewFeatureBase::setImgBufProviderClient(sp<IImgBufProviderClient>const& rpClient)
{
    bool ret = false;
    //
 ...
 ******************************************************************************/
void PreviewFeatureBase::setCallbacks(sp<CamMsgCbInfo> const& rpCamMsgCbInfo)
{
    Mutex::Autolock _l(mModuleMtx);
    //
    //  value copy
 ...
 *
 ******************************************************************************/
void PreviewFeatureBase::enableMsgType(int32_t msgType)
{
 ...
 *
 ******************************************************************************/
void PreviewFeatureBase::disableMsgType(int32_t msgType)
{
 ...
 *
 ******************************************************************************/
bool PreviewFeatureBase::isMsgEnabled()
{
 ...
 *
 ******************************************************************************/
bool PreviewFeatureBase::onStateChanged()
{
    bool ret = true;
    //
    ...
    {
#if 1
        MY_LOGD("create onClientThreadLoopThread");
        pthread_create(&onClientThreadLoopThread, NULL, onClientThreadLoopFunc, this);
#else
        status_t status = run("PreviewFeatureBase");
        if  ( OK != status )
        {
            MY_LOGE("Fail to run thread, status[%s(%d)]", ::strerror(-status), -status);
            ret = false;
 ...
 *
 ******************************************************************************/
status_t PreviewFeatureBase::sendCommand(int32_t cmd, int32_t arg1, int32_t)
{
    bool ret = false;
 ...
 *
 ******************************************************************************/
bool PreviewFeatureBase::stopPreview()
{
    bool ret = true;
 ...
 *
 ******************************************************************************/
bool PreviewFeatureBase::stopFeature(int32_t Cancel)
{
    Mutex::Autolock _l(mModuleMtx);
    MY_LOGD("+");
    int err;
...
#if 1
    MY_LOGD("join onClientThreadLoopThread");
    pthread_join(onClientThreadLoopThread, NULL);
#else
    requestExit();
    status_t status = join();
    if  ( OK != status )
 ...
 *
 ******************************************************************************/
bool PreviewFeatureBase::startPanorama(int32_t ShotNum)
{
    ...
    {
        MY_LOGW("FeatureClient is not NULL!, startPanorama without stop");
    }
    //
    if ( !isEnabledState() )
    {
 ...
 ******************************************************************************/
 /*
bool PreviewFeatureBase::stopPanorama(int Cancel)
{
    int err;
    Mutex::Autolock _l(mModuleMtx);
 ...
 *
 ******************************************************************************/
bool PreviewFeatureBase::isEnabledState()
{
 ...
 *
 ******************************************************************************/
bool PreviewFeatureBase::performCallback(int32_t mvX, int32_t mvY, int32_t mStitchDir, MBOOL isShot, MBOOL isSound)
{
    bool ret = true;
 ...
 *
 ******************************************************************************/
bool PreviewFeatureBase::captureDoneCallback(int32_t message, int32_t id, uintptr_t bufferAddr, int32_t bufferSize)
{
    MY_LOGD("+");
    bool ret = true;
 ...
 bool dump = atoi(value);
    ...
    pthread_t  capDoneCBFuncThread;
    pthread_create(&capDoneCBFuncThread, NULL, capDoneCBThreadFunc, pmem);
...
*
********************************************************************************/
MVOID* PreviewFeatureBase::capDoneCBThreadFunc(void *arg)
{
    MY_LOGI("+");

    ::prctl(PR_SET_NAME,"capDoneCBThread", 0, 0, 0);
    ::pthread_detach(::pthread_self()); // Make this thread releases all its resources when it's finish.

    camera_memory* pmem = (camera_memory*)arg;
    sp<CamMsgCbInfo> pCamMsgCbInfo = BasObj->mpCamMsgCbInfo;

    pCamMsgCbInfo->mDataCb(MTK_CAMERA_MSG_EXT_DATA
                            , pmem
                            , 0
                            , NULL
...
*
*******************************************************************************/
MBOOL PreviewFeatureBase::createJpegImg(MVOID* pMPImageInfo
              , MVOID* Srcbuf
              , MINT32 SrcbufmemID
              , int u4SrcWidth
              , int u4SrcHeight
...
*
*******************************************************************************/
MBOOL PreviewFeatureBase::makeExifHeader(MUINT32 const u4CamMode,
...
*
*******************************************************************************/
MBOOL PreviewFeatureBase::handlePanoImgCallBack(MVOID* const puJpegBuf,MINT32 puJpegBufMemID, int u4SrcWidth, int u4SrcHeight)
{
    ...
    BasObj->createJpegImg((MVOID*)&imageBuf
                          ,(MVOID*)puJpegBuf
                          ,(MINT32)puJpegBufMemID
                          ,u4SrcWidth,u4SrcHeight
                          ,jpegBuf->getBufSizeInBytes(0),(MINT32)jpegBuf->getFD(0)
                          ,(MUINT32)jpegBuf->getBufVA(0),(MUINT32)jpegBuf->getBufPA(0)
                          ,u4JpegSize);

    //save final image
    MY_LOGD("[handlePanoImgCallBack] imageBuf 0x%p, u4JpegSize %d", (void*)imageBuf, u4JpegSize);
