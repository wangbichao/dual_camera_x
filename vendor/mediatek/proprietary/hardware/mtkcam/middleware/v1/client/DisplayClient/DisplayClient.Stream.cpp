...
#define CAMERA_DISPLAY_FORMAT_ROTATION      (CameraParameters::PIXEL_FORMAT_YUV422I)
#define CAMERA_DISPLAY_FORMAT_HAL_ROTATION  (HAL_PIXEL_FORMAT_YCbCr_422_I)

#define CAMERA_GRALLOC_USAGE        ( GRALLOC_USAGE_SW_READ_RARELY  \
                                    | GRALLOC_USAGE_SW_WRITE_NEVER  \
                                    | GRALLOC_USAGE_HW_TEXTURE )

#define CAMERA_GRALLOC_USAGE_ROTATION  ( GRALLOC_USAGE_SW_READ_RARELY  \
                                       | GRALLOC_USAGE_SW_WRITE_NEVER  \
                                       | GRALLOC_USAGE_HW_TEXTURE      \
                                       | GRALLOC_USAGE_CAMERA_ORIENTATION )
...
//            "and then call cleanupQueue()"
    ...
    if(mi4Orientation & HAL_TRANSFORM_ROT_90)
    {
...
dequePrvOps(sp<StreamImgBuf>& rpImgBuf)
{
    ...
    profile.print_overtime(30, "dequeue_buffer > 30ms");
    //
    ...
    if(mi4Orientation != 0)
    {
        err = GraphicBufferMapper::get().lock(*phBuffer, CAMERA_GRALLOC_USAGE_ROTATION, bounds, &address);
    }
    else
    {
    ...
    if  ( fdIon >= 0 )
    {
        MY_LOGD_IF((2<=miLogLevel), "getIonFd(): fdIon=%d, status[%d]", fdIon, -err);
    }
    else
    {
        fdIon = -1;
        MY_LOGW("getIonFd(): fdIon=-1, status[%d]", -err);
    }
#endif
    //
...
enquePrvOps(sp<StreamImgBuf>const& rpImgBuf)
{
...
#if ('1'==MTKCAM_HAVE_GRALLOC_EXTRA)
    {
        ...
        //
        int setOrientation = rpImgBuf->getRotation();
    ...
    String8 trace;
    if  ( ATRACE_ENABLED() ) {
        trace = String8::format(
            "Cam:%d:Fwk:Gpu|timestamp(ns):%" PRId64 " duration(ns):%" PRId64 " %dx%d f:%s NeedDisplayRotation:%d Rotation:%d",
            mOpenId, rpImgBuf->getTimestamp(), ::systemTime()-rpImgBuf->getTimestamp(),
            rpImgBuf->getImgWidth(), rpImgBuf->getImgHeight(), rpImgBuf->getImgFormat().string(),
            rpImgBuf->getNeedDisplayRotation(), rpImgBuf->getRotation()
        );
        CAM_TRACE_BEGIN(trace.string());
    }
