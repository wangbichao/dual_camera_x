...
IDisplayClient*
IDisplayClient::
createInstance(int32_t const openId)
{
    return  new DisplayClient(openId);
}
...
*******************************************************************************/
DisplayClient::
DisplayClient(int32_t const openId)
    : IDisplayClient()
    //
    , mModuleMtx()
    , mOpenId(openId)
...
#if 1
/******************************************************************************
*  Restore the buffer count as minimum undequeue buffer count
*******************************************************************************/
void
DisplayClient::
restoreBufCount()
{
    ...
    //
    //  Restore the number of buffers count to >= 3
    MY_LOGI(
        "get min_undequeued_buf_count(%d)",
        min_undequeued_buf_count
    );
    if  ( min_undequeued_buf_count < 3 )
        min_undequeued_buf_count = 3;
    MY_LOGI(
        "restore set_buffer_count = min_undequeued_buf_count(%d)",
        min_undequeued_buf_count
    );
    err = mpStreamOps->set_buffer_count(mpStreamOps, min_undequeued_buf_count);
    if  ( err )
    {
...
createDisplayThread()
{
    ...
    if  (
            mpDisplayThread == 0
        ||  OK != (status = mpDisplayThread->run("Camera@Display"))
        )
    {
        MY_LOGE(
...
setFirstFrameBlack()
{
    MY_LOGD("set first frame black");
    mFirstFrameBlack = true;
}
...
void
DisplayClient::
SetOrientationForDisplay(int32_t orientation)
{
    mi4Orientation = orientation;
    MY_LOGD("set Orientation = %d", mi4Orientation);
}
