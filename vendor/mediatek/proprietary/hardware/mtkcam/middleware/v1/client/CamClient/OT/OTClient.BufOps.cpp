...
createDetectedBuffers()
{
    ...
    if ( NULL != mpDetectedObjs )
    {
        ...
        if ( NULL != obj &&  NULL != posInfo)
        {
            ...
            mpDetectedObjs->ImgWidth = 640; // init value
            mpDetectedObjs->ImgHeight = 480; // init value
...
createWorkingBuffers(sp<IImgBufQueue>const &rpBufQueue)
{
    ...
    //
    MY_LOGD("[seed OT buffer] w: %d, h: %d, format: %s, rot: %d bufsize: %z", bufWidth, bufHeight, format.string(), rot, bufsize);
    //
