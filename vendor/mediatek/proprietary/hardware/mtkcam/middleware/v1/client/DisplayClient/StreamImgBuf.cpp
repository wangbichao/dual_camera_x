...
#define GET_IIMGBUF_IMG_STRIDE_IN_BYTE(pImgInfo, plane)   \
            ((Format::queryPlaneCount(MtkCameraParameters::queryImageFormat(pImgInfo->ms8ImgFormat)) >= (plane+1)) ? \
            ((getImgWidthStride(plane)*Format::queryPlaneBitsPerPixel(MtkCameraParameters::queryImageFormat(pImgInfo->ms8ImgFormat),plane)))>>3 : 0)
...
#define container_of(ptr, type, member) \
    ...
    MY_LOGE_IF(mpANWBuffer->stride != i4Stride,
            "mismatch stride: %d %d", mpANWBuffer->stride, i4Stride);
    MY_LOGE_IF(
            mpANWBuffer->width  != (int)mpImgInfo->mu4ImgWidth,
            "mismatch width: %d %d", mpANWBuffer->width, mpImgInfo->mu4ImgWidth);
    MY_LOGE_IF(mpANWBuffer->height != (int)mpImgInfo->mu4ImgHeight,
            "mismatch height: %d %d", mpANWBuffer->height, mpImgInfo->mu4ImgHeight);
...
#if ('1'==MTKCAM_HAVE_GRALLOC_EXTRA)
        status_t err = 0;
        err = ::gralloc_extra_query(*mpBufHndl, GRALLOC_EXTRA_GET_BYTE_2ND_STRIDE, &mi4Stride);
        if(err)
            MY_LOGE("gralloc_extra_query GRALLOC_EXTRA_GET_BYTE_2ND_STRIDE err = %d",err);
#endif
        mBufSize = (mpImgInfo->mu4ImgWidth * mi4Stride * Format::queryPlaneBitsPerPixel(MtkCameraParameters::queryImageFormat(mpImgInfo->ms8ImgFormat),0))>>3;
    }
    else
    {
        //If not rotated, the fromat must be YV12
        //For YV12 Format
        size_t const y_size = mpImgInfo->mu4ImgHeight * (GET_IIMGBUF_IMG_STRIDE_IN_BYTE(rpImgInfo,0));
        size_t const vu_size= (mpImgInfo->mu4ImgHeight>>1) * ((GET_IIMGBUF_IMG_STRIDE_IN_BYTE(rpImgInfo,1)) + (GET_IIMGBUF_IMG_STRIDE_IN_BYTE(rpImgInfo,2)));
...
getImgWidthStride(uint_t const uPlaneIndex) const
{
    ...
    {
        return ((0 == uPlaneIndex) ? mi4Stride : 0);
    }
}
