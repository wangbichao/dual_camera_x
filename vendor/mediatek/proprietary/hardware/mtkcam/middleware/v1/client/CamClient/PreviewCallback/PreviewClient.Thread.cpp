...
getCommand(Command& rCmd)
{
    ...
    if  ( ! mCmdQue.empty() )
    {
        ...
        //
        if( !isEnabledState() &&
            rCmd.eId != Command::eID_EXIT)
        {
            MY_LOGD("Preview has stopped, que size(%d)", mCmdQue.size());
            ret = false;
        }
    }
    //
...
onClientThreadLoop(Command const& rCmd)
{
    ...
    while   (1)
    {
        ...
        {
            MY_LOGI("Preview client disabled, Msg(%d) Prv(%d) BufQueueRunning(%d)",
                    ::android_atomic_release_load(&mIsMsgEnabled),
                    ::android_atomic_release_load(&mIsPrvStarted),
