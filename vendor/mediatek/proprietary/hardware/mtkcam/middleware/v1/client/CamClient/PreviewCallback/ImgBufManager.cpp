    ...
    , mi4BufCohe(i4BufCohe)
{
     ...
     */
        uint32_t const y_stride = (~15) & (15 + (mu4ImgWidth));    //  Y: 16-alignment
        uint32_t const c_stride = (~15) & (15 + (y_stride>>1));    //  Y: 16-alignment
        size_t const y_size = y_stride * mu4ImgHeight;
        size_t const c_size = c_stride * (mu4ImgHeight>>1);
        mImgBufSize = y_size + c_size + c_size;
    }
    else {
        mImgBufSize = (( mu4BitsPerPixel
                       * NSCam::Utils::Format::queryPlaneWidthInPixels(mi4ImgFormat, 0, mu4ImgWidth)
                       * mu4ImgHeight ) >> 3);
    }
...
getBufStridesInPixels(MUINT index)  const
{
     ...
     */
        uint32_t const y_stride = (~15) & (15 + (mu4ImgWidth));    //  Y: 16-alignment
        uint32_t const c_stride = (~15) & (15 + (y_stride>>1));    //  Y: 16-alignment
        return  (0 == index) ? y_stride : c_stride;
    }
    //
    return NSCam::Utils::Format::queryPlaneWidthInPixels(mi4ImgFormat, index, mu4ImgWidth);
}
...
ImgBufManager(
    ...
    , mi4ImgFormat(MtkCameraParameters::queryImageFormat(szImgFormat))
    , mu4ImgWidth(u4ImgWidth)
    , mu4ImgHeight(u4ImgHeight)
