...
//
#define GET_IIMGBUF_IMG_STRIDE_IN_BYTE(pStreamImgBuf, plane)   \
                ((NSCam::Utils::Format::queryPlaneCount(MtkCameraParameters::queryImageFormat(pStreamImgBuf->getImgFormat().string())) >= (plane+1)) ? \
                ((pStreamImgBuf->getImgWidthStride(plane)*NSCam::Utils::Format::queryPlaneBitsPerPixel(MtkCameraParameters::queryImageFormat(pStreamImgBuf->getImgFormat().string()),plane)))>>3 : 0)
...
prepareOneTodoBuffer(sp<IImgBufQueue>const& rpBufQueue)
{
    ...
    {
        //
        char cDebugColor[PROPERTY_VALUE_MAX] = {'\0'};
        int iDebugColor, iFirstFrameColor;
        ::property_get("debug.camera.FColor", cDebugColor, "0");
        iDebugColor = ::atoi(cDebugColor);
        if (iDebugColor == 0 )
        {
            iFirstFrameColor = 128;
        }
        else
        {
            iFirstFrameColor = iDebugColor;
            MY_LOGD("First Frame color (%d)", iFirstFrameColor);
        }
        MY_LOGD("first Frame black format=%s ori=%d w=%d h=%d bufsize=%d",
            pStreamImgBuf->getImgFormat().string(),
            pStreamImgBuf->getRotation(),
            pStreamImgBuf->getImgWidth(),
            pStreamImgBuf->getImgHeight(),
            pStreamImgBuf->getBufSize()
            );
        //
        char* virtAddr = (char*)pStreamImgBuf->getVirAddr();
        int value = 0;
        if(pStreamImgBuf->getRotation() & HAL_TRANSFORM_ROT_90)  //It also included rot 270 case, because rot 270 = rot90|flipH|flipV )
        {
            ...
            for(int i=0; i<pStreamImgBuf->getBufSize()/2; i++)
            {
               memset((void*)(virtAddr+i*2),value,1);
            }
        }
        else
        {
            ...
            //For YV12 Format
            int YSize = pStreamImgBuf->getImgHeight() * GET_IIMGBUF_IMG_STRIDE_IN_BYTE(pStreamImgBuf,0);
            int UVSize = pStreamImgBuf->getBufSize()-YSize;
            memset((void*)virtAddr, value, YSize);
            memset((void*)(virtAddr+YSize), iFirstFrameColor, UVSize);
        }
        //
    ...
    if(mi4Orientation != 0)
    {
        ret = rpBufQueue->enqueProcessor(
            ImgBufQueNode(pStreamImgBuf, ImgBufQueNode::eSTATUS_TODO, 0, 0, (uint32_t)mi4Orientation)
        );
    }
    else
    {
...
handleReturnBuffers(Vector<ImgBufQueNode>const& rvQueNode)
{
    ...
    for (int32_t i = 0; i < queSize; i++)
    {
        ...
        if  ( i == idxToDisp ) {
            MY_LOGD_IF(
                (1<=miLogLevel),
                "+ %s(%lld) %s(%lld), Show frame:%d %d [ion:%d %p/%d %lld]",
                (_msDuration_buffer_timestamp < 0 ) ? "time inversion!" : "", _msDuration_buffer_timestamp,
                (_msDuration_dequeProcessor > 34) ? "34ms < Duration" : "", _msDuration_dequeProcessor,
