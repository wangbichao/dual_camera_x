...
onClientThreadLoop()
{
    ...
    while ( isEnabledState() )
    {
        ...
        if ( rQueNode.getImgBuf()->getImgWidth() != srcWidth || rQueNode.getImgBuf()->getImgHeight() != srcHeight) {
            MY_LOGD("free buf img(%dx%d), preview(%dx%d)", rQueNode.getImgBuf()->getImgWidth(), rQueNode.getImgBuf()->getImgHeight(), srcWidth, srcHeight);
            continue;
        }
            ...
            if(isMsgEnabled())
            {
                ...
                if(isAsdEnabled) {
                    ...
                    if(mpASDClient != NULL) {
                        ...
                        ASDScene = mpASDClient->update(mpDDPBuffer, srcWidth, srcHeight, FaceNum);
                    }
                }
                #else
                MUINT32 width, height;
