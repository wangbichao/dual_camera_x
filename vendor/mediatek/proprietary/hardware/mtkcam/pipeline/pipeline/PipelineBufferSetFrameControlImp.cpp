...
//
#define LOGLEVEL                    0
#define MY_LOG1(...)                MY_LOGD_IF(1<=LOGLEVEL, __VA_ARGS__)
#define MY_LOG2(...)                MY_LOGD_IF(2<=LOGLEVEL, __VA_ARGS__)
...
onLastStrongRef(const void* /*id*/)
{
    ...
    if  ( pAppCallback == 0 ) {
        ...
        MY_LOG1("requestNo:%u frameNo:%u frame end", getRequestNo(), getFrameNo());
...
)   const
{
    ...
    if  ( ! pItem->mBitStatus.hasBit(eBUF_STATUS_ACQUIRE) ) {
        if  ( pItem->mBitStatus.hasBit(eBUF_STATUS_ACQUIRE_FAILED) ) {
            pItem->mUsersManager->markUserStatus(userId, IUsersManager::UserStatus::RELEASE);
            MY_LOGW(
                "[requestNo:%u frameNo:%u streamId:%#" PRIxPTR "] Failure in previous acquiring buffer",
                getRequestNo(), getFrameNo(), streamId
                );
            return NULL;
        }
        ...
        typename Helper::Params param = { getRequestNo(), userId, pBufferProvider.get() };
...
applyPreRelease(UserId_T userId)
{
    ...
    if ( pAppCallback == 0 ) {
        ...
        MY_LOGD("Prerelease: requestNo:%u frameNo:%u userId:%#" PRIxPTR " OAppMeta#(left:%zd)",
