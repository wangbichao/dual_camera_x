...
dumpMap() const
{
    RWLock::AutoRLock _l(mRWLock);
    //
    MY_LOGD("Consumer Map: size(%d) used(%d) released(%d) prereleased(%d) releasedstilluse(%d)",
            mConsumerMap.size(),
            mConsumerMap.mBitSetUsed.count(), mConsumerMap.mBitSetReleased.count(),
            mConsumerMap.mBitSetPreReleased.count(), mConsumerMap.mBitSetReleasedStillUse.count()
            );
    //
    MY_LOGD("Producer Map: size(%d) used(%d) released(%d) prereleased(%d) releasedstilluse(%d)",
            mProducerMap.size(),
            mProducerMap.mBitSetUsed.count(), mProducerMap.mBitSetReleased.count(),
            mProducerMap.mBitSetPreReleased.count(), mProducerMap.mBitSetReleasedStillUse.count()
            );
