...
namespace {
    ...
    class AppSimulator
        //: public IPipelineFrameControl::IListener
        : public NSCam::v3::Utils::IStreamBufferSetControl::IAppCallback
    {
        public:

                                                AppSimulator() {}
            virtual                            ~AppSimulator() {FUNCTION_IN }
        public:     ////                        Operations.

            virtual MVOID   onFrameDone(
                                IPipelineFrameControl* /*pFrame*/,
                                MVOID* /*pCookie*/
                            )
                            {
                                FUNCTION_IN
        ...
        public:     ////                        Operations.
            ...
            virtual MVOID   updateFrame(
                            ...
                            {
                                MY_LOGD(
                                    "[updateFrame] frameNo:%u userId:%#" PRIxPTR,
                                    frameNo, userId
                                );
                            }

#if 0
            virtual MVOID   returnResult(
                                Result const& result
                            )
                            {
                                MY_LOGD(
                                    "[returnResult] frameNo:%u #image:%zu #meta:%zu",
                                    result.frameNo,
                                    result.vImageBuffers.size(),
                                    result.vMetaBuffers.size()
                                );
                                for (size_t i = 0; i < result.vImageBuffers.size(); i++) {
                                    MY_LOGD(
                                        "[returnResult] %d %s",
                                        result.vImageBuffers[i]->getStreamInfo()->getStreamId(),
                                        result.vImageBuffers[i]->getStreamInfo()->getStreamName()
                                    );
                                }
                                for (size_t i = 0; i < result.vMetaBuffers.size(); i++) {
                                    MY_LOGD(
                                        "[returnResult] %d %s",
                                        result.vMetaBuffers[i]->getStreamInfo()->getStreamId(),
                                        result.vMetaBuffers[i]->getStreamInfo()->getStreamName()
                                    );
                                    PrintMeta(result.vMetaBuffers[i]);
                                }
                            }

         protected:  ////
             virtual MVOID  PrintMeta(sp<IMetaStreamBuffer> const &streamBuffer){
                                IMetadata metadata = *streamBuffer->tryReadLock("appsimu");
                                MY_LOGD("meta id: %d, size: %d",
