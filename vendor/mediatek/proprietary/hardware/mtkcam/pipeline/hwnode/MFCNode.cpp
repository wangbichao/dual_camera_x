...
CRequestFrame::FrameImage::~FrameImage()
{
    MY_LOGD("delete FrameImage streamId:0x%x, nodeId:%d, callerName:%s",
            streamId,
            nodeId,
            callerName.c_str());
...
CRequestFrame::FrameMetadata::~FrameMetadata()
{
    MY_LOGD("delete FrameMetadata streamId:0x%x, nodeId:%d, callerName:%s",
            streamId,
            nodeId,
            callerName.c_str());
...
, m_index(index)
{
    ...
    m_streamIdImages = std::move(imageStreamInfoProvider());
    m_streamIdMetadatas = std::move(metadataStreamInfoProvider());
        ...
        do { // {{{
            ...
            for (size_t i = 0; i < mapSet.size(); i++) {
                    ...
                    (int inout, int index, bool bReleaseOnly) -> int
                {
                    ...
                    {
                        MUINT8 isDrop = 0;
                        tryGetMetadata<MUINT8>(m->pMetadata, MTK_HAL_REQUEST_ERROR_FRAME, isDrop);
                        if (isDrop != 0)
                            return -1;
                    }
                    ...
                    MY_LOGD("process metadata streamId:%#x", m->streamId);
                    // matching by value
                    do {
                        bool bMatched = false;
                    ...
                    (int inout, int index, bool bReleaseOnly = false) -> int
                {
                    ...
                    err = [this, &inout, &in, &i, &index]()->MERROR
                    {
                        ...
                        if (in->imageBufferHeap == NULL) {
                            MY_LOGE("get image buffer heap failed. inout=%d, i=%d, index=%d", inout, i, index);
                            return INVALID_OPERATION;
                        }
                        ...
                        if (in->imageBuffer == NULL) {
                            MY_LOGE("get image buffer failed. inout=%d, i=%d, index=%d", inout, i, index);
                            return INVALID_OPERATION;
                        }
                    ...
                    MY_LOGD("process image streamId:%#x", in->streamId);
                    do {
                        bool bMatched = false;
...
void MFCNodeImp::dumpExifInfo(const IMetadata &halMeta)
{
    ...
    result = tryGetMetaData<IMetadata>(
    ...
    iso.result = tryGetMetaData<MINT32>(&exifMeta, MTK_3A_EXIF_AE_ISO_SPEED,        iso.val);
    exp.result = tryGetMetaData<MINT32>(&exifMeta, MTK_3A_EXIF_CAP_EXPOSURE_TIME,   exp.val);
...
static MINT32 getIso(IMetadata *metaHal)
{
    ...
    result = MFCNodeImp::tryGetMetaData<IMetadata>(
            metaHal,
            MTK_3A_EXIF_METADATA,
            exifMeta);
    if (result == MFALSE) {
    ...
    result = MFCNodeImp::tryGetMetaData<MINT32>(&exifMeta, MTK_3A_EXIF_AE_ISO_SPEED, iso);
    if (result != MTRUE) {
...
MINT32 MFCNodeImp::nrStrategy(MINT32 iso, IMetadata *metaHal)
{
    ...
    // query NR mode
    if (!tryGetMetadata<MINT32>(metaHal, MTK_NR_MODE, nrType))
    ...
    if (nrType == MTK_NR_MODE_AUTO) {
        ...
        if (!tryGetMetadata<MINT32>(metaHal, MTK_NR_SWNR_THRESHOLD_ISO, thr_swnr))
            MY_LOGW("no MTK_NR_SWNR_THRESHOLD_ISO");

        if (!tryGetMetadata<MINT32>(metaHal, MTK_NR_MNR_THRESHOLD_ISO, thr_mnr))
            ...
            )
{
    ...
    /* HWNR case */
        ...
        /* QParams tag */
        params.mvStreamTag.push_back(ENormalStreamTag_Normal); // TODO: time sharing need using ENormalStreamTag_Vss ??
        ...
        /* retrieving register map */
        {
            ...
            registerChunk = std::unique_ptr<char>(new char[sizeof(dip_x_reg_t)]{0});
            rTuningParam.pRegBuf = registerChunk.get();
            MUINT8 restore_3a_isp_profile = 0;

            tryGetMetaData<MUINT8>(metaHal, MTK_3A_ISP_PROFILE, restore_3a_isp_profile);
            updateEntry<MUINT8>(metaHal, MTK_3A_ISP_PROFILE, EIspProfile_Capture_MultiPass_HWNR);
            ...
            updateEntry<MUINT8>(metaHal, MTK_3A_ISP_PROFILE, restore_3a_isp_profile);

            params.mvTuningData.push_back(rTuningParam.pRegBuf);
        }
...
{
    if (!tryGetMetadata<MRect>(pMetadata, MTK_SCALER_CROP_REGION, outRgn)) {
        return MFALSE;
    }
    else {
